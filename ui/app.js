...
const state = {
  ...
  cvSaving: false,   // 畫布存檔中——鎖住存檔鈕，連按兩次不會送兩次 PUT
  ...
  reader: null,        // 閱讀面板（US-103）：{node}＝右側滑出的是哪一步的成品；null＝沒開。狀態在這，輪詢重繪不會關掉它
  ...
  // ---- 合併輪（US-105、ADR-008）：「Workflow 庫」整頁退場，改成 Workflow 頁自己的左樹 ----
  wfEmpty: false,      // Workflow 頁「還沒選任何流程」＝右邊印空狀態三件事（不是「建立新 Workflow」）
  treeQ: '',           // 樹的搜尋字（同時比對流程名與分類名）
  treeOff: false,      // 整棵樹收起（開站時從本機讀回，重開還在）
  recent: null,        // 跨流程最近幾趟（GET /api/dashboard 的 recent，倒序）：樹的狀態點與空狀態三件事共用這一份；null＝還沒讀到
  lastRun: null,       // 目前流程最近一趟的完整紀錄（上次執行摘要、步驟卡標籤用）；null＝沒跑過或讀不到
  catClosed: new Set(), // 樹上收合中的分類（跟整棵樹的收起一起記在本機）
  // ---- 左欄規劃輪（US-107）：主導覽可收、四種讀取狀態、記住上次看的流程 ----
  navOff: false,       // 主導覽收成只剩圖示（224／72；開站時從本機讀回）
  treeLoad: 'ok',      // 流程清單讀取：ok｜loading（重新讀取中）｜error（讀不到——樹停用、右邊保留正在看的內容）
  wfOpening: false,    // 進 Workflow 頁正在打開上次看的那條（右邊骨架、樹停用）
  ...
  sharedUpload: null,  // 公司頁／分類頁按「上傳」後記 {scope, kind}，#shared-file 的 change 事件讀它決定傳到哪一層哪一區
  ...
  wfStats: undefined,  // GET …/runs/stats 近 30 天成效統計（US-099）；undefined＝還沒讀到、null＝讀不到
  shared: { company: null, dept: null, err: null }, // 兩層共用檔清單（GET /api/shared/:scope/files）；null＝還沒讀到；err＝哪一支讀不到的一句
  sharedOpen: false,   // 「公司 N 份・分類 M 份共用檔 → 查看」浮窗
  ...
  rowMenu: null,       // Workflow 列「⋯」選單 {cat, id, name, x, y, sub}（sub＝「移至分類」子清單展開）
  calMore: false,      // 行事曆右上「⋯」（重新整理 Google 快照）開著沒
  assets: null,        // 共用素材整頁 {tab:'全部'|'角色情境'|'常用片段', edit:{field, orig, name, text, err}|null, err}
  // ---- 連線輪（US-100〜102）：使用者 Claude 上已連好的外部服務 ----
  connectors: undefined, // GET /api/connectors 的快取 {checked_at, servers}；undefined＝還沒讀、null＝讀不到
  connBusy: false,     // 「重新檢查」進行中（鎖鈕改字）
  connPick: null,      // 步驟標籤點開的勾選選單 {node, x, y}；Esc、點外面關
  permFlashId: null,   // 健檢「修這裡」要亮哪一列：permissions.files（產檔）｜permissions.connectors（讀服務）
};
...
// ---- 提示與詢問：取代瀏覽器原生的 alert／confirm ----
// 原生那兩個會卡住整個分頁（輪詢停擺、畫面凍住），而且長相不受控、在不同瀏覽器都不一樣。
// toast＝不擋路的通知，右下角浮出自己消失；askBox＝要回答才走得下去的，畫面正中、回 Promise。
let toastSeq = 0;
function toast(msg, kind = 'info') {
  ...
  el.innerHTML = `<span class="tmsg">${esc(text)}</span><button type="button" class="tx" aria-label="關閉這則通知"><i class="ph ph-x"></i></button>`;
  const kill = () => { clearTimeout(timer); el.remove(); if (!host.children.length) host.remove(); };
  el.querySelector('.tx').addEventListener('click', kill);
  // 壞消息留久一點（8 秒），一般的 4.5 秒；滑過就不倒數，免得讀到一半消失
  const ms = kind === 'bad' ? 8000 : 4500;
  let timer = setTimeout(kill, ms);
  el.addEventListener('mouseenter', () => clearTimeout(timer));
  el.addEventListener('mouseleave', () => { timer = setTimeout(kill, 1800); });
  host.appendChild(el);
  return el;
}

// 要回答才走得下去的問句。回 Promise<boolean>：確定＝true，取消／Esc／點背景＝false。
// danger:true 給「會刪東西」的用（主鈕變紅）。呼叫端一律 await，不能再用同步的 if (!confirm())。
function askBox(msg, opts = {}) {
  const { ok = '確定', cancel = '取消', danger = false, title = '' } = opts;
  return new Promise((resolve) => {
    const back = document.createElement('div');
    back.className = 'askback';
    back.innerHTML = `<div class="askbox" role="alertdialog" aria-modal="true"${title ? ` aria-label="${esc(title)}"` : ''}>
      ${title ? `<h3>${esc(title)}</h3>` : ''}
      <p>${esc(String(msg ?? '')).replace(/\n/g, '<br>')}</p>
      <div class="askacts">
        <button type="button" class="btn" data-ask="0">${esc(cancel)}</button>
        <button type="button" class="btn ${danger ? 'btn-danger' : 'btn-primary'}" data-ask="1">${esc(ok)}</button>
      </div></div>`;
    ...
    // Esc／Enter 在捕獲階段就吃掉，不讓底下的畫布快捷鍵或彈窗一起反應
    const onKey = (e) => {
      if (e.key === 'Escape') { e.stopPropagation(); e.preventDefault(); done(false); }
      else if (e.key === 'Enter' && !e.isComposing) { e.stopPropagation(); e.preventDefault(); done(true); }
    };
    ...
    back.querySelector('[data-ask="1"]')?.focus();
  });
}
// 要打一段字才走得下去的問句（取代 window.prompt）。回 Promise<string|null>：取消／Esc／空白＝null。
function askInput(msg, opts = {}) {
  const { ok = '確定', cancel = '取消', value = '', placeholder = '', title = '' } = opts;
  return new Promise((resolve) => {
    const back = document.createElement('div');
    back.className = 'askback';
    back.innerHTML = `<div class="askbox" role="dialog" aria-modal="true"${title ? ` aria-label="${esc(title)}"` : ''}>
      ${title ? `<h3>${esc(title)}</h3>` : ''}
      <p>${esc(String(msg ?? '')).replace(/\n/g, '<br>')}</p>
      <input class="askin" type="text" value="${esc(value)}"${placeholder ? ` placeholder="${esc(placeholder)}"` : ''} autocomplete="off">
      <div class="askacts">
        <button type="button" class="btn" data-ask="0">${esc(cancel)}</button>
        <button type="button" class="btn btn-primary" data-ask="1">${esc(ok)}</button>
      </div></div>`;
    const input = back.querySelector('.askin');
    const prev = document.activeElement;
    const done = (v) => {
      document.removeEventListener('keydown', onKey, true);
      back.remove();
    ...
    const take = () => { const s = input.value.trim(); done(s || null); };
    const onKey = (e) => {
      if (e.key === 'Escape') { e.stopPropagation(); e.preventDefault(); done(null); }
      else if (e.key === 'Enter' && !e.isComposing) { e.stopPropagation(); e.preventDefault(); take(); }
    };
    back.addEventListener('click', (e) => {
      const b = e.target.closest('[data-ask]');
      if (b) (b.dataset.ask === '1' ? take() : done(null));
...
// F1（Codex 09-25 審查）：執行頁的「進頁代次」。進一趟、換一趟、離開執行頁都 +1（進頁四站呼叫 runGenNext、離頁各站走 stopPoll）。
// 輪詢（schedulePoll）與重抓（refreshRun）發請求時記下當時的代次，回應到手先比：代次不同＝這份是給已離開（或已換掉）那一頁的，
// 整個丟掉——不寫 state.run、不重繪、不再排下一輪。用序號不比 run id：A→B→A 回來的 A 是新的一次進頁，舊 A 遲到的回應一樣不准進來。
// 同一頁內輪詢與重抓互相換掉 state.run 不算換頁（代次不動）——否則按鈕後的重抓會被自己的輪詢誤判成舊頁而丟掉。
let runGen = 0;
const runGenNext = () => { runGen += 1; };
// 離開執行頁的每一站都走這裡：停輪詢＋換代。clearTimeout 取消不了已出門的 GET，靠代次把它的回應擋在門外。
const stopPoll = () => { clearTimeout(state.pollTimer); state.pollTimer = null; runGenNext(); };
...
const SOURCE_CHIP = { web: ['AI 上網查', 'chip'], paste: ['你貼・必填', 'chip wait'], upload: ['你上傳', 'chip'], shared: ['組織／分類參考檔', 'chip'], upstream: ['前一步', 'chip'], later: ['從你的服務讀', 'chip'] };
...
let dsKeyHeld = null; // US-107：開站時流程清單讀不到＝鍵先收起來（不讀也不存，免得拿空清單把草稿判成「已刪」），重新讀取成功再接回
let dsTimer = null;
...
function dsLiveKeys() {
  ...
  // 組織清單沒抓到（/api/orgs 失敗那一瞬間開站）就只認得目前這把——
  // 拿不完整的名單去清，會把別的組織沒存的草稿一起清掉（窗口很窄但清掉就回不來）
  if (state.orgsFailed) return live;
...
// 跨分頁同步：兩個分頁各存各的、後關的贏，沒存的草稿就這樣不見。
// storage 事件只有「別的分頁」寫入才會觸發，本頁自己寫不會。
function dsOnStorage(e) {
  if (!dsKey || !e || e.key !== dsKey || e.newValue == null) return;
  // 這邊正在編輯的東西：有的話不敢覆蓋，只提醒；沒有的話直接把人家寫的讀回來
  const editing = Object.keys(dsSnapshot().packs ?? {}).length;
  if (!editing) {
    dsLoad();
    render();
    return;
  ...
  if (!dsWarnedOtherTab) {
    dsWarnedOtherTab = true;
    toast('另一個分頁也在編輯同一份草稿。兩邊的改動不會合併——關掉其中一個再繼續，比較保險。', 'bad');
  }
}
let dsWarnedOtherTab = false;
try { window.addEventListener('storage', dsOnStorage); } catch { /* 隱私模式連掛事件都可能炸 */ }
// 側欄草稿列的「最後編輯」相對時間
function agoText(ms) {
...
// 流程清單讀不到（US-107）：記成 error——Workflow 頁的樹停用、右邊保留正在看的內容附「重新讀取」；讀到了就回 ok。錯誤照樣往外丟，呼叫端原本的處理不變
...
function habitChipsHtml(p) {
  ...
  const chip = (c) => `<span class="hchip${c.probe ? ' probe' : ''}${picked === c.id ? ' on' : ''}" data-act="mem-pick" data-key="${esc(p.key)}" data-id="${esc(c.id)}" title="${esc(scopeHint(c))}" tabindex="0" role="button">
  ...
  const more = list.length > HABIT_SHOW
    ? `<span class="hchip more" data-act="mem-more" data-key="${esc(p.key)}" tabindex="0" role="button">${open ? '收起' : `更多 ${list.length - HABIT_SHOW}`}</span>` : '';
...
function flowMemHtml() {
  ...
  return `<div class="flowmem" data-flowmem>
    <div class="box"><h6><i class="ph ph-identification-card"></i>每步都帶<span class="chip" data-count="core">關於你 ${core.length} 條・分類守則 ${rules.length} 條</span></h6>
      ${m.paused ? '<div class="note" style="margin:0 0 4px">整層暫停中：關於你不帶，分類守則照帶</div>' : ''}${core.length + rules.length ? box1 : none}</div>
...
const MEM_OVER_BY = { group: '分類守則', workflow: '這條 Workflow', run: '這一次的設定' };
function memoryUsedHtml(node, step) {
  ...
  const head = cards.length
    ? `<span class="pbtn" data-act="mem-toggle" data-node="${esc(node.id)}" tabindex="0" role="button"><i class="ph ph-cards"></i>這步用了 ${cards.length} 條記憶 ${open ? '▾' : '▸'}</span>`
  ...
  const over = (mem.overridden ?? []).map((o) => `<div class="over"><i class="ph ph-arrow-bend-down-right"></i><span>這步蓋掉了${String(o.id).startsWith('g-') ? '分類守則' : '核心'}的「${esc(o.text)}」——${MEM_OVER_BY[o.by] ?? esc(o.by)}定了「${esc(o.by_text)}」</span></div>`).join('');
  const list = open ? `<ul>${cards.map((c) => `<li>${memTagHtml(c)}<span class="t">${esc(c.text)}</span><span class="lnk" data-act="mem-card" data-id="${esc(c.id)}" data-bucket="${esc(c.bucket)}" data-text="${esc(c.text)}" data-cat="${esc(cat)}" tabindex="0" role="button">出處</span></li>`).join('')}</ul>` : '';
...
function drawerMemoryHtml(n) {
  const m = state.wfMemory;
  // 組織／分類規範每步自動帶（讀 state.shared 的 rules 數）；記憶讀不到但有規範仍印規範那兩段
  ...
  // 移植第一批 T11（-2）：一行分開數，不加「條」；U4c（-2）：四段 關於你・分類守則・公司規範・分類規範，0 的段省略、全 0 印「沒有記憶與規範」
...
async function memCardAction(act) {
  ...
  if (act === 'del' && !await askBox('把這張卡移到記憶垃圾桶？30 天內都能復原。')) return;
...
async function refreshCompanyName() {
  ...
  try {
    ...
    state.orgsFailed = false;
  } catch {
    state.orgs = []; // 讀不到＝當成單組織：側欄不印切換器，設定頁組織清單空著，其餘照舊
    state.orgId = '';
    state.orgsFailed = true; // 名單不完整——這一輪不准拿它去清別的組織的草稿（見 dsLiveKeys）
  }
}
...
// 存：公司→既有 PUT /api/settings（company_name）；分類→PUT /api/categories/:name；流程→既有存新版本（PUT def，後端註記「改名：舊→新」）。
...
// 左欄規劃輪（US-107）：組織壓成一列「目前組織（小字）／名稱／切換」，不再是一張大卡，也不再另起一列「切換組織 N」
function companyNodeHtml() {
  const now = state.categoryPage?.category === '_company';
  return `<div class="company-row${now ? ' active' : ''}"><div class="wf companynode${now ? ' now' : ''}"${now ? ' aria-current="page"' : ''} data-act="open-company" tabindex="0" role="button" title="組織層共用檔"><span class="company-icon" aria-hidden="true"><i class="ph ph-buildings"></i></span><span class="tree-name"><small>目前組織</small><strong>${esc(companyName())}</strong></span></div>${renameBtnHtml('company', '', '', companyName())}${orgSwitchHtml()}</div>`;
}
// 多組織：組織列尾的「切換」——只有兩個以上組織才印出來（一個組織時沒有東西可換，列上不多一顆鈕）。
// 點開沿用「⋯」選單的 .rowmenu 浮層樣式，目前那個打勾。
function orgSwitchHtml() {
  ...
  return `<div class="org-switch"><button type="button" class="org-switch-btn" data-act="org-menu" aria-haspopup="menu" aria-expanded="${!!state.orgMenu}" title="切換組織（共 ${list.length} 個）" aria-label="切換組織（共 ${list.length} 個）">切換</button>${menu}</div>`;
}
...
// ---------- 側欄（US-105、ADR-008：瘦身）----------
// 只剩品牌、四個全域入口、「＋ 建立新 Workflow」、底部連線狀態。
// 原本住在下半的組織區塊、分類與流程清單、共用素材全部搬進 Workflow 頁自己的樹（wfTreeHtml）。
// 目前在哪一頁：Workflow 頁＝沒有開別的整頁（分類頁、共用素材、垃圾桶都算 Workflow 頁裡面，樹要留著）
const onWfRoute = () => !state.intro && !state.dash && !state.calendar && !state.settings && !state.run;
// 左欄規劃輪（US-107）：主導覽可收成只剩圖示（224／72）。收起時品牌只留「剝繭」、文字標籤與連線狀態藏起來、
// 建立鈕只剩「＋」——所以每個圖示都帶 title／aria-label；收合狀態記在本機（navStateSave／navStateLoad）
function sideHtml() {
  const entry = (on, act, icon, label, title = '') => `<div class="wf calentry ${on ? 'now' : ''}" data-act="${act}" tabindex="0" role="button"${on ? ' aria-current="page"' : ''} title="${title || label}" aria-label="${label}">
      <i class="${on ? 'ph-fill' : 'ph'} ${icon}" aria-hidden="true"></i><span class="wfname">${label}</span>`;
  const off = !!state.navOff;
  const navLabel = off ? '展開主要導覽' : '收起主要導覽';
  return `<aside class="side">
    ...
    ${entry(onWfRoute(), 'open-wf-page', 'ph-flow-arrow', 'Workflow', '左邊一棵樹找 Workflow，右邊直接改')}</div>
    ...
    <div class="newflow" data-act="new-flow" tabindex="0" role="button" title="建立新 Workflow" aria-label="建立新 Workflow"><span class="newflow-ic" aria-hidden="true">＋</span><span class="newflow-label">建立新 Workflow</span></div>
    <button type="button" class="navtoggle" data-act="nav-toggle" aria-expanded="${!off}" aria-label="${navLabel}" title="${navLabel}"><span class="navtoggle-ic" aria-hidden="true">${off ? '›' : '‹'}</span><span class="navtoggle-label">${off ? '展開導覽' : '收起導覽'}</span></button>
...
// 複製（上桌a）：讀定義另存一份，名字加「（副本）」，同分類；不帶執行紀錄與版本（新 id 由後端配）
...
// 移至分類：既有搬移 API；正在看的那條（state.wf／執行頁 state.run）跟著換分類，暫存包換鍵（同分類改名的同步）
...
// ---------- 合併輪（US-105、ADR-008）：Workflow 頁的左樹（組織 → 分類 → 流程）----------
// 「Workflow 庫」那一頁退場——「找到某個流程」這件事由這棵樹做，樹只到流程為止（步驟不進樹，展開的事交給右邊）。
// 樹只讀 state.workflows／state.categories／state.recent，自己不叫 API。
const TREE_KEY = 'bojian.tree';
// 摺疊狀態記在本機（隱私模式連讀 localStorage 都會炸，dsStore 已經包好 try）：{off, closed:[分類名]}
function treeStateSave() {
  try { dsStore()?.setItem(TREE_KEY, JSON.stringify({ off: !!state.treeOff, closed: [...state.catClosed] })); } catch { /* 存不進＝重開時全部展開，不擋畫面 */ }
}
function treeStateLoad() {
  try {
    const raw = dsStore()?.getItem(TREE_KEY);
    if (!raw) return;
    const v = JSON.parse(raw);
    state.treeOff = v?.off === true;
    if (Array.isArray(v?.closed)) state.catClosed = new Set(v.closed.filter((c) => typeof c === 'string'));
  } catch { /* 壞值當沒存過 */ }
}
// 主導覽收合（US-107 ①）：同一套 try 包法——隱私模式照常可用，只是不記
const NAV_KEY = 'bojian.nav';
function navStateSave() {
  try { dsStore()?.setItem(NAV_KEY, state.navOff ? 'off' : 'on'); } catch { /* 存不進＝重開時展開，不擋畫面 */ }
}
function navStateLoad() {
  try { state.navOff = dsStore()?.getItem(NAV_KEY) === 'off'; } catch { state.navOff = false; }
}
// 上次看的流程（US-107 ⑥）：記在本機 {org, category, id}。進 Workflow 頁時直接打開它；
// 組織不同、清單裡找不到（刪了）＝回 null 走落地頁。搬了分類或分類改名：同組織裡 id 只有一條就認它（不因改名丟掉）
const LAST_FLOW_KEY = 'bojian.lastFlow';
function lastFlowSave(category, id) {
  try { dsStore()?.setItem(LAST_FLOW_KEY, JSON.stringify({ org: state.orgId ?? '', category, id })); } catch { /* 存不進＝下次進來看落地頁 */ }
}
function lastFlowPick() {
  if (state.treeLoad === 'error' || state.treeLoad === 'loading') return null; // 清單讀不到（或還在讀）時不能拿空清單判「刪了」
  try {
    const ls = dsStore();
    const v = JSON.parse(ls?.getItem(LAST_FLOW_KEY) ?? 'null');
    if (!v || typeof v.id !== 'string' || (v.org ?? '') !== (state.orgId ?? '')) return null;
    const same = state.workflows.find((w) => w.category === v.category && w.id === v.id);
    const byId = state.workflows.filter((w) => w.id === v.id);
    const hit = same ?? (byId.length === 1 ? byId[0] : null);
    if (!hit) { ls.removeItem(LAST_FLOW_KEY); return null; } // 那條已經不在了：忘掉它
    return { category: hit.category, id: hit.id };
  } catch { return null; } // 壞值／隱私模式＝當沒記過
}
// 四種讀取狀態（US-107 ③）：loading＝骨架、error＝清單讀不到、empty＝組織還沒有流程、partial＝執行狀態只讀回一部分、ideal＝正常
// 「只讀回一部分」＝清單回來了，但有幾筆沒帶 last_run 或帶了卻缺狀態（例如後端還是舊版）——那幾筆的灰點不能說「沒跑過」。
// 狀態點改讀每條流程自己的最近一趟之後，不再因為趟數多就出現（清單那支整個讀不到＝error，另有畫面）
function lastRunMissing(w) {
  if (!w || !Object.prototype.hasOwnProperty.call(w, 'last_run')) return true;
  const r = w.last_run;
  return r !== null && (typeof r !== 'object' || typeof r.status !== 'string');
}
function statusPartial() {
  return state.workflows.some(lastRunMissing);
}
function wfLoadState() {
  if (state.treeLoad === 'loading' || state.wfOpening) return 'loading';
  if (state.treeLoad === 'error') return 'error';
  if (!state.workflows.length) return 'empty';
  return statusPartial() ? 'partial' : 'ideal';
}
// 右邊的狀態畫面（文案照使用者的 demo）。compact＝附在正在看的內容上方（不清掉內容）；否則整塊置中
function wfStatePanelHtml(kind, compact = false) {
  const cls = `state-panel${compact ? ' compact' : ''}`;
  if (kind === 'loading') return `<section class="${cls}" data-screen="loading" aria-label="正在載入 Workflow" aria-busy="true"><div class="skeletons"><div class="skeleton"></div><div class="skeleton"></div><div class="skeleton"></div></div></section>`;
  if (kind === 'error') return `<section class="${cls}" data-screen="error" role="alert"><div><div class="state-icon"><i class="ph ph-warning-circle"></i></div><h2>Workflow 清單讀不到</h2><p>目前無法讀取工作樹。右邊保留正在看的內容，避免操作中斷。</p><button class="btn" type="button" data-act="tree-retry">重新讀取</button></div></section>`;
  if (kind === 'partial') return `<section class="${cls}" data-screen="partial"><div><div class="state-icon"><i class="ph ph-circle-half"></i></div><h2>部分執行狀態還沒讀回來</h2><p>Workflow 內容可以照常編輯；灰色圓點暫時不代表「沒跑過」。</p><button class="btn" type="button" data-act="recent-retry">再讀一次狀態</button></div></section>`;
  // empty：組織還沒有任何流程。垃圾桶在樹底（此時停用），裡面有東西就在這裡給一個還原的路
  const trash = state.trash?.length ? `<button class="btn" type="button" data-act="view-trash"><i class="ph ph-trash"></i>垃圾桶（${state.trash.length}）</button>` : '';
  return `<section class="${cls}" data-screen="empty" data-wf-empty><div><div class="state-icon"><i class="ph ph-flow-arrow"></i></div><h2>這個組織還沒有 Workflow</h2><p>先建立一條常做的工作，之後就能從左邊直接找到並執行。</p>
    <div class="btns"><button class="btn btn-primary" type="button" data-act="new-flow">建立第一條 Workflow</button><button class="btn" type="button" data-act="pick-import"><i class="ph ph-upload-simple"></i>匯入</button>${trash}</div></div></section>`;
}
// Workflow 頁右邊外面再包一層狀態：清單讀不到／狀態只讀回一部分＝提示條放在正在看的內容上方（內容不清掉、照常可編輯）；
// 落地頁（沒選流程）時讀不到＝整塊提示；載入中只有沒東西可看時才換成骨架
function wfStateWrap(inner, landing) {
  const ls = wfLoadState();
  if (ls === 'loading') return landing ? wfStatePanelHtml('loading') : inner;
  if (ls === 'error') return landing ? wfStatePanelHtml('error') : wfStatePanelHtml('error', true) + inner;
  if (ls === 'partial') return wfStatePanelHtml('partial', true) + inner;
  return inner;
}
// 狀態點只讀這條流程自己的最近一趟、不做彙總（ADR-008 第 5 條）：資料＝GET /api/workflows 每筆的 last_run（US-107，不再有 50 趟上限）。
// ok＝完成、half＝進行中或停在半路、none＝還沒跑過、lost＝那一趟的紀錄讀不到（灰色，但不冒充「沒跑過」或「停在半路」，收 L062）
function wfDotTone(row) {
  return !row ? 'none' : row.status === 'done' ? 'ok' : row.status === 'unreadable' ? 'lost' : 'half';
}
const DOT_TITLE = { ok: '上次跑完了', half: '上次停在半路', none: '還沒跑過', lost: '這趟紀錄讀不到' };
// unsure＝執行狀態只讀回一部分（US-107 ③）：灰點不能說「還沒跑過」，改說「執行狀態尚未讀回」；綠黃是讀回來的，照舊
function wfDotHtml(row, unsure = false) {
  const tone = wfDotTone(row);
  const label = tone === 'none' && unsure ? '執行狀態尚未讀回' : DOT_TITLE[tone];
  return `<span class="flow-st ${tone}" role="img" title="${label}" aria-label="${label}"></span>`;
}
// 搜尋有沒有落空（US-107 ④）：有字、而且一條流程都沒命中——搜尋框底下出「清除搜尋」鈕
function treeSearchMiss() {
  const q = (state.treeQ ?? '').trim().toLowerCase();
  if (!q) return false;
  return !state.workflows.some((w) => catLabel(w.category).toLowerCase().includes(q) || String(w.name ?? '').toLowerCase().includes(q));
}
function searchEmptyHtml() {
  return `<button type="button" class="search-empty" id="tree-search-empty" data-act="tree-search-clear"${treeSearchMiss() ? '' : ' hidden'}>找不到符合項目，清除搜尋</button>`;
}
// 分類與流程列（搜尋只換這一塊，不整頁 render——打字不被洗掉）
function treeNodesHtml() {
  const q = (state.treeQ ?? '').trim().toLowerCase();
  const byCat = {};
  for (const c of state.categories) byCat[c] = [];
  for (const w of state.workflows) (byCat[w.category] ??= []).push(w);
  // 開著分類頁／共用素材／垃圾桶／空狀態時，流程列不該還標「正在看」
  const onPage = state.categoryPage || state.assets || state.showTrash || state.importPreview || state.wfEmpty;
  const catHit = (c) => catLabel(c).toLowerCase().includes(q);
  const wfHit = (w) => String(w.name ?? '').toLowerCase().includes(q);
  let hits = 0;
  const cats = Object.entries(byCat).map(([cat, all]) => {
    // 搜尋比對流程名與分類名：分類名命中＝整個分類留著；否則只留名字命中的流程，兩邊都沒命中的分類整個不印
    const wfs = !q || catHit(cat) ? all : all.filter(wfHit);
    if (q && !catHit(cat) && !wfs.length) return '';
    hits += wfs.length;
    const now = state.categoryPage?.category === cat;
    // 搜尋中有命中的分類一律展開（不然命中的流程躲在收合的分類裡看不到）；只改畫面、不動 catClosed，清掉搜尋字就回到原本的收合
    const open = now || !!q || !state.catClosed.has(cat);
    const rows = wfs.map((w) => {
      const nowFlow = !!(state.wf && !onPage && !subjectIsDraft() && w.id === state.wf.id && w.category === state.wf.category);
      return `<div class="flow-row${nowFlow ? ' active' : ''}"><div class="wf${nowFlow ? ' now' : ''}" data-act="open" data-cat="${esc(w.category)}" data-id="${esc(w.id)}" tabindex="0" role="button"${nowFlow ? ' aria-current="page"' : ''}>${wfDotHtml(lastRunMissing(w) ? null : w.last_run, lastRunMissing(w))}<span class="tree-name">${esc(w.name)}</span></div>${renameBtnHtml('flow', w.category, w.id, w.name)}<button type="button" class="rowmore" data-act="row-menu" data-cat="${esc(w.category)}" data-id="${esc(w.id)}" data-name="${esc(w.name)}" title="更多" aria-haspopup="menu" aria-label="「${esc(w.name)}」更多動作"><i class="ph ph-dots-three"></i></button></div>`;
  ...
  if (q && !hits) return treeDraftRowHtml(); // 都沒命中：分類整批不印，改由搜尋框底下的「找不到符合項目，清除搜尋」交代（searchEmptyHtml）
  return `${treeDraftRowHtml()}<div class="company-children">${cats}</div>`; // 樹線與縮排沿用側欄那一套（.company-children）
}
// 還沒存進 Workflow 庫的草稿一列（原本在側欄「＋ 建立新 Workflow」下方，隨樹搬進來）
function treeDraftRowHtml() {
  const onPage = state.categoryPage || state.assets || state.showTrash || state.wfEmpty;
  const draftNow = !onPage && draftLive();
  if (!wsByFlow.has('__draft__') && !draftNow) return '';
...
// 整棵樹：上＝組織一列、搜尋框（落空時底下一顆清除鈕）；中＝分類與流程；下＝共用素材、垃圾桶。
// 左欄規劃輪（US-107 ②）：樹貼齊整頁（不是浮起的卡片）、固定寬 248，收合鈕是貼在樹右緣的小分頁鈕；
// 收起時只剩 38 寬的一條窄邊可再展開（右邊寬度回到合併前，畫布不因此變窄——ADR-008 第 4 條）。
// 載入中／清單讀不到／組織還沒有流程＝樹停用（半透明、不可點，inert 讓鍵盤也進不去）；收合鈕不停用
function wfTreeHtml() {
  const toggle = (off) => `<button type="button" class="treebtn" data-act="tree-toggle" aria-expanded="${!off}" title="${off ? '展開 Workflow 工作樹' : '收起 Workflow 工作樹，右邊變寬'}" aria-label="${off ? '展開' : '收起'} Workflow 工作樹">${off ? '›' : '‹'}</button>`;
  if (state.treeOff) return `<div class="wftree off" data-tree="off">${toggle(true)}</div>`;
  const ls = wfLoadState();
  const unavail = ls === 'loading' || ls === 'error' || ls === 'empty';
  const lock = unavail ? ' inert' : '';
  return `<div class="wftree${unavail ? ' unavail' : ''}" data-tree="on" data-load="${ls}">${toggle(false)}<div class="treehead"${lock}>${companyNodeHtml()}
    <input id="tree-search" class="treesearch" type="search" placeholder="搜尋 Workflow 或分類⋯" aria-label="搜尋 Workflow 或分類" value="${esc(state.treeQ ?? '')}"${unavail ? ' disabled' : ''}>${searchEmptyHtml()}</div>
    <div class="treenodes" id="tree-nodes" aria-busy="${ls === 'loading'}"${lock}>${treeNodesHtml()}</div>
    <div class="treefoot"${lock}><div class="asset-entry${state.assets ? ' active' : ''}" data-act="open-assets" tabindex="0" role="button"${state.assets ? ' aria-current="page"' : ''}><i class="ph ph-files"></i>共用素材</div>
      <div class="asset-entry trash-entry${state.showTrash ? ' active' : ''}" data-act="view-trash" tabindex="0" role="button"${state.showTrash ? ' aria-current="page"' : ''}><i class="ph ph-trash"></i>垃圾桶（${state.trash.length}）</div></div></div>`;
}
// 流程卡（ 抽出； 照樣稿 uxFlowCards）：分類頁用；「打開」走既有 open，「⋯」共用 rowMenuHtml
...
// ---------- 空狀態（US-106）：還沒選任何流程時右邊不留白，列三件事 ----------
// 資料就是樹的狀態點那一份（state.recent，GET /api/dashboard 的 recent），不另外叫 API。
// 三件事各自算不出來就那一列不出現；三列都沒有才出建立引導。
function wfEmptyRowsHtml() {
  const recent = state.recent ?? [];
  const rows = [];
  // ① 卡住的那趟＝最近一趟不是完成也不是還在跑的（停在半路等你）
  const stuck = recent.find((r) => r.status !== 'done' && r.status !== 'running');
  if (stuck) {
    rows.push(`<div class="lastrun" data-empty-row="stuck"><span class="chip wait"><i class="ph-fill ph-hand-palm"></i>有一趟停在半路</span>
      <span class="lrtxt">${esc(stuck.name ?? stuck.id)}・${esc(fmtWhen(stuck.started_at) || '時間讀不到')} 開跑</span>
      <button class="btn sm2 btn-primary" data-act="todo-go" data-cat="${esc(stuck.category)}" data-id="${esc(stuck.id)}" data-rid="${esc(stuck.run_id)}">去處理</button></div>`);
  }
  // ② 最近跑完的
  const done = recent.find((r) => r.status === 'done');
  if (done) {
    rows.push(`<div class="lastrun" data-empty-row="done"><span class="chip quiet"><i class="ph-fill ph-check-circle"></i>最近跑完的</span>
      <span class="lrtxt">${esc(done.name ?? done.id)}・${esc(fmtWhen(done.finished_at ?? done.started_at) || '時間讀不到')}・${(done.finals ?? []).length} 份成品</span>
      <button class="btn sm2" data-act="open-run-reader" data-src="empty" data-cat="${esc(done.category)}" data-id="${esc(done.id)}" data-rid="${esc(done.run_id)}">看成品</button></div>`);
  }
  // ③ 還沒跑過的＝recent 裡完全沒出現過的流程（recent 有上限，所以這裡說的是「最近沒跑過」）
  const ran = new Set(recent.map((r) => `${r.category}/${r.id}`));
  // US-107：清單帶了 last_run 就用它（真的沒跑過＝null，不受最近幾趟的上限影響）；沒帶的才退回看 recent
  const fresh = state.workflows.find((w) => (lastRunMissing(w) ? !ran.has(`${w.category}/${w.id}`) : w.last_run === null));
  if (fresh) {
    rows.push(`<div class="lastrun" data-empty-row="fresh"><span class="chip quiet"><i class="ph ph-moon"></i>還沒跑過的</span>
      <span class="lrtxt">${esc(fresh.name)}・${esc(catLabel(fresh.category))}</span>
      <button class="btn sm2" data-act="try-run" data-cat="${esc(fresh.category)}" data-id="${esc(fresh.id)}">試跑</button></div>`);
  }
  return rows.join('');
}
function wfEmptyHtml() {
  const n = state.workflows.length;
  // 組織還沒有任何流程（US-107 ③ empty）：使用者 demo 的引導畫面「建立第一條 Workflow」；匯入是這個畫面原本就有的入口，留著
  if (!n) return wfStatePanelHtml('empty');
  const rows = wfEmptyRowsHtml();
  return `<div class="wfempty" data-wf-empty><h1>${n} 條 Workflow</h1>
    <p class="sub">左邊選一個開始，或看看有沒有卡住的。</p>${rows}</div>`;
}
...
function crumbsHtml(extra) {
  ...
  return `<nav class="crumbs" aria-label="資料夾路徑">${items.map((x, i) => `${i ? '<i class="ph ph-caret-right"></i>' : ''}${i === last
    ...
    : `<span data-act="${x.act}"${x.cat ? ` data-cat="${esc(x.cat)}"` : ''} tabindex="0" role="button">${esc(x.label)}</span>`}`).join('')}</nav>`;
}
...
function workHeadHtml() {
  const def = subjectDef();
  // （樣稿 uxChatPage newFlow）：還沒拆出草稿＝建立新 Workflow 頁——麵包屑回 Workflow 頁＋頁標題；沒東西可看，不印四模式
  if (!def) return `<nav class="crumbs" aria-label="資料夾路徑"><span data-act="open-wf-page" tabindex="0" role="button">${esc(companyName())}</span><i class="ph ph-caret-right"></i><span aria-current="page">建立新 Workflow</span></nav>${pageHeadHtml('把工作說清楚', '先確認成品長相，再拆成可以逐步驗收的 Workflow。', '<button class="btn" data-act="open-wf-page">回 Workflow</button>')}`;
  ...
  // 驗收第 5 條：保留「Workflow 設定」，旁邊「⋯」（移至分類／複製／匯出／移到垃圾桶，共用 rowMenuHtml）；分類歸屬由麵包屑交代
  ...
  const tools = saved ? `<div class="actions"><span class="saved">${state.cvSaving ? '存檔中⋯' : (state.cvDirty ? '有改動未存檔' : '所有變更已儲存')}</span><button class="btn" data-act="cv-save"${state.cvDirty && !state.cvSaving ? '' : ' disabled'}>${state.cvSaving ? '存檔中⋯' : '存檔'}</button><button class="btn btn-primary" data-act="flow-tab" data-tab="data">準備執行</button></div>` : '';
...
function permRowHtml() {
  ...
  const flash = state.permFlashUntil > Date.now() && state.permFlashId !== 'permissions.connectors';
...
// 剝繭不保管帳密：清單是宿主（使用者的 Claude）已連好的服務，後端快取在 GET /api/connectors，「重新檢查」才 POST refresh 重抓。
// 步驟 connectors＝服務全名陣列（畫面用 label），connectors_set_by＝ai｜user（使用者改過之後拆解器不再改回去）。
const CONN_LINK = 'https://claude.ai/settings/connectors';
const connServers = () => state.connectors?.servers ?? [];
// 顯示名：清單上有就用它的 label；沒有（拿掉了、還沒檢查）就把「claude.ai 」前綴去掉
const connLabel = (name) => connServers().find((s) => s.name === name)?.label ?? String(name ?? '').replace(/^claude\.ai\s+/, '');
const CONN_STATUS = { connected: ['已連上・只讀', 'chip'], 'needs-auth': ['還沒授權', 'chip wait'], failed: ['連不上', 'chip bad'] };
// 上次檢查：今天＝HH:MM，不是今天＝M/D HH:MM
function connTimeText(iso) {
  const d = new Date(iso);
  if (Number.isNaN(d.getTime())) return '';
  const hm = `${String(d.getHours()).padStart(2, '0')}:${String(d.getMinutes()).padStart(2, '0')}`;
  return d.toDateString() === new Date().toDateString() ? hm : `${d.getMonth() + 1}/${d.getDate()} ${hm}`;
}
// render 途中用：還沒讀過就背景讀一次，回來重畫（打字中不重畫，下次重繪自然帶上）
let connLoading = false;
function connEnsure() {
  if (state.connectors !== undefined || connLoading) return;
  connLoading = true;
  api('GET', '/api/connectors').then((r) => { state.connectors = r; }, () => { state.connectors = null; })
    .finally(() => { connLoading = false; if (!isTyping()) render(); });
}
// 「重新檢查」：設定頁、健檢擋下卡共用；成功後健檢重算（剛授權好的服務馬上不擋）
async function refreshConnectors() {
  if (state.connBusy) return;
  state.connBusy = true;
  render();
  try {
    state.connectors = await api('POST', '/api/connectors/refresh', {});
  } catch (e) {
    toast(`服務清單沒檢查成：${e.message}`, 'bad');
  } finally {
    state.connBusy = false;
  }
  render();
  if (state.wf && !subjectIsDraft()) recheckHealth();
}
const connGoLink = (primary = false, href = CONN_LINK) => `<a class="btn sm2${primary ? ' btn-primary' : ''}" href="${esc(/^https:\/\/claude\.ai\//.test(href) ? href : CONN_LINK)}" target="_blank" rel="noopener"><i class="ph ph-arrow-square-out"></i>去 claude.ai 連</a>`;
const connCheckBtn = (label = '重新檢查') => `<button class="btn sm2" data-act="conn-refresh"${state.connBusy ? ' disabled' : ''}><i class="ph ph-arrows-clockwise"></i>${state.connBusy ? '檢查中⋯' : label}</button>`;
// 設定→連線→外部服務：頭一列「上次檢查 HH:MM」＋重新檢查；每家一列＝名稱、一句話、狀態膠囊（沒連上的附「去 claude.ai 連」）
function setExtHtml() {
  const c = state.connectors;
  const why = hint('剝繭不保管任何帳密，授權都留在你的 Claude 上；第一版只准讀，寄出、修改、刪除一律擋。想加新的服務：到 claude.ai 的連接器頁授權，回來按「重新檢查」。');
  if (c === undefined) return `<div class="group" data-connectors><h5>你的 Claude 連好的服務${why}</h5>${setRow('讀取中⋯', '', '')}</div>`;
  if (c === null) return `<div class="group" data-connectors><h5>你的 Claude 連好的服務${why}</h5>${setRow('服務清單讀不到', '', connCheckBtn('檢查一次'))}</div>`;
  if (!c.checked_at) return `<div class="group" data-connectors><h5>你的 Claude 連好的服務${why}</h5>${setRow('還沒檢查過', '', connCheckBtn('檢查一次'))}</div>`;
  const list = c.servers ?? [];
  const rows = list.map((s) => {
    const [txt, cls] = CONN_STATUS[s.status] ?? CONN_STATUS.failed;
    return `<div class="row connrow" data-conn="${esc(s.name)}"><div class="l">${esc(s.label ?? s.name)}</div>${s.summary ? `<div class="d">${esc(s.summary)}</div>` : ''}<div class="c"><span class="${cls}">${txt}</span>${s.status === 'connected' ? '' : connGoLink()}</div></div>`;
  }).join('') || setRow('你的 Claude 上還沒有連好的服務', '', connGoLink());
  return `<div class="group" data-connectors><div class="connhead"><h5>你的 Claude 連好的服務・${list.length} 家${why}</h5><span class="conntime">上次檢查 ${esc(connTimeText(c.checked_at))}</span>${connCheckBtn()}</div>${rows}</div>`;
}
// 步驟上的小標籤（清單卡）：勾了＝藍標籤常駐、沒勾＝灰「＋ 用服務」；點了開勾選選單。人做步驟與分岔等不出。
// US-113 只印例外：沒勾那顆帶 .hoveronly——位置照舊佔著、卡片滑過／focus-within 才浮出（opacity/visibility，不跳動）；右欄檢視器走 connPickerHtml 不受影響
function stepConnTagHtml(n) {
  if (kindOf(n) !== 'task' || n.executor === 'human') return '';
  const on = n.connectors ?? [];
  const body = on.length ? on.map((x) => `<span class="chip blue">${esc(connLabel(x))}</span>`).join('') : '<span class="chip quiet">＋ 用服務</span>';
  return `<button type="button" class="conntag${on.length ? '' : ' hoveronly'}" data-act="conn-pick" data-node="${esc(n.id)}" aria-haspopup="true" title="這步要用哪家服務（點開改）">${body}<i class="ph ph-caret-down"></i></button>`;
}
const connToggled = (list, name) => ((list ?? []).includes(name) ? (list ?? []).filter((x) => x !== name) : [...(list ?? []), name]);
// 勾選區（選單、檢視器、步驟彈窗同一個）：dlg＝彈窗版——點了只改畫面與隱藏欄，按「套用」才寫回（跟彈窗其他欄位一樣）
function connPickerHtml(n, dlg = false) {
  const on = n.connectors ?? [];
  const list = connServers();
  const hidden = dlg ? `<input type="hidden" id="cv-connectors" value="${esc(JSON.stringify(on))}">` : '';
  if (!list.length && !on.length) {
    const why = state.connectors === undefined ? '讀取中⋯' : state.connectors?.checked_at ? '你的 Claude 上還沒有連好的服務。' : '還沒檢查過你的服務。';
    return `<div class="connpick">${hidden}<span class="connnone">${why}</span><button type="button" class="btn btn-mini" data-act="conn-go-settings">去設定看</button></div>`;
  }
  const act = dlg ? 'conn-dlg-toggle' : 'conn-toggle';
  const names = [...list.map((s) => s.name), ...on.filter((x) => !list.some((s) => s.name === x))];
  const pills = names.map((name) => {
    const s = list.find((x) => x.name === name);
    const picked = on.includes(name);
    const bad = !s ? '不在清單上' : s.status === 'needs-auth' ? '還沒授權' : s.status !== 'connected' ? '連不上' : '';
    const dis = bad && !picked; // 已勾但沒授權的還能取消
    const text = `${connLabel(name)}${bad ? `（${bad}）` : ''}`;
    return `<button type="button" class="pill${picked ? ' on' : ''}" data-act="${act}" data-node="${esc(n.id)}" data-conn-name="${esc(name)}" data-conn-text="${esc(text)}" aria-pressed="${picked}"${dis ? ' disabled' : ''}>${picked ? '<i class="ph ph-check"></i>' : ''}${esc(text)}</button>`;
  }).join('');
  const ai = n.connectors_set_by === 'ai'
    ? `<p class="connai">${on.length ? `AI 建議勾 ${esc(on.map(connLabel).join('、'))}` : 'AI 建議這步不用服務'}。你改過之後，它就不會再自動改回去。</p>` : '';
  return `<div class="connpick">${hidden}<div class="pills">${pills}</div>${ai}</div>`;
}
// 標籤點開的小選單（照「⋯」選單的做法：掛浮窗串尾、fixed 定位、Esc／點外面關）
function connPickPopHtml() {
  const p = state.connPick;
  if (!p) return '';
  const n = cvDef()?.nodes.find((x) => x.id === p.node);
  if (!n) return '';
  return `<div class="connpop" role="dialog" aria-label="這步要用哪家服務" style="left:${p.x}px;top:${p.y}px"><b>這步要用</b>${connPickerHtml(n)}</div>`;
}
function connPickPos(r, vw, vh) {
  const w = 300;
  return { x: Math.round(Math.max(8, Math.min(r.left, vw - w - 8))), y: Math.round(Math.max(8, Math.min(r.bottom + 6, vh - 200))) };
}
// 開跑卡那一行：健檢頂層 connector_reads [{node, index, labels}] 依服務併起來——「Gmail（第 1、3 步）、雲端硬碟（第 2 步）」
function connReadsHtml(result, def) {
  const reads = result?.connector_reads ?? [];
  if (!reads.length) return '';
  const seq = def ? stepSeqMap(def) : new Map();
  const by = new Map();
  for (const r of reads) {
    const i = seq.get(r.node) ?? r.index;
    // 名字跟步驟標籤同一套（清單上的 label）；那一步在定義裡找不到才用健檢給的 labels
    const own = def?.nodes.find((x) => x.id === r.node)?.connectors;
    for (const l of own?.length ? own.map(connLabel) : r.labels ?? []) (by.get(l) ?? by.set(l, []).get(l)).push(i);
  }
  if (!by.size) return '';
  const parts = [...by].map(([l, is]) => `${esc(l)}（第 ${[...new Set(is)].join('、')} 步）`).join('、');
  return `<p class="connreads" data-connreads><i class="ph ph-eye"></i><span>這一趟會<b>讀</b>你的 ${parts}。不會寄出或修改任何東西。</span></p>`;
}
// 連線兩條健檢的人話（健檢卡的列與開跑擋下卡共用同一句）
function connIssueText(i) {
  const code = i.code ?? i.rule;
  if (code === 'connector-unavailable') {
    const def = subjectDef();
    const node = def?.nodes.find((x) => x.id === i.node);
    const no = def ? stepSeqMap(def).get(i.node) : null;
    const x = i.label ?? connLabel(i.connector);
    const why = i.status === 'needs-auth' ? '還沒授權' : i.status === 'failed' ? '現在連不上' : i.status === 'unchecked' ? '還沒檢查過連上了沒' : '不在你的服務清單上';
    const where = no ? `第 ${no} 步「${node?.title ?? ''}」` : node ? `「${node.title}」` : '有一步';
    const detail = i.status === 'unchecked' ? `按「重新檢查」看你的 Claude 連了哪些服務，${x} 連上就能開跑。` : `到 claude.ai 的連接器頁授權 ${x}，回來按「重新檢查」就能開跑。`;
    return { title: `${where}要用 ${x}，但 ${x} ${why}。`, detail };
  }
  if (code === 'connector-permission') {
    const list = i.connectors ?? i.connector; // 後端給 connectors（全名陣列）；契約草稿寫 connector，兩個都吃
    const names = (Array.isArray(list) ? list : [list]).filter(Boolean).map(connLabel).join('、');
    return { title: `這條是匯入的，要用 ${names}——要打開「允許這條 Workflow 讀你的服務」才能跑`, detail: '' };
  }
  return null;
}
// 開跑擋下卡（startCheckHtml 原文釘住不動）吃的是換好句子的健檢條：連線兩條換成人話、修這裡指到那一步／那個開關
const pfIssuesView = (issues) => issues.map((i) => {
  const t = connIssueText(i);
  if (!t) return i;
  const fix = (i.code ?? i.rule) === 'connector-unavailable' ? { kind: 'node', id: i.node } : (i.fix ?? { kind: 'flow', id: 'permissions.connectors' });
  return { ...i, ...t, fix };
});
// 健檢一列（健檢卡）：連線兩條有自己的句子與按鈕，其餘照舊「標題＋說明＋修這裡」
function pfRowHtml(i) {
  const lv = i.level === 'block' ? 'block' : 'warn';
  const ic = `<span class="pfic"><i class="ph-fill ${lv === 'block' ? 'ph-x-circle' : 'ph-warning'}"></i></span>`;
  const code = i.code ?? i.rule;
  const t = connIssueText(i);
  if (code === 'connector-unavailable') {
    return `<div class="pfrow ${lv} pfconn" data-pf="connector-unavailable">${ic}
      <div class="pftext"><span class="pft">${esc(t.title)}</span>
        <div class="pfbtns">${connGoLink(true, i.link || CONN_LINK)}${connCheckBtn()}<button class="btn sm2" data-act="pf-fix" data-kind="node" data-id="${esc(i.node)}">改這一步</button></div></div>
    </div>`;
  }
  if (code === 'connector-permission') {
    const fix = i.fix ?? { kind: 'flow', id: 'permissions.connectors' };
    return `<div class="pfrow ${lv} pfconn" data-pf="connector-permission">${ic}
      <div class="pftext"><span class="pft">${esc(t.title)}</span></div>
      <button class="btn sm2" data-act="pf-fix" data-kind="${esc(fix.kind)}" data-id="${esc(fix.id)}">修這裡</button>
    </div>`;
  }
  return `<div class="pfrow ${lv}">
      ${ic}
...
// 讀服務權限列（permissions.connectors）：同產檔那列的做法——已存流程才有、開關即 PUT；說明收進問號，下面只列這條要用哪幾家
function connRowHtml() {
  if (subjectIsDraft() || !state.wf) return '';
  const def = state.wf.def;
  const on = def.permissions?.connectors !== false;
  const flash = state.permFlashUntil > Date.now() && state.permFlashId === 'permissions.connectors';
  const used = [...new Set((def.nodes ?? []).flatMap((n) => n.connectors ?? []))].map(connLabel);
  return `<div class="permrow ${on ? 'on' : ''} ${flash ? 'flash' : ''}" id="perm-connectors">
    <label class="switch" title="${on ? '點一下關掉' : '點一下打開'}"><input type="checkbox" data-act="perm-conn-toggle"${on ? ' checked' : ''} aria-label="允許這條 Workflow 讀你的服務"><span class="knob"></span></label>
    <span class="permtxt"><b>允許這條 Workflow 讀你的服務${hint('開著＝勾了服務的步驟可以讀你在 Claude 上連好的服務，只准讀，不會寄出或修改。匯入別人的 Workflow 預設關，要你自己打開。')}</b>
      ${used.length ? `<span class="permnote">要用：${esc(used.join('、'))}</span>` : ''}
      ${state.permConnErr ? `<span class="permerr"><i class="ph-fill ph-warning"></i>${esc(state.permConnErr)}</span>` : ''}</span>
  </div>`;
}
// 匯入預覽：這條想用哪幾家（依步驟勾的服務去重）；匯入後讀服務權限一律關
function importConnHtml(def) {
  const used = [...new Set((def.nodes ?? []).flatMap((n) => n.connectors ?? []))].map(connLabel);
  if (!used.length) return '<p class="note importconn" data-import-conn>這條不用你的外部服務。</p>';
  return `<div class="alert importconn" data-import-conn><b><i class="ph ph-plugs-connected"></i> 這條想讀你的：${esc(used.join('、'))}</b>
      <div class="why">匯入後預設關：「允許這條 Workflow 讀你的服務」要你自己到 Workflow 設定打開才會讀。沒打開就按開始，健檢會擋下並指到開關。</div></div>`;
}
...
function checkRowHtml() {
  ...
  return `<div class="permrow checkrow ${on ? 'on' : ''}" id="check-delivery">
    ...
    <span class="permtxt"><b>每步交貨先查</b>
      <span class="permnote">每個 AI 步驟做完先對照原始資料與你的要求；攔到會自動重做一次再往下走，步驟卡會標出來。關掉就不查、也不多花 token。</span></span>
    ${subSw}
  </div>
...
function flowSettingsModalHtml() {
  ...
  return `<div class="pvback memmodal flowsettings" data-act="flow-settings-close"><div class="modal" role="dialog" aria-label="Workflow 設定">
    <div class="cvdhead" style="margin-bottom:var(--s2)"><span class="overline">Workflow 設定・${esc(state.wf.def.name)}${hint('僅套用於本 Workflow，即時生效')}</span>
    ...
    ${permRowHtml()}${connRowHtml()}${checkRowHtml()}
  </div></div>`;
}
...
function dataDetailHtml(def, open) {
  ...
  if (open === 'data') {
    ...
    ${habitsNoteHtml()}
    <label class="label" for="run-note">本次補充${hint('本趟所有 AI 步驟皆可見')}</label><textarea id="run-note" class="runnote" data-keep maxlength="2000" placeholder="例如：這次特別留意新品類的表現（選填）">${esc(state.runNote ?? '')}</textarea>
    ${runAttachBoxHtml()}`,
  ...
  if (open === 'execution') return box('確認執行選項', '這些設定屬於整條 Workflow，不是本次覆寫；點了就存。', `${permRowHtml()}${connRowHtml()}${checkRowHtml()}`);
  return '';
}
...
// 檔案大小的人話（本次附件列表用；欄位上傳框自己有一份逐字釘住的，不共用）
const fileSizeText = (n) => `${Math.max(1, Math.ceil((n ?? 0) / 1024))} KB`;
async function uploadRunFile(key, file) {
  const wf = state.wf;
  // 本次附件可以好幾個：累加成陣列；欄位型的上傳照舊一格一檔，直接覆蓋
  const multi = key === RUN_ATTACH_KEY;
  const put = (v, at) => {
    if (!multi) { state.runUploads[key] = v; return 0; }
    const list = Array.isArray(state.runUploads[key]) ? state.runUploads[key] : (state.runUploads[key] ? [state.runUploads[key]] : []);
    if (at === null) { list.push(v); state.runUploads[key] = list; return list.length - 1; }
    list[at] = v;
    state.runUploads[key] = list;
    return at;
  };
  const slot = put({ busy: true, name: file.name }, null);
  render();
  try {
    ...
    put({ token: up.token, name: up.name, size: up.size }, slot);
  } catch (err) {
    put({ err: err.message }, slot);
  }
  render();
  recheckHealth();
}
...
// 本次附件可以好幾個——欄位型的上傳仍是一格一檔，所以不共用 uploadBoxHtml。
// state.runUploads[RUN_ATTACH_KEY] 是陣列：{token,name,size}｜{busy,name}｜{err}
const runAttachList = () => {
  const v = state.runUploads?.[RUN_ATTACH_KEY];
  return Array.isArray(v) ? v : (v ? [v] : []);
};
const runAttachBoxHtml = () => {
  const rows = runAttachList().map((f, i) => {
    if (f.busy) return `<div class="atrow busy"><i class="ph ph-circle-notch"></i><span>${esc(f.name ?? '')} 上傳中⋯</span></div>`;
    if (f.err) return `<div class="atrow bad"><i class="ph ph-warning"></i><span>${esc(f.err)}</span><button type="button" class="btn btn-mini" data-act="run-attach-del" data-i="${i}">移除</button></div>`;
    return `<div class="atrow"><i class="ph ph-paperclip"></i><span class="atname">${esc(f.name)}</span><span class="atsize">${fileSizeText(f.size)}</span>
      <button type="button" class="btn btn-mini" data-act="run-attach-del" data-i="${i}">移除</button></div>`;
  }).join('');
  return `<label class="label">本次附件${hint('這一趟每個 AI 步驟都看得到這些檔；只給這一趟用，下次不留。可以加好幾個。')}</label>
    <div class="attachbox" data-attachbox>${rows}
      <button type="button" class="btn btn-mini atadd" data-act="run-attach-add"><i class="ph ph-plus"></i>${runAttachList().length ? '再加一個檔' : '選檔'}</button>
    </div>`;
};
// 開始時多帶的兩欄：上傳好的 token、本次補充；都沒有＝不帶（body 與現況逐字相同）
function runStartExtras() {
  // 本次附件是陣列（可以好幾個），欄位型的是單一物件——各自取 token，空的不送
  const uploads = Object.fromEntries(Object.entries(state.runUploads ?? {}).map(([k, u]) => {
    if (Array.isArray(u)) return [k, u.filter((x) => x?.token).map((x) => x.token)];
    return [k, u?.token];
...
function flowMemLineHtml() {
  ...
  return `<div class="flowmemline" data-flowmemline><i class="ph ph-identification-card"></i><span>每步都帶 關於你 ${core}・分類守則 ${rules}　開跑選項 ${k} 張${m.paused ? '　<span class="chip wait">整層暫停中</span>' : ''}</span>
    <span class="btn sm2 btn-ghost" data-act="flowmem-open" tabindex="0" role="button"><i class="ph ph-arrow-up-right"></i>查看</span></div>`;
}
...
async function refreshFolder() {
  ...
  const [runs, company, dept, stats] = await Promise.all([
    settle(api('GET', `${wfPath(wf)}/runs?detail=1`)),
    settle(api('GET', sharedPath('_company'))),
    settle(api('GET', sharedPath(wf.category))),
    settle(api('GET', `${wfPath(wf)}/runs/stats`)), // US-099 履歷→執行紀錄頂端的成效統計，跟歷次執行一起抓
  ]);
  if (state.wf?.category !== wf.category || state.wf?.id !== wf.id) return; // 讀到一半已換流程
  state.wfRuns = runs.err ? null : runs.value;
  state.wfStats = stats.err ? null : stats.value;
  state.shared = { company: company.err ? null : company.value, dept: dept.err ? null : dept.value, err: (company.err ?? dept.err)?.message ?? null };
}
// 跨流程最近幾趟：只剩落地頁的「卡住的那趟／最近跑完的」要用（要名字、時間、成品數）。走現成的儀表板那一支；
// 樹上狀態點 US-107 起改讀清單每筆的 last_run，不靠這一份。讀不到＝落地頁少幾列，不擋畫面。
async function refreshRecent() {
  try {
    const d = await api('GET', '/api/dashboard?limit=50&days=1');
    state.recent = Array.isArray(d?.recent) ? d.recent : [];
  } catch { /* 讀不到就留著上一份：只影響落地頁的「卡住的／最近跑完的」兩列，樹上狀態點不靠它（US-107） */ }
}
// 目前流程最近一趟的完整紀錄：上次執行摘要與步驟卡標籤要「逐步的結果」，摘要清單（runs?detail=1）沒有這層
async function refreshLastRun() {
  state.lastRun = null;
  const wf = state.wf;
  if (!wf || subjectIsDraft()) return;
  const row = (state.wfRuns ?? []).find((r) => r.status !== 'unreadable');
  if (!row) return;
  try {
    const full = await api('GET', `${wfPath(wf)}/runs/${encodeURIComponent(row.run_id)}`);
    if (state.wf?.category === wf.category && state.wf?.id === wf.id) state.lastRun = { ...full, workflow: { category: wf.category, id: wf.id } };
  } catch { /* 讀不到＝摘要與標籤都不出現（不是印 0），Workflow 頁照開 */ }
}
...
function flowAsideHtml() {
  ...
  const refsBody = `${fileRows || '<p class="note">還沒有參考檔。</p>'}
      ${files.length > FOLDER_MAX && !fo.refsAll ? `<span class="pbtn" data-act="refs-all" tabindex="0" role="button"><i class="ph ph-caret-down"></i>查看全部 ${files.length} 份</span>` : ''}
      <span class="pbtn" data-act="ref-upload" tabindex="0" role="button"><i class="ph ph-upload-simple"></i>上傳參考檔</span>`;
  ...
  const runsBody = runs === null ? '<p class="note">讀不到歷次執行。</p>'
    : !runs.length ? '<p class="note">還沒開跑過。</p>'
      ...
      ${runs.length > FOLDER_MAX && !fo.runsAll ? `<span class="pbtn" data-act="runs-all" tabindex="0" role="button"><i class="ph ph-caret-down"></i>查看全部 ${runs.length} 趟</span>` : ''}`;
  ...
  const sharedLine = cn === null || dn === null ? (sh.err ? '共用檔讀不到。' : '讀取中⋯')
    : cn + dn === 0 ? '上層還沒有共用檔'
      : `組織 ${cn} 份・分類 ${dn} 份共用檔 <span data-act="shared-open" tabindex="0" role="button"><i class="ph ph-arrow-up-right"></i>查看</span>`;
  return `<aside class="flowaside" aria-label="Workflow 資料夾"><h5>Workflow 資料夾</h5>
    <div class="folderparents"><span class="btn sm2 btn-ghost" data-act="open-company" title="組織頁：全組織共用的規範與參考" tabindex="0" role="button"><i class="ph ph-buildings"></i>${esc(companyName())}</span><i class="ph ph-caret-right"></i><span class="btn sm2 btn-ghost" data-act="open-category" data-cat="${esc(wf.category)}" title="分類頁：這個分類共用的守則、規範與參考" tabindex="0" role="button"><i class="ph ph-folder"></i>${esc(catLabel(wf.category))}</span></div>
...
function inspectorStepHtml(def) {
  ...
  return `${head}<div class="inspchips">${execChipHtml(n)}${n.stop_point === 'always' ? '<span class="chip wait"><i class="ph-fill ph-hand-palm"></i>停點</span>' : ''}</div>
    ...
    ${ai ? `${lb('這步要用')}${connPickerHtml(n)}` : ''}
    ${lb('交付內容')}${spec}
    ${lb('驗收重點')}<p>${n.review_focus ? esc(n.review_focus) : '<span class="note">沒設</span>'}</p>
    ${edit}`;
}
// 共用檔浮窗（demo showParentResources）：兩層各列 檔名・規範／參考 chip・字數，唯讀；上傳與刪除去公司頁／分類頁。掛 .layout 外（地雷 5）
function sharedModalHtml() {
  ...
  return `<div class="pvback memmodal sharedmodal" data-act="shared-close"><div class="modal" role="dialog" aria-label="上層共用檔">
    <div class="cvdhead" style="margin-bottom:var(--s2)"><span class="overline">上層共用檔・${esc(wf.def.name)}${hint('於任務開始時將設定檔注入context')}</span>
      <button class="btn btn-ghost iconb" data-act="shared-close" style="margin-left:auto" title="關閉（Esc）"><i class="ph ph-x"></i></button></div>
    ${layer('company', companyName(), sh.company, `<span class="btn sm2 btn-ghost" data-act="open-company" tabindex="0" role="button"><i class="ph ph-arrow-up-right"></i>去組織頁管理</span>`)}
    ${layer('dept', catLabel(wf.category), sh.dept, `<span class="btn sm2 btn-ghost" data-act="open-category" data-cat="${esc(wf.category)}" tabindex="0" role="button"><i class="ph ph-arrow-up-right"></i>去分類頁管理</span>`)}
  </div></div>`;
}
...
function chatAsideHtml() {
  ...
  return `<aside class="right chatside"><div class="panel"><h3>這次拆解會參考</h3>${rows}</div>
    <div class="panel"><h3>你保有最後決定${hint('預填答案皆有依據，可逐格修改')}</h3></div></aside>`;
}

// 成品長相卡（樣稿 uxProductCard）：拆解器第一趟回的七格＋資料從哪來＋分類＋兩鈕。
...
function shapeMakeHtml(c, dis) {
  ...
  // 三列的名字要跟數字對得上：r.dept 是「分類層上傳的規範檔」份數，不是分類頁打字寫的那些規矩。
  // 標成「分類守則」會讓人以為守則有幾份——右欄一直叫它「分類規範」，這裡跟它一致。
  // 份數＋字數：草稿畫的是字數。份數是 0 或讀不到時就不加字數那段，免得印出「0 份・0 字」
  const amt = (cnt, ch, unit) => {
    const head = n(cnt, unit);
    return cnt && ch ? `${head}・${fmtInt(ch)} 字` : head;
  };
  const rules = !r ? '<p class="note">讀取中⋯</p>' : [
    ['組織規範', amt(r.company, r.companyChars, '份')], ['分類規範', amt(r.dept, r.deptChars, '份')],
  ...
  return `<div class="shapemake">
    ...
    <label class="label">會照這些規範${hint('組織規範、分類守則、關於你，逐條注入每步工作單')}</label>
    <div class="shaperules">${rules}</div>
    <label class="label">照著像的舊作品<span class="opt">選填</span>${hint('模仿結構、章節順序與語氣，數字仍取本次資料')}</label>
    ${sample}
  </div>`;
}
...
// 拆解器第一趟會帶什麼（同 server /api/compose 組 ctx 的四樣：關於你 selectCore、公司規範、分類規範、能耐表）。
// 已存流程（重拆）：關於你／組織／分類直接讀開流程時抓好的 state.wfMemory／state.shared；草稿沒有這兩包→自己打
...
async function refreshShapeRefs() {
  ...
  // 份數之外再帶字數：份數講「有幾個檔」，字數講「AI 這一步要讀多少」，後者才是使用者關心的量。
  // chars 讀不到的檔（舊資料）當 0，不讓一個壞檔把整段變成 null。
  const chars = (d) => (d ? (d.rules ?? []).reduce((s, f) => s + (Number(f.chars) || 0), 0) : null);
  c.refs = {
    core,
    company: company.value ? (company.value.rules?.length ?? 0) : null,
    companyChars: chars(company.value),
    dept: dept.value ? (dept.value.rules?.length ?? 0) : null,
    deptChars: chars(dept.value),
...
// ---------- 上次執行摘要／步驟卡標籤／最近三趟（US-106）----------
// 三塊都只讀「最近一趟」，不做彙總統計（成效統計留在履歷頁 US-099）。
// state.lastRun＝最近一趟的完整紀錄（逐步的結果只有它有，摘要清單沒有）；讀不到＝那一塊整個不出現，不印 0 硬湊。
function lastRunEdits(run) {
  return Object.values(run?.steps ?? {}).filter((x) => x?.edited_output != null).length;
}
function lastRunToneHtml(run) {
  if (run.status === 'done') return '<span class="chip quiet"><i class="ph-fill ph-check-circle"></i>上次全部完成</span>';
  if (Object.values(run.steps ?? {}).some((x) => x?.status === 'failed')) return '<span class="chip bad"><i class="ph-fill ph-warning"></i>上次有步驟出錯</span>';
  return '<span class="chip wait"><i class="ph-fill ph-hand-palm"></i>上次停在半路</span>';
}
function lastRunLineHtml() {
  const run = state.lastRun;
  if (!run || !state.wf || subjectIsDraft()) return '';
  const ms = run.finished_at && run.started_at ? new Date(run.finished_at) - new Date(run.started_at) : 0;
  const arts = runArtifactNodes(run).length;
  const txt = [fmtWhen(run.started_at), ms > 0 ? ovDuration(ms) : '', `${arts} 份成品`, `你改了 ${lastRunEdits(run)} 處`].filter(Boolean).join('・');
  const see = arts
    ? `<button class="btn sm2" data-act="open-run-reader" data-src="lastrun" data-cat="${esc(state.wf.category)}" data-id="${esc(state.wf.id)}" data-rid="${esc(run.run_id)}">看上次成品</button>` : '';
  return `<div class="lastrun" data-lastrun>${lastRunToneHtml(run)}<span class="lrtxt">${esc(txt)}</span>${see}</div>`;
}
// 每張步驟卡右側一個標籤＝上次這一步的結果。查核攔過就先講攔了幾次（那是要回去調的訊號），
// 再講你改過，都沒有才是通過；有跑但沒跑完的那步講「上次停在這裡」——不能謊報「沒跑過」或「上次通過」
function stepLastTagHtml(n) {
  const run = state.lastRun;
  const st = run?.steps?.[n.id];
  // US-113 只印例外：沒跑過＝預設值，不印（參考物 A 案）
  if (!run || !st || !st.status || st.status === 'pending') return '';
  // US-111 redone＝攔過一次、自動重寫後放行（attempts 裡那筆 blocked 是它的前半段，不另數）；US-112 三種說法跟步驟卡的查核膠囊同一套：
  // 重寫後全對上＝灰色一句、仍有數字沒出處＝黃色提醒（不是攔）、舊趟缺欄位＝「上次重寫一次」
  if (st.check?.status === 'redone') {
    const [tone, txt] = redoneLabel(st.check, '上次');
    return `<span class="chip ${tone}"><i class="ph-fill ph-arrows-clockwise"></i>${txt}</span>`;
  }
  const blocked = (st.attempts ?? []).filter((a) => a?.check?.status === 'blocked').length;
  if (blocked) return `<span class="chip wait"><i class="ph-fill ph-hand-palm"></i>上次被查核攔 ${blocked} 次</span>`;
  if (st.edited_output != null) return '<span class="chip quiet"><i class="ph ph-pencil-simple"></i>你改過</span>';
  if (st.status === 'done') return '<span class="chip ok"><i class="ph-fill ph-check-circle"></i>上次通過</span>'; // US-113：綠 chip ok 只給這一句
  return '<span class="chip wait"><i class="ph-fill ph-hand-palm"></i>上次停在這裡</span>';
}
// 最近三趟：每趟可直接開成品——走閱讀輪那個右側滑出面板（open-run-reader），不另做一套
function recentRunsHtml() {
  if (!state.wf || subjectIsDraft()) return '';
  const runs = (state.wfRuns ?? []).filter((r) => r.status !== 'unreadable').slice(0, 3);
  if (!runs.length) return '';
  const rows = runs.map((r) => {
    const ms = r.finished_at && r.started_at ? new Date(r.finished_at) - new Date(r.started_at) : 0;
    const meta = [ms > 0 ? ovDuration(ms) : '', `${(r.finals ?? []).length} 份成品`].filter(Boolean).join('・');
    const act = (r.finals ?? []).length
      ? `<button class="btn sm2" data-act="open-run-reader" data-src="recent" data-cat="${esc(state.wf.category)}" data-id="${esc(state.wf.id)}" data-rid="${esc(r.run_id)}">看成品</button>`
      : `<button class="btn sm2" data-act="todo-go" data-cat="${esc(state.wf.category)}" data-id="${esc(state.wf.id)}" data-rid="${esc(r.run_id)}">打開</button>`;
    return `<div class="runrow" data-run="${esc(r.run_id)}">${runStatusChip(r)}<span class="rtime">${esc(fmtWhen(r.started_at))}</span><span class="meta">${esc(meta)}</span>${act}</div>`;
  }).join('');
  return `<div class="recentruns" data-recent-runs><h5>最近三趟</h5>${rows}</div>`;
}

// US-113 只印例外：停點從單一手掌圖標改成一句「做完等你核可」（chip wait，參考物 A 案）；人做步驟照舊
function stopMark(n) {
  ...
  return n.stop_point === 'always'
    ? '<span class="chip wait"><i class="ph-fill ph-hand-palm"></i>做完等你核可</span>'
    : '';
}

// 執行者膠囊：onlyHuman＝清單卡（US-113 只印例外：AI 是預設值不印；人做也不印「你來」——使用者 09-24「兩個收成一個，留人形圓標」，序號旁的 mkic.you 就是記號）；右欄檢視器不帶參數，照舊印「AI」／「你來」
function execChipHtml(n, onlyHuman = false) {
  if (onlyHuman) return '';
  if (n.executor === 'human') return '<span class="chip"><i class="ph ph-user"></i>你來</span>';
  return '<span class="chip">AI</span>';
}
...
function stepListHtml(def) {
  ...
  // US-113 只印例外：「編輯」是次要操作，帶 .dim 平常淡出、卡片滑過／聚焦才亮（style.css article.step .dim）
  const btns = (n) => `<button class="btn btn-ghost sm2 dim" data-act="edit-step" data-node="${esc(n.id)}"><i class="ph ph-pencil-simple"></i>編輯</button>`;
  ...
  return topoNodes(def).map((n) => {
    ...
    return `<article class="step${cls}${sel(n)}"${pick(n)}><div class="stephead"><span class="num">${seqTxt(seq.get(n.id))}</span><h3>${esc(n.title)}</h3>
      ${stepConnTagHtml(n)}${execChipHtml(n, true)}${stopMark(n)}${stepLastTagHtml(n)}${btns(n)}</div>
...
function paramRowsHtml(def) {
  ...
  const row = (p) => `
    <div class="param"><span>${esc(p.label)}${p.required ? '<span class="chip wait req" title="開跑前健檢：這欄沒填不給跑">必填</span>' : ''}<span class="chip unusedchip" data-unused="${esc(p.key)}" ${unused.includes(p.key) ? '' : 'hidden'} title="這個欄位沒有任何步驟的指示引用到——填了也沒人看">沒有步驟用到</span></span>
      <div class="val"><textarea class="autogrow" rows="1" id="param-now-${esc(p.key)}" data-keep data-param="${esc(p.key)}" placeholder="${esc(p.hint ?? '')}">${esc(state.paramNow[p.key] ?? p.default)}</textarea>${habitChipsHtml(p)}</div></div>`;
...
function listModeHtml() {
  ...
  if (!def) {
    return `<div class="empty-c"><div class="ic"><i class="ph ph-list-checks"></i></div>
      <h3>還沒選 Workflow</h3><p>左邊那棵樹挑一條打開，或切到「聊天」講一件你的事拆一個新的。</p>
  ...
  const addParam = state.addingParam
    ...
    : `<div class="param expander" style="justify-content:flex-start;color:var(--ink-400);font-size:12px" data-act="add-param" tabindex="0" role="button"><i class="ph ph-plus"></i> 新增欄位</div>`;
  if (subjectIsDraft()) {
  ...
  return `<div class="steplist">
    ...
    ${stepListHtml(def)}
    ${flowMemLineHtml()}
    <details class="panel fieldsfold" data-fields${open ? ' open' : ''}><summary data-act="folder-toggle" data-k="fields"><i class="ph ph-caret-down"></i>執行需要的資料<span class="meta">・${def.params.length} 欄</span>${hint('欄位屬本 Workflow，本次的值在「本次資料」')}</summary>${fieldRows}${addParam}
      ${state.wf?.dictSimilar?.length ? `<p class="note"><i class="ph ph-info"></i> 這幾個欄位名跟詞典裡的很像：${esc(state.wf.dictSimilar.join('、'))}——已經照存，沒擋你。</p>` : ''}</details>
    ${recentRunsHtml()}</div>`;
}
...
function presetRowHtml(field, targetId) {
  ...
  return `<div class="presetrow">
    ...
    <span class="pbtn" data-act="preset-save" data-field="${field}" data-target="${targetId}" title="把目前欄位內容存成常用" tabindex="0" role="button"><i class="ph ph-star"></i>存為片段</span>
    <span class="pbtn" data-act="preset-del" data-field="${field}" data-target="${targetId}" title="刪掉上面選中的常用" tabindex="0" role="button"><i class="ph ph-minus-circle"></i></span>
  </div>`;
}
...
// 三段「這條流程／分類／公司」各自勾（demo v8RefSections）。勾選鍵＝`scope:name`（同名兩層不撞，壓測 A3）；
...
function refFilesInner(n, checkedOverride) {
  ...
  // 下載鈕（走 GET .../files/<檔名>）：上傳完只有 AI 讀得到的話，這個檔等於丟進黑洞
  const flowRows = (state.wfFiles ?? []).map((f) => `<label class="refrow">${box('flow', f)}<span class="wfname">${esc(f)}${/\.(docx|xlsx)$/i.test(f) ? '<span class="tplchip" title="Word／Excel 範本：在「產出規格」頁選為範本，工人會照它產檔">範本</span>' : ''}</span>
    <a class="ph ph-download-simple wfdl" href="${state.wf ? `${wfPath(state.wf)}/files/${encodeURIComponent(f)}` : '#'}" download="${esc(f)}" title="下載這個參考檔" aria-label="下載 ${esc(f)}"></a>
  ...
  return `<div class="flabel">參考檔（勾＝這一步會看）</div>
    <div class="reflist">
      ${seg('flow', '這條 Workflow', `${flowRows || '<p class="note">尚無參考</p>'}<span class="pbtn" data-act="ref-upload" tabindex="0" role="button"><i class="ph ph-upload-simple"></i>上傳參考檔</span>`)}
...
function templateInner(n, keepVal) {
  ...
  return `<div class="flabel">範本填空（選填）${hint('{{output}} 代入本步產出，{{參數}} 照常代入')}</div>
...
function refFilesSectionHtml(n) {
  if (subjectIsDraft()) {
    return `<div class="flabel">參考檔${hint('給Ai參考與模仿的合格成品')}</div>`;
  }
  return `<div id="ref-section">${refFilesInner(n)}</div>`;
}
function templateSectionHtml(n) {
  if (subjectIsDraft()) {
    return `<div class="flabel">範本填空${hint('需先存入 Workflow 庫')}</div>`;
  }
...
function supChecksHtml(n) {
  ...
  return `<div class="flabel">監工可以：${hint('勾選才啟用；備註為附加，不覆寫指示')}</div>
...
function canvasEditorHtml(n, def) {
  ...
  if (kind === 'task') {
    ...
    const head = `${isAI ? drawerInputsHtml(n, def) + drawerMemoryHtml(n) : ''}${titleRow}
      ...
      <div class="cvgrid2">
        <div class="cvcol">${textFieldHtml('cv-instruction', `任務${hint('這裡是後台原文，{{欄位}} 代碼在前台會自動變成欄位值。')}`, n.instruction, '這一步要做什麼', null)}</div>
      ...
      </div>${isAI ? `<span class="label">這步要用${hint('只准讀：寄出、修改、刪除一律擋。沒勾的步驟摸不到任何外部服務。')}</span>${connPickerHtml(n, true)}` : ''}`;
    // 那句「後台原文」講的是任務欄的內容，收進任務旁的說明鈕——原本印在彈窗底部，離它講的東西很遠
    const tfoot = foot('<button class="btn btn-ghost btn-danger" data-act="cv-delete">刪除這步</button>');
...
// ---------- 履歷模式 ----------
// US-099 成效統計卡（PROTO/成效統計輪-履歷頁-三案草稿.html 定案：A 的四格＋B 的「最常出事的步驟」排名）
// 一趟通常多久：「40 秒」「6 分」「1 小時 20 分」「2 小時」
function fmtDur(ms) {
...
function runStatsHtml() {
  const st = state.wfStats;
  if (st === undefined) return '';
  if (st === null || !st.window) return '<p class="note runstatsbad">統計讀不到。</p>';
  const w = st.window;
  const days = st.days ?? 30;
  const dash = '—';
  const kpi = (val, label, extra = '') => `<div class="kpi"><b>${esc(val)}</b><span>${esc(label)}</span>${extra}</div>`;
  let cmp = '';
  if (w.rate != null && st.prev_rate != null) {
    const d = Math.round((w.rate - st.prev_rate) * 100);
    cmp = d > 0 ? `<div class="d up">比前 ${days} 天 +${d}%</div>` : d < 0 ? `<div class="d dn">比前 ${days} 天 -${-d}%</div>` : `<div class="d">比前 ${days} 天 持平</div>`;
  }
  const seq = stepSeqMap(state.wf.def);
  const top = st.top_steps ?? [];
  const rank = top.length
    ? top.map((t, i) => {
      const n = seq.get(t.node);
      const label = n ? `第 ${n} 步 ${t.title}` : t.title;
      return `<div class="rank"><span class="n">${i + 1}</span><span class="l">${esc(label)}</span>${t.failed ? `<span class="chip bad">失敗 ${t.failed} 次</span>` : ''}${t.edited ? `<span class="chip wait">被你改 ${t.edited} 次</span>` : ''}</div>`;
    }).join('')
  ...
  if (tab === 'runs') {
    ...
    const body = runs === null ? '<p class="note">讀不到歷次執行。</p>'
      : !runs.length ? '<p class="note">這條 Workflow 還沒開跑過。</p><button class="btn btn-primary" data-act="flow-tab" data-tab="data">準備執行</button>'
        : runStatsHtml() + runs.map((r) => folderRunHtml(state.wf, r)).join('');
...
function importPreviewHtml() {
  ...
  const steps = def.nodes.map((n) => {
    ...
    return `<details class="artifact" ${hits.length ? 'open' : ''}><summary><i class="ph ph-file-text"></i>${esc(n.title)} <span class="chip">${(n.kind ?? 'task') !== 'task' ? esc(n.kind) : n.executor === 'human' ? '你來' : 'AI'}</span>${(n.connectors ?? []).map((x) => ` <span class="chip blue">${esc(connLabel(x))}</span>`).join('')} ${flag}</summary>
...
function trashViewHtml() {
  ...
  return `${pageHeadHtml('垃圾桶', '刪掉的 Workflow 放 30 天，期限內都能整個復原（含履歷和跑過的紀錄）。', '<button class="btn" data-act="close-trash"><i class="ph ph-caret-left"></i>回 Workflow</button>')}
...
const CHECK_CHIP = {
  pass: ['quiet', 'ph-fill ph-shield-check', '已覆核'], // US-112：一次過關＝已覆核（原「查過」）
  'redo-pass': ['quiet', 'ph-fill ph-arrows-clockwise', '重做過一次'],
  redone: ['quiet', 'ph-fill ph-arrows-clockwise', '重寫一次'], // US-111 攔過一次、自動重寫後放行（runner 產生）；US-112 字與色由 redoneLabel 依 recheck_blocks 定，這列是舊趟缺欄位的退路。舊值一個不拿掉
  blocked: ['wait', 'ph-fill ph-hand-palm', '查核攔下'],
  incomplete: ['wait', 'ph-fill ph-warning', '未完成查核'],
  accepted: ['quiet', 'ph ph-check', '你說放行'],
};
// US-112 三種說法（redone 專用）：recheck_blocks＝重寫那份由程式再對一次後仍找不到出處的數字。
// 空陣列＝全對上（灰）；有 N 條＝黃色提醒（沿用 wait 樣式，不是攔）；欄位缺席或不是陣列（舊趟）＝「重寫一次」不加後綴
function redoneLabel(check, prefix = '') {
  const rb = check?.recheck_blocks;
  if (!Array.isArray(rb)) return ['quiet', `${prefix}重寫一次`];
  if (!rb.length) return ['quiet', `${prefix}重寫一次、已覆核`];
  return ['wait', `${prefix}重寫一次、仍有 ${rb.length} 個數字沒出處`];
}
function checkChip(step, usage) {
  const hit = CHECK_CHIP[step?.check?.status];
  if (!hit) return '';
  let [tone, icon, txt] = hit;
  if (step.check.status === 'redone') [tone, txt] = redoneLabel(step.check);
  const n = (usage?.input ?? 0) + (usage?.output ?? 0);
  return `<span class="chip ${tone}"><i class="${icon}"></i>${txt}${n ? ` · 查核 ${fmtInt(n)} token` : ''}</span>`;
}

// 標黃行：查核沒攔下、但值得你看一眼的三種情形（number-unsourced＝流程沒開程式權限，數字沒法由程式對，只標不攔）
const FLAG_TXT = { 'conclusion-changed': '這一步把結論或排序改了', format: '格式跟要求不同', 'number-unsourced': '程式沒對過的數字' };
...
function patchRendered(h) {
  ...
  if (state.reader) readerTagNums(); // 排版是後到的：閱讀面板裡剛換上的表格也要補 .num
}
// 「看原文／看排版」切換鈕；切換只改那一格（整頁重繪會把成品區展開的收起來）
function mdToggleHtml(key) {
  if (mdDown) return '';
  const raw = state.rawView.has(key);
  return `<span class="pbtn mdtoggle" data-act="md-toggle" data-key="${esc(key)}" tabindex="0" role="button"><i class="ph ${raw ? 'ph-text-aa' : 'ph-code'}"></i>${raw ? '看排版' : '看原文'}</span>`;
}
function applyMdView(key) {
  ...
  if (state.reader) readerTagNums(); // 面板裡切回「看排版」：新換上的表格補 .num
}
...
function fileChipHtml(cat, id, rid, name, note) {
  ...
  return `<span class="chip expander filechip" data-act="preview-file" data-cat="${esc(cat)}" data-id="${esc(id)}" data-rid="${esc(rid)}" data-fname="${esc(name)}" title="${esc(note ?? '點開看內容，浮窗裡可下載')}" tabindex="0" role="button"><i class="ph ${FILE_ICON[ext] ?? 'ph-file-text'}"></i>${esc(name)}</span>`;
}
...
// 共用檔查看共用同一個浮窗——pv.shared＝層（_company｜分類名）；GET …/files/:name/view（md／txt 原文、docx 排版）
...
function previewHtml() {
  ...
  if (pv.err) body = `<div class="pverr"><i class="ph-fill ph-warning"></i><span>預覽讀不到：${esc(pv.err)}。先用下面的「下載」開原檔。</span></div>`;
    ...
    if (sheets.length > 1) tabs = `<div class="seg pvtabs">${sheets.map((s, j) => `<span class="${j === i ? 'on' : ''}" data-act="preview-sheet" data-i="${j}" tabindex="0" role="button">${esc(s.name)}</span>`).join('')}</div>`;
...
function supervisorNoteHtml(node) {
  ...
  if (briefText || handoffText) {
    ...
    head = `<span class="pbtn" data-act="sup-toggle" data-node="${esc(node.id)}" tabindex="0" role="button">監工交代 ${open ? '▾' : '▸'}</span>${body}`;
  }
...
function branchChoiceCardHtml(node) {
  ...
  const options = node.branches.map((b) => `
    <div class="step expander" data-act="choose-branch" data-node="${esc(node.id)}" data-target="${esc(b.next)}" tabindex="0" role="button">
...
function editRulesHtml(node, step) {
  ...
  return `<div class="rules"><div class="rhead"><b>後面每步會守：</b>
      <span class="pbtn" data-act="edit-rules-open" data-node="${esc(node.id)}" tabindex="0" role="button"><i class="ph ph-pencil-simple"></i>編輯</span></div>
    <ul>${items}</ul></div>`;
}
...
function runHtml() {
  ...
  else if (!histNode && runStuck(run)) main = runStuckHtml();
  else {
...
// 這趟留得下成品的步（清單、閱讀面板的上一份／下一份、總覽副標的份數都用這一份順序）
// 人做步驟有交出內容的也算，清單上標「（你交出的）」
function runArtifactNodes(run) {
  return topoNodes(run.def).filter((n) => kindOf(n) === 'task' && run.steps[n.id]?.status === 'done'
    && (n.executor === 'ai' || run.steps[n.id].output));
}

// AI 在成品開頭宣告的「【已標注資料不全：…】」抽出來，改成文件最上方的黃色提示條，不再混在內文第一行。
// 抽不到就一個字不動（notice 空、body 原樣）。
function splitDataNotice(text) {
  const t = String(text ?? '');
  const m = /^【已標注資料不全：([\s\S]*?)】\n?/.exec(t);
  return m ? { notice: m[1].trim(), body: t.slice(m[0].length) } : { notice: '', body: t };
}

const clipTxt = (s, n) => (s.length > n ? `${s.slice(0, n)}⋯` : s);
// 清單列的一行摘要＝這份成品的前一句完整句子（不是截 200 字）。表格列與分隔線跳過；
// 整份都找不到句號時才退回第一行可讀的字（標題行也算），再找不到就空著。
function firstSentence(text, max = 64) {
  let fallback = '';
  for (const raw of splitDataNotice(text).body.split('\n')) {
    const s = raw.trim();
    if (!s || s.startsWith('|') || /^[-*_=]{3,}$/.test(s)) continue;
    const plain = s.replace(/^#{1,6}\s+/, '').replace(/^>\s*/, '').replace(/^([-*+]|\d+[.)])\s+/, '').replace(/[*`_~]/g, '').trim();
    if (!plain) continue;
    const end = /[。！？!?]/.exec(plain);
    if (end) return clipTxt(plain.slice(0, end.index + 1), max);
    if (!fallback) fallback = plain;
  }
  return fallback ? clipTxt(fallback, max) : '';
}

// 總覽的文字欄位（重點句、圖標題、丟掉的原因）後端存進卷宗前已經跳脫過（ADR-007 第 1 條）。
// 這裡先還原那五個實體再 esc 一次：再跳脫一次會變成畫面上看到 &lt;，少跳脫又等於信 AI 的字面——
// 還原＋重跳脫兩邊都不吃虧，契約萬一破了也仍然是純文字。
const OV_ENTITY = { amp: '&', lt: '<', gt: '>', quot: '"', '#39': "'" };
const ovText = (s) => esc(String(s ?? '').replace(/&(amp|lt|gt|quot|#39);/g, (m, e) => OV_ENTITY[e]));
// 圖上的數字只是把成品裡寫過的數字排到畫面上（ADR-007 第 3 條：總覽不自己算新數字），整數補千分位
const ovNum = (v) => (typeof v === 'number' && Number.isFinite(v) ? (Number.isInteger(v) ? fmtInt(v) : String(v)) : '');
// 單位（chart.unit，選填）：後端限 6 字且已跳脫——還原、截、再跳脫一次，切不到半截實體；空的就不顯示
const ovUnit = (u) => { const raw = String(u ?? '').replace(/&(amp|lt|gt|quot|#39);/g, (m, e) => OV_ENTITY[e]).trim().slice(0, 6); return raw ? esc(raw) : ''; };
// 共多久：秒／分秒／時分，頁尾那行用
function ovDuration(ms) {
...
// 一張圖：四種版型（ADR-007 第 2 條）——bar 比大小、line 比時間、kpi 單一重點大字、table 表格。
// 全部用 HTML／CSS 畫（長條照草稿），不引任何圖表套件；baseline 只照原文標出來，不另算差距。
function ovChartHtml(c) {
  const series = (c?.series ?? []).filter((p) => p && typeof p.value === 'number' && Number.isFinite(p.value));
  if (!series.length) return '';
  const base = typeof c.baseline?.value === 'number' && Number.isFinite(c.baseline.value) ? c.baseline.value : null;
  // unit＝選填的單位（%、元、次⋯；後端限 6 字、已跳脫）。先還原再截再跳脫，才不會把 &quot; 切成半截；
  // 沒給就照舊只印數字——覆核抓到的「13.28」本來在原文裡是「13.28%」
  const unit = ovUnit(c?.unit);
  const val = (v) => { const n = ovNum(v); return n ? `${n}${unit}` : ''; };
  const baseLab = `${ovText(c.baseline?.label) || '比較基準'} ${val(base)}`;
  const vals = series.map((p) => p.value);
  let body = '';
  if (c.kind === 'bar') {
    // 負數也要畫得出來（覆核第 1 條）：軌道的範圍是 [min(0,…), max(0,…)]，0 一定在裡面，
    // 長條從 0 那條線往正的方向長、負的往左長。全正時 0 在最左邊，畫出來跟草稿一模一樣。
    const lo = Math.min(...vals, base ?? 0, 0);
    const hi = Math.max(...vals, base ?? 0, 0);
    const span = hi - lo;
    const at = (v) => (span > 0 ? ((v - lo) / span) * 100 : 0); // 值在軌道上的位置（%）；全部一樣大（含全零）＝沒有長短可比，都是 0
    const r1 = (n) => Math.round(n * 10) / 10;
    const zero = r1(at(0));
    const neg = lo < 0; // 有負數才畫 0 的縱線與那行說明
    const small = Math.min(...vals);
    const avg = base === null ? '' : `<span class="avg" style="left:${r1(at(base))}%"></span>`;
    const zl = neg ? `<span class="zeroline" style="left:${zero}%"></span>` : '';
    body = series.map((p) => {
      const a = r1(Math.min(at(p.value), at(0)));
      const w = r1(Math.max(at(p.value), at(0)) - Math.min(at(p.value), at(0)));
      // 全正時沿用草稿的「最小的那條淡色」；有負數時顏色留給正負之分，不再另外標最小
      const cls = p.value < 0 ? ' neg' : !neg && series.length > 1 && p.value === small ? ' low' : '';
      return `<div class="bar"><span class="bl" title="${ovText(p.label)}">${ovText(p.label)}</span><span class="track"><span class="fill${cls}" style="left:${a}%;width:${w}%"></span>${zl}${avg}</span><span class="bv">${val(p.value)}</span></div>`;
    }).join('')
      + (neg ? '<div class="avglab">縱線＝0，往左是負的</div>' : '')
      + (base === null ? '' : `<div class="avglab">虛線＝${baseLab}</div>`);
  } else if (c.kind === 'line') {
    const hi = Math.max(...vals);
    const lo = Math.min(...vals);
    const span = hi - lo || 1;
    const x = (i) => (series.length === 1 ? 50 : (i * 100) / (series.length - 1));
    const y = (v) => (39 - ((v - lo) / span) * 36).toFixed(1);
    const pts = series.map((p, i) => `${x(i).toFixed(1)},${y(p.value)}`).join(' ');
    body = `<svg class="lineplot" viewBox="0 0 100 42" preserveAspectRatio="none" aria-hidden="true"><polyline points="${pts}" vector-effect="non-scaling-stroke"></polyline></svg>
      <div class="linelab">${series.map((p) => `<span><b>${val(p.value)}</b>${ovText(p.label)}</span>`).join('')}</div>`
      + (base === null ? '' : `<div class="avglab">比較基準：${baseLab}</div>`);
  } else if (c.kind === 'kpi') {
    body = series.map((p) => `<div class="kpi"><span class="kv">${val(p.value)}</span><span class="kl">${ovText(p.label)}</span></div>`).join('')
      + (base === null ? '' : `<div class="avglab">比較基準：${baseLab}</div>`);
  } else if (c.kind === 'table') {
    body = `<table class="ovtable"><tr><th>項目</th><th class="n">數字</th></tr>${series.map((p) => `<tr><td>${ovText(p.label)}</td><td class="n">${val(p.value)}</td></tr>`).join('')}</table>`;
  } else return ''; // 認不得的版型不猜（後端 sanitizeOverview 已經擋過一層，這裡是第二道）
  return `<div class="chartbox" data-chart="${esc(c.kind)}"><div class="cap">${ovText(c.title)}</div>${c.note ? `<div class="note">${ovText(c.note)}</div>` : ''}${body}</div>`;
}

// 頁尾一行：幾步全過、查核攔幾次、共多久，末尾「看這趟的紀錄」。
// 算不出來的那幾項就不講（覆核第 3 條）——舊的趟沒有這些數字，`buildStats` 一律回 0，
// 照 0 印出來會變成「沒有算得上的步驟・共 0 秒」，那是硬掰。整行都沒東西可講＝整行不出現。
function ovFootHtml(stats) {
  const st = stats ?? {};
  const num = (v) => (typeof v === 'number' && Number.isFinite(v) ? v : null);
  const total = num(st.steps_total);
  const passed = num(st.steps_passed) ?? 0;
  const blocked = num(st.check_blocked);
  const unresolved = num(st.recheck_unresolved) ?? 0; // US-112：重寫後仍有數字沒出處的步數 M；舊趟沒這欄＝0＝不印那句
  const ms = num(st.duration_ms);
  const parts = [];
  // 一步都沒有＝這趟的步數根本沒算出來，連「查核攔幾次」也無從說起，整句略過
  if (total) {
    const steps = passed >= total ? `${total} 步全過` : `${total} 步裡 ${passed} 步過`;
    const more = unresolved > 0 ? `，其中 ${unresolved} 步重寫後仍有數字沒出處` : '';
    const chk = blocked === null ? '' : blocked ? `，查核攔下 ${blocked} 次已自動重寫${more}` : '，查核一次都沒攔下';
    parts.push(`<span><b>${steps}</b>${chk}</span>`);
  }
  if (ms) parts.push(`<span>共 ${ovDuration(ms)}</span>`); // 0 毫秒＝沒有開跑／收工時刻可算，不是真的跑了 0 秒
  if (!parts.length) return '';
  return `<div class="ovfoot">${parts.join('<span>·</span>')}<span>·</span>
    <button type="button" class="btn sm2" data-act="ov-record"><i class="ph ph-notepad"></i>看這趟的紀錄</button></div>`;
}

// 中欄頂端的總覽（US-104）。沒有總覽（設定關掉、那趟沒寫成、上線前跑的舊趟）＝整塊不出現，
// 中欄直接從成品清單開始，不留空殼。
function overviewHtml(run) {
  const ov = run.overview;
  const summary = ov?.summary ?? [];
  const charts = ov?.charts ?? [];
  if (!ov || ov.fail_note || (!summary.length && !charts.length)) return '';
  const seq = stepSeqMap(run.def);
  const when = fmtWhen(ov.at ?? run.finished_at);
  const sub = [esc(run.def.name), when ? `${when} 跑完` : '', `${runArtifactNodes(run).length} 份成品`].filter(Boolean).join(' · ');
  const pts = summary.map((s) => `<div class="pt"><span class="dot"></span><span>${ovText(s.text)}<button type="button" class="cite" data-act="ov-cite" data-node="${esc(s.cite)}" title="開這一步的閱讀面板">出處 › 第 ${seq.get(s.cite) ?? '?'} 步</button></span></div>`).join('');
  // dropped＝哪幾張圖沒畫出來、為什麼（ADR-007 第 3 條）：一條一行灰字，接在圖區底下
  const drops = (ov.dropped ?? []).length
    ? `<div class="ovdrop">${ov.dropped.map((d) => `<div>${ovText(d)}</div>`).join('')}</div>` : '';
  return `<div class="ovcard"><div class="ovhead">這次得到什麼</div><div class="ovsub">${sub}</div>
    ${pts}${charts.map(ovChartHtml).join('')}${drops}${ovFootHtml(ov.stats)}</div>`;
}

// 成品清單一列：序號＋標題＋一行摘要（不再內嵌半截內文）；點了開閱讀面板
function artifactRowHtml(run, n, i) {
  const s = run.steps[n.id] ?? {};
  const tag = n.executor === 'human' ? '（你交出的）' : s.edited_output != null ? '（你改過的版本）' : '';
  return `<button type="button" class="artrow" data-act="open-reader" data-node="${esc(n.id)}" aria-label="讀「${esc(n.title)}」">
      <span class="n">${i}</span><span class="nm">${esc(n.title)}${tag}</span><span class="sum">${esc(firstSentence(s.edited_output ?? s.output))}</span>${s.file ? '<i class="ph ph-paperclip" title="這一步有產出檔"></i>' : ''}<i class="ph ph-caret-right"></i></button>`;
}

// 跑完（樣稿 uxRunComplete）：一張白卡——總覽→成品清單（標題＋一行摘要，點開右側閱讀面板）→這趟的紀錄（整趟監工總結）→提議→回饋
...
// ---------- 閱讀面板（US-103，草稿 A 案）----------
// 點成品清單任一列＝右側滑出約三分之二寬（最小 560px）的面板，左側蓋半透明暗層；
// Esc 或點暗層關閉，關掉焦點回原來那一列。文件一律先排版再顯示——沿用 mdBlock／POST /api/render，
// 不另寫一套渲染；面板內只有一條捲軸（.rdbody），裡面的產出框不再各自開一條（樣式在 style.css）。
function readerHtml() {
  const rd = state.reader;
  if (!rd) return '';
  const run = rd.run ?? state.run; // 合併輪：「最近三趟」開的是過去那一趟（帶在 rd.run），執行頁開的還是 state.run
  if (!run) return '';
  const list = runArtifactNodes(run);
  const i = list.findIndex((n) => n.id === rd.node);
  if (i < 0) return ''; // 那一步不在清單上了（換了 run、重跑）＝面板自己收掉
  const n = list[i];
  const s = run.steps[n.id] ?? {};
  const key = `art:${n.id}`;
  ...
  return `<div class="rdback" data-act="reader-back"><aside class="rddrawer" role="dialog" aria-modal="true" aria-label="讀「${esc(n.title)}」">
    <div class="rdhead"><span class="step">第 ${stepSeqMap(run.def).get(n.id) ?? 0} 步／共 ${list.length} 份</span><b class="t" title="${esc(n.title)}">${esc(n.title)}</b>
      ${nav(list[i - 1], 'prev', '‹ 上一份')}${nav(list[i + 1], 'next', '下一份 ›')}${mdToggleHtml(key)}${dl}
      <button type="button" class="btn btn-ghost iconb" data-act="reader-close" title="關閉（Esc）" aria-label="關閉"><i class="ph ph-x"></i></button></div>
    <div class="rdbody"><div class="rddoc">${notice ? `<div class="warnbar"><i class="ph-fill ph-warning"></i><span>資料不全：${esc(notice)}</span></div>` : ''}${mdBlock(body, key)}</div></div></aside></div>`;
}
// 面板開著時關掉它：焦點回原來那一列（2026-09-22 無障礙輪的寫法——重繪後照 data-node 找回同一顆）
// 面板是「哪一顆鈕」開的——關掉時焦點要回那一顆。執行頁的成品清單是 open-reader（認 data-node），
// Workflow 頁的「最近三趟」「看上次成品」與空狀態「看成品」是 open-run-reader（認 cat／id／rid），
// 只認 open-reader 會讓後面這三個入口關掉後焦點掉回 body。屬性值只跳脫 " 與 \（CSS.escape 是給識別字用的，
// 放進引號裡的屬性值反而會把數字開頭的值escape 成對不上的字）
const attrEsc = (v) => String(v).replace(/["\\]/g, '\\$&');
function readerSourceSelector(from) {
  const at = (k) => (from?.[k] == null || from[k] === '' ? '' : `[data-${k}="${attrEsc(from[k])}"]`);
  return `[data-act="${attrEsc(from.act)}"]${at('node')}${at('cat')}${at('id')}${at('rid')}${at('src')}`;
}
function closeReader() {
  const from = state.reader?.from ?? null;
  state.reader = null;
  render();
  if (from) document.querySelector(readerSourceSelector(from))?.focus();
}
// 開面板：重繪後焦點進面板（關閉鈕），鍵盤走得進去。
// run 給了＝讀那一趟（Workflow 頁的「最近三趟」「看上次成品」）；沒給＝執行頁目前那趟。
// from＝開它的那顆鈕（沒給就當是成品清單那一列），關掉時照它把焦點放回去
function openReader(node, run = null, from = null) {
  state.reader = { node, run, from: from ?? { act: 'open-reader', node } };
  render();
  document.querySelector('[data-act="reader-close"]')?.focus();
}
// 「看成品」：把那一趟整份讀回來，開第一份成品的閱讀面板（同一個面板，不另做一套）
async function openRunReader(category, id, rid, from = null) {
  const run = { ...(await api('GET', `${wfPath({ category, id })}/runs/${encodeURIComponent(rid)}`)), workflow: { category, id } };
  const list = runArtifactNodes(run);
  if (!list.length) { toast('這趟沒有留下成品', 'bad'); return; }
  openReader(list[0].id, run, from ?? { act: 'open-run-reader', cat: category, id, rid });
}
// 表格數字不折行：排版後的表格沒有「這格是數字」的標記，開著面板時就地補上 .num（純顯示，不改內容）
const NUM_CELL = /^[\s\d.,+\-%$¥£€元件次人天%／/:：~～]+$/;
function readerTagNums() {
  for (const td of document.querySelectorAll('.rdbody .docview td')) {
    const t = td.textContent.trim();
    td.classList.toggle('num', !!t && /\d/.test(t) && NUM_CELL.test(t));
  }
}
...
// 卡在半路：停著、卻沒有任何一步在等你也沒有在跑。最常見的成因是處理停點時剝繭被關掉——
// 那一步已經標完成，但放行沒跑到，下游永遠不會開始，畫面上也沒有任何卡片可以按。
function runStuck(run) {
  if (run.status !== 'paused') return false;
  return !Object.values(run.steps ?? {}).some((s) => {
    const st = s?.status ?? '';
    return st.startsWith('waiting') || st === 'time_pending' || st === 'failed' || st === 'running';
  });
}
function runStuckHtml() {
  return `<div class="panel stuckcard">
    <h3>這一趟停住了，但沒有在等你</h3>
    <p class="note">多半是上次處理停點的時候剝繭被關掉，中間那一下沒接上。按「繼續跑」會從沒跑完的地方接下去，已經做完的步驟不會重來。</p>
    <div class="saverow"><button class="btn btn-primary" data-act="run-resume-stuck"><i class="ph ph-play"></i>繼續跑</button></div>
  </div>`;
}
...
function promptModalHtml(view) {
  ...
  return `<div class="modalback" data-act="prompt-back"><div class="modal" style="max-width:680px">
      <div class="cvdhead"><b style="font-size:15px">當時送出的指示——${esc(view.title)}</b>
        <span class="btn btn-ghost iconb" data-act="prompt-close" style="margin-left:auto" tabindex="0" role="button"><i class="ph ph-x"></i></span></div>
...
function dashRunCard(r) {
  ...
  return `<div class="runcard${open ? ' open' : ''}">
    <div class="rchead" data-act="dash-expand" ${ids} tabindex="0" role="button"><b>${esc(r.name)}</b>${chip}
...
function usageModalHtml(d) {
  ...
  const seg = `<span class="seg sm">
    <span class="${d.usageView === 'flow' ? 'on' : ''}" data-act="dash-usage-view" data-view="flow" tabindex="0" role="button">按 Workflow</span>
    <span class="${d.usageView === 'day' ? 'on' : ''}" data-act="dash-usage-view" data-view="day" tabindex="0" role="button">按日</span></span>`;
  return `<div class="modalback" data-act="dash-usage-back"><div class="modal usagemodal" role="dialog" aria-label="用量明細">
      <div class="cvdhead"><b style="font-size:15px"><i class="ph ph-chart-bar"></i> 用量明細${d.usageView === 'day' ? hint('近30天') : ''}</b>${seg}
        <span class="btn btn-ghost iconb" data-act="dash-usage-close" style="margin-left:auto" title="關閉（Esc）" tabindex="0" role="button"><i class="ph ph-x"></i></span></div>
...
function dashHtml() {
  ...
  const showAll = recent.length > 3 ? `<span class="pbtn" data-act="dash-show-all" tabindex="0" role="button"><i class="ph ph-caret-${d.showAll ? 'up' : 'down'}"></i>${d.showAll ? '收起' : `查看全部（${recent.length}）`}</span>` : '';
  const entries = d.data.usage ?? [];
  ...
  return `<div class="dashpage">
    ...
    <aside class="homeaside">
      ...
      <section class="panel asideblock">
        ...
        <span class="pbtn" data-act="dash-usage-open" tabindex="0" role="button"><i class="ph ph-list-numbers"></i>用量明細 →</span>
      </section>
    </aside>
    </div>
    ${usageModalHtml(d)}
...
function calEvChip(e) {
  ...
  return `<div class="cev ${e.kind}" data-act="cal-ev" data-kind="${e.kind}" ${dr} ${runAttrs} data-time="${esc(e.time)}" data-title="${esc(e.title)}" tabindex="0" role="button">
...
function calDrawerHtml() {
  ...
  if (d.kind === 'goog') {
    return `<div class="cvdrawer" style="z-index:9">
      <div class="cvdhead"><span class="overline">Google 行程（唯讀快照）</span>
        <span class="btn btn-ghost iconb" data-act="cal-drawer-close" tabindex="0" role="button"><i class="ph ph-x"></i></span></div>
  ...
  const chips = (sched.remind_leads ?? []).map((l, i) => {
    ...
    return `<span class="rchip">${esc(label)}<button data-act="cal-lead-del" data-i="${i}" aria-label="移除這個提醒"><i class="ph ph-x"></i></button></span>`;
  }).join('');
  const steps = (state.calDrawerDef?.nodes ?? []).map((n) => `
    <div class="steprow" data-act="cal-step" data-node="${esc(n.id)}" tabindex="0" role="button">
  ...
  return `<div class="cvdrawer" style="z-index:9">
    <div class="cvdhead"><span class="overline">這一次</span>
      <span class="btn iconb" data-act="cal-run-now" title="現在就跑" style="color:var(--accent-text);background:rgb(var(--accent-rgb)/.12)" tabindex="0" role="button"><i class="ph-fill ph-play"></i></span>
      <span class="btn btn-ghost iconb" data-act="cal-drawer-close" tabindex="0" role="button"><i class="ph ph-x"></i></span></div>
    <h3 style="margin:6px 0">${esc(sched.name)}${hint('拖曳改時間，垃圾桶取消本次')}</h3><div class="sub">${esc(d.occ ? d.occ.replace('T', ' ') : '')}${sched.enabled === false ? '・<b style="color:var(--amber-text)">已暫停</b>' : ''}</div>
  ...
  if (state.schedManage) {
    ...
    const rows = (state.calendar?.scheds ?? []).map((s) => {
      ...
      return `<div class="steprow" style="cursor:default">
        ...
        <button class="btn sm2 btn-danger" data-act="smgr-del" data-sid="${esc(s.id)}" data-name="${esc(s.name ?? s.workflow_id)}">刪除</button>
      </div>`;
    }).join('');
    html += `<div class="modalback" data-act="smgr-back"><div class="modal">
      <div class="cvdhead"><b style="font-size:15px">排程清單${hint('含已暫停、過期與本月無場次者')}</b>
        <span class="btn btn-ghost iconb" data-act="smgr-close" style="margin-left:auto" tabindex="0" role="button"><i class="ph ph-x"></i></span></div>
    ...
    if (n) {
      ...
      html += `<div class="modalback" data-act="sm-back"><div class="modal">
        <div class="cvdhead"><span class="who ${n.executor === 'human' ? 'hu' : 'ai'}">${n.executor === 'human' ? '你' : 'AI'}</span>
          ...
          <span class="btn btn-ghost iconb" data-act="sm-close" style="margin-left:auto" tabindex="0" role="button"><i class="ph ph-x"></i></span></div>
  ...
  if (state.schedModal) {
    ...
    html += `<div class="modalback" data-act="sc-back"><div class="modal">
      <div class="cvdhead"><b style="font-size:15px">${sched ? '整條排程設定' : '新增排程'}</b>
        <span class="btn btn-ghost iconb" data-act="sc-close" style="margin-left:auto" tabindex="0" role="button"><i class="ph ph-x"></i></span></div>
    ...
    return `<option value="${v}" ${f.lead === v ? 'selected' : ''}>${t}</option>`;
      ...
      <div class="frow2"><span class="lb2">狀態${hint('提前提醒在點開單次事件的抽屜裡設定；建立後會直接出現在行事曆上。')}</span><label style="display:inline-flex;align-items:center;gap:6px;font-size:12.5px">
...
function closeCategory() {
  ...
  state.addingCategory = false; // 離開組織頁收起「新增分類」表單
...
function memNoticeLine(n, { where, idx = 0 } = {}) {
  ...
  if (n.kind === 'card') {
    ...
    return `<div class="memnotice"><i class="ph ph-cards"></i><span>記下來了：${esc(n.text)}</span>
      <span class="lnk ${busy ? 'busy' : ''}" data-act="${where === 'chat' ? 'mem-undo-chat' : 'mem-undo'}" data-card="${esc(n.card)}" data-idx="${idx}" tabindex="0" role="button">${busy ? '收回中⋯' : '不要記'}</span></div>`;
  }
...
// ---------- 公司頁／分類頁的共用檔兩區——規範（每步都帶）／參考（勾了才帶）；每檔一列 檔名・字數（二進位印 KB）・時間・刪除 ----------
...
function sharedSectionHtml(cp, kind) {
  ...
  // 共用檔那支讀不到：只有這兩區印一句＋重試（走 reload-category 整頁重讀），分類守則卡照常
...
// 分類頁最下面：該分類的 Workflow 卡（與 Workflow 庫同一張卡；樣稿 h2.section-title「分類流程」＋.libgrid）
...
// （樣稿 uxOrgPage）：組織頁＝麵包屑「工作空間」→ page-head（修改名稱、＋ 新增分類）→ 分類捷徑 → 兩張檔案卡；
// 分類頁＝麵包屑「工作空間 › 組織名」→ page-head（修改名稱）→ 整寬守則卡（拆好的規矩收在卡內 <details>，驗收第 7 條）→ 兩張檔案卡 → 分類 Workflow
function categoryPageHtml() {
  const cp = state.categoryPage;
  const company = cp.category === '_company'; // 公司頁＝分類頁特例（標題＝公司名，沒有守則卡、沒有 Workflow 區）
  // 「修改名稱」（公司→設定的 company_name；分類→PUT /api/categories/:name）；「未分類」不能改名，沒有鈕
  ...
  if (company) {
    // 驗收第 2 條：「新增分類」放組織頁（原側欄行內表單搬來，confirm-category 語意不變）
  ...
  if (cp.err) {
    return `<div class="catpage">${head}<section class="panel guidelines err"><h3><i class="ph-fill ph-warning"></i> 這個分類的守則讀不到</h3>
...
const SET_GROUPS = [['個人與記憶', '偏好、身分與資訊範圍'], ['Workflow 預設', '只影響新建的 Workflow'], ['連線', 'AI、行事曆與外部服務'], ['執行與排程', '啟動、提醒與補跑'], ['資料管理', '備份、垃圾桶與版本']];
const SET_TABS = {
  個人與記憶: ['關於你', '身分', '記憶總覽', '欄位詞典'],
...
async function openSettings(group = '個人與記憶', tab = null) {
  ...
  state.settings = { group, tab: tab ? { [group]: tab } : {}, detail: !!tab, save: null, data: null, err: null, idEdit: null, merge: null, confirm: null, msg: null, busy: null };
  render();
  await loadSettings();
  render();
}
...
async function loadSettings() {
  ...
  try {
    ...
    state.connectors = conns; // 外部服務清單（讀不到＝null，外部服務那段講一句）
  } catch (e) {
...
async function setPut(patch) {
  const s = state.settings;
  try { await api('PUT', '/api/settings', patch); if (s) s.save = '已儲存'; } catch (e) { toast(e.message, 'bad'); if (s) s.save = '儲存失敗'; }
...
async function setCardAct(act, id) {
  ...
  try {
    if (act === 'set-del') { if (!await askBox('把這張卡移到記憶垃圾桶？30 天內都能復原。')) return; await api('DELETE', p, undefined); }
...
function settingsHtml() {
  ...
  // 方案 B：先看這組的卡片總覽（每張＝一項設定＋現在的值），點卡整頁換成那一項，上方「‹ 返回」回總覽——卡片與詳情不同時出現
  const secs = setSections(body);
  let head = `<h2>${esc(s.group)}${hint(SET_LEAD[s.group] ?? '')}</h2>`;
  if (secs.length) {
    const cur = s.detail ? secs.find(([id]) => id === setTab(s.group)) : null;
    if (cur && !SET_OFF[cur[0]]) {
      head = '';
      body = setDetailHtml(s.group, cur[0], cur[2]);
    } else {
      const issues = {
        記憶總覽: d && !d.summary.paused && exN ? `${exN} 項要看` : '',
        Claude: state.claude === false ? '連不上' : '',
        'Google 行事曆': d?.snapshot && d.snapshot.status !== 'ok' ? '抓不到' : '',
        外部服務: (() => { const k = connServers().filter((x) => x.status !== 'connected').length; return k ? `${k} 家要處理` : ''; })(),
      };
      body = setOverviewHtml(s.group, secs, issues);
    }
  }
  return `${pageHeadHtml('設定', '個人偏好、Workflow 預設與系統執行，各有明確範圍。')}
    <div class="setwrap" data-settings><nav class="setnav" aria-label="設定項目">${nav}</nav><section class="setbody" data-group="${esc(s.group)}">${head}${body}</section></div>`;
}
// 組內分段標記（不印東西）：settingsHtml 照它把一組切成總覽卡片，第二個參數是卡面上那句現值
// （樣稿 PROTO/設定頁-三方案比較-demo.html 方案 B；段名與段內容都沒動，總覽只用段名與現值，點進去才印內容）
const SEC_RE = /<!--sec:([^|]*)\|([\s\S]*?)-->/;
// 現值裡可能有使用者自己打的字（組織名稱就是一格輸入框）——把會把標記切斷的兩種字換掉，不然整組設定會消失
const secSafe = (s) => String(s ?? '').replace(/\|/g, '｜').replace(/--+>/g, '→');
const setSec = (t, sum = '') => `<!--sec:${t}|${secSafe(sum)}-->`;
// 換總覽／詳情後，設定區頂端若已捲出畫面就捲回它——不捲到整頁最上面（窄版側欄平鋪在上方，捲到頂會把詳情擋在一整條側欄底下）
function setScrollTop() {
  const el = document.querySelector('[data-settings]');
  if (el && el.getBoundingClientRect().top < 0) el.scrollIntoView({ block: 'start' });
}
// 還沒做的項目：卡片灰掉、點不進去（連線輪起「連接器與金鑰」換成「外部服務」，現在沒有灰掉的）
const SET_OFF = {};
// 總覽：一張卡＝一項設定，卡面只有名字與現在的值；有狀況的換成琥珀膠囊，其餘一個「›」
const setOverviewHtml = (g, secs, issues = {}) => `<div class="ov-cards">${secs.map(([id, sum]) => {
  const off = SET_OFF[id];
  const iss = issues[id];
  return `<button type="button" class="ov-card${off ? ' off' : ''}" data-act="set-open" data-g="${esc(g)}" data-k="${esc(id)}"${off ? ' disabled' : ''}><b>${esc(id)}</b><p>${esc(sum)}</p>${iss ? `<span class="issue">${esc(iss)}</span>` : '<span class="arrow" aria-hidden="true">›</span>'}</button>`;
}).join('')}</div>`;
// 詳情：整頁只放這一項；標題旁是這組的範圍，右邊是儲存狀態（setPut 寫 state.settings.save）
function setDetailHtml(g, id, body) {
  const s = state.settings;
  const desc = SET_GROUPS.find(([x]) => x === g)?.[1] ?? '';
  const save = s.save ?? '改了就存';
  return `<div class="backline"><button type="button" data-act="set-back">‹ 返回${esc(g)}</button></div>
    <header class="detail-head"><div><h2>${esc(id)}</h2><p>${esc(desc)}</p></div><span class="save${save === '儲存失敗' ? ' bad' : ''}" data-save role="status">${esc(save)}</span></header>
    <div class="detail-body">${body}</div>`;
}
// 把一組的 HTML 依標記切成 [[段名, 現值, 內容]]；沒有標記（讀不到、還在載）就原樣一段
function setSections(html) {
  const out = [];
  let rest = html;
  let m = SEC_RE.exec(rest);
  if (!m) return out;
  rest = rest.slice(m.index);
  while (m) {
    const [tag, id, sum] = [m[0], m[1], m[2]];
    rest = rest.slice(rest.indexOf(tag) + tag.length);
    const next = SEC_RE.exec(rest);
    out.push([id, sum, next ? rest.slice(0, next.index) : rest]);
    m = next;
  }
  return out;
}
const setRow = (l, d, c, stack = false) => `<div class="row${stack ? ' stack' : ''}"><div class="l">${l}</div>${d ? `<div class="d">${d}</div>` : ''}<div class="c">${c}</div></div>`;
const setSw = (on, act, extra = '') => `<span class="sw ${on ? 'on' : ''}" data-act="${act}" ${extra} role="switch" aria-checked="${on}" tabindex="0">${on ? '開' : '關'}<i></i></span>`;
...
function setConnHtml(d) {
  ...
  return `${setSec('Claude', on ? '已連上' : '連不上')}<div class="group">
      ...
      ${setRow(`登入過期怎麼辦${hint('此處僅偵測指令存在與否。步驟持續失敗時，於終端機執行 claude 重新登入')}`, '', '')}
      ${setRow(`模型檔位對照${hint('各檔位對應的實際模型')}`, '', '<span class="sub" style="margin:0">快而省 Haiku・均衡 Sonnet・深而慢 Opus</span>')}</div>
    ${setSec('Google 行事曆', !snap ? '還沒抓過' : ok ? '有快照' : '抓不到')}<div class="group">
      ...
      ${setRow(`授權${hint('跟Claude共用Mcp系統，需於Claude解除授權')}`, '', '')}</div>${setMsgHtml('cal')}
    ${setSec('外部服務', extSum())}${setExtHtml()}`;
}
...
function setDefaultsHtml(d) {
  ...
  const seg = (k, cur, opts) => `<div class="seg sm" data-def="${k}">${opts.map(([v, t]) => `<span class="${String(cur) === v ? 'on' : ''}" data-act="set-def-val" data-k="${k}" data-v="${v}" tabindex="0" role="button">${t}</span>`).join('')}</div>`;
  const fl = df.supervisor_flags ?? {};
  const tierTxt = { null: '不設', fast: '快而省', balanced: '均衡', deep: '深而慢' };
  const retry = String(df.retry ?? 'null');
  const aiSec = `${setSec('AI 步驟', `模型 ${tierTxt[df.model_tier ?? 'null']}・重試 ${retry === 'null' ? '不設' : `${retry} 次`}`)}<div class="group"><h5>模型與重試</h5>
      ${setRow(`模型檔位${hint('選擇使用模型')}`, '', seg('model_tier', df.model_tier ?? 'null', [['null', '不設'], ['fast', '快而省'], ['balanced', '均衡'], ['deep', '深而慢']]))}
      ${setRow(`出錯自動重試${hint('失敗後自動重試次數；「不設」＝不重試')}`, '', seg('retry', df.retry ?? 'null', [['null', '不設'], ['0', '0 次'], ['1', '1 次'], ['2', '2 次']]))}
      ${setLater('產出語言', '現在跟指示的語言一樣。')}</div>`;
  // 拆之前先出成品卡讓你確認（預設開）；關掉＝一句話直接出草稿，清單頂端印拆解器自己定的長相對照
  const stopSec = `${setSec('停點', `先確認成品長相 ${d.cfg.compose?.confirm_shape !== false ? '開' : '關'}`)}<div class="group">${setRow(`拆之前先確認成品長相${hint('關閉＝直接出草稿，不先確認成品長相')}`, '', setSw(d.cfg.compose?.confirm_shape !== false, 'set-compose-sw', 'data-k="confirm_shape"'))}</div>
    <div class="group">${setRow('停點', '哪幾步要停下來等你，在每個步驟自己的設定裡改，不在這一頁。', '')}${setLater('停著沒處理多久提醒', '停點提醒間隔下一輪。')}</div>`;
  const permSec = `${setSec('權限與查核', `產出檔案 ${df.permissions_files !== false ? '開' : '關'}・每步都查 ${df.check_enabled !== false ? '開' : '關'}・監工 ${df.supervisor_enabled !== false ? '開' : '關'}`)}<div class="group"><h5>產出檔案的權限</h5>
      ${setRow(`親手建的 Workflow${hint('允許工人於產出資料夾寫檔與執行程式')}`, '', setSw(df.permissions_files !== false, 'set-def-sw', 'data-k="permissions_files"'))}
      ${setRow(`匯入的 Workflow${hint('固定關閉。匯入的指示不可信，需逐條 Workflow 手動開啟')}`, '', '<span class="chip">固定關</span>')}</div>
    <div class="group"><h5>交貨查核</h5>
      ${setRow(`每步都查${hint('詳細比對步驟內容與產出(增加Token消耗)')}`, '', setSw(df.check_enabled !== false, 'set-def-sw', 'data-k="check_enabled"'))}
      ${setRow(`數字對原始資料${hint('「看 Workflow」＝有必填欄位才比對原始資料')}`, '', seg('check_facts', df.check_facts ?? 'auto', [['auto', '看 Workflow'], ['on', '固定開'], ['off', '固定關']]))}</div>
    <div class="group"><h5>監工</h5>
      ${setRow(`監工：開場備註、每步交接、跑完紀錄${hint('新 Workflow 的預設值，可個別關閉')}`, '', setSw(df.supervisor_enabled !== false, 'set-def-sw', 'data-k="supervisor_enabled"'))}
      ${setRow(`每步「監工可以」的預設${hint('新 Workflow 的預設勾選，步驟可個別改')}`, '', `<div class="pills">${SUP_CHK.map(([, k, t]) => `<span class="pill ${fl[k] ? 'on' : ''}" data-act="set-def-flag" data-k="${k}" tabindex="0" role="button">${t}</span>`).join('')}</div>`)}</div>`;
...
function setExecHtml(d) {
  ...
  return `${setSec('常駐', `開機自動啟動 ${unsupported ? '不支援' : on ? '開' : '關'}`)}<div class="group">
    ...
    ${setSec('排程', `錯過時${ex.auto_makeup ? '自動補' : '先詢問'}・提醒 ${leads.length} 檔`)}<div class="group">
      ${setRow(`錯過時${hint('新排程的預設值：詢問或直接補跑；既有排程不受影響')}`, '', `<div class="seg sm" data-exec="auto_makeup"><span class="${ex.auto_makeup ? '' : 'on'}" data-act="set-exec-val" data-k="auto_makeup" data-v="false" tabindex="0" role="button">先詢問</span><span class="${ex.auto_makeup ? 'on' : ''}" data-act="set-exec-val" data-k="auto_makeup" data-v="true" tabindex="0" role="button">自動補</span></div>`)}
      ${setRow(`提醒預設時間點${hint('新排程的提前提醒（可多選）；既有排程不受影響')}`, '', `<div class="pills" data-exec="remind_leads">${LEAD_ORDER.map((k) => `<span class="pill ${leads.includes(k) ? 'on' : ''}" data-act="set-exec-lead" data-k="${k}" tabindex="0" role="button">${LEAD_TXT[k]}</span>`).join('')}</div>`, true)}</div>
    ${setSec('AI 工人', `允許查網路 ${ex.web !== false ? '開' : '關'}・跑完出總覽 ${ex.overview_enabled !== false ? '開' : '關'}`)}<div class="group">
      ...
      ${setRow('跑完出總覽', ex.overview_enabled === false ? '關著：跑完直接看成品清單，不多花一次呼叫。' : '開著：一趟跑完多發一次呼叫，整理出「這次得到什麼」——三到五句重點，數字能畫的就畫。', setSw(ex.overview_enabled !== false, 'set-exec-sw', 'data-k="overview_enabled"'))}
      ${setRow(`外部服務${hint('只准讀：寄出、修改、刪除一律擋；只有勾了服務的步驟摸得到，勾哪家摸哪家。有哪些服務看「連線→外部服務」。')}`, '', '<span class="chip">只讀</span>')}</div>`;
}
...
function orgManageHtml() {
  ...
  return `<div class="group" data-orgs><h5>全部組織・${list.length} 個${hint('各組織資料獨立不互通，切換將重載畫面')}</h5>
...
function setDataHtml(d) {
  ...
  const cleanSec = `${setSec('清理', `垃圾桶 ${rows.length} 項`)}<div class="group"><h5>垃圾桶・${rows.length} 項${hint('刪除項保留 30 天可復原，到期自動清除')}</h5>
  ...
  const orgSec = `${setSec('組織', d.cfg.company_name || '還沒取名')}<div class="group">
      ${setRow(`組織名稱${hint('側欄、麵包屑、組織頁同步變更；清空顯示「組織」')}`, '', `<input class="notein" id="set-company-name" type="text" maxlength="60" placeholder="組織" value="${esc(d.cfg.company_name ?? '')}" style="margin:0;max-width:260px">`)}
      ${setRow('規範上限', '單檔 4,000 字・每層合計 8,000 字', '<span class="chip">固定</span>')}${setMsgHtml('company')}</div>
    ${orgManageHtml()}
    ${setSec('位置與備份', `${bk.length} 份備份`)}<div class="group"><h5>位置</h5>
      ${setRow('資料夾', `<span class="path" data-datadir>${esc(d.cfg.data_dir ?? '')}</span>`, '<span class="chip">換位置下一輪</span>')}</div>
    <div class="group"><h5>備份</h5>
      ${setRow(`立即備份${hint('複製整個資料夾至同層「資料夾名-backups」，不含金鑰')}`, '', `<button class="btn sm2" data-act="set-backup-now" ${s.busy ? 'disabled' : ''}><i class="ph ph-copy"></i>${s.busy === 'backup' ? '備份中⋯' : '立即備份'}</button>`)}
...
function setConfirmModalHtml() {
  ...
  return `<div class="pvback memmodal" data-act="set-clear-mem-back" data-confirm="memory"><div class="modal" role="dialog" aria-label="清空記憶">
    ...
    <p class="sub" style="margin:8px 0">習慣卡與認識卡都會進記憶垃圾桶，30 天內可以在「資料管理→清理」逐張復原；欄位詞典、分類守則、身分、Workflow 都不動。清空之後每步指示不再附「關於你」，開跑表單也不再有習慣選項，直到你重新記。</p>
...
function setMemoryHtml(d) {
  ...
  const open = state.settings?.detail === true;
  const c = d.summary.counts;
  // 跟其他組一樣出切段標記；但只有正在看的那一項才算內容（記憶總覽要跑全部卡），總覽時四段都是空的
  const sums = { 關於你: `${c.profile} 張認識卡`, 身分: `${d.identities.length} 個身分`, 記憶總覽: `習慣 ${c.habit}・認識 ${c.profile}`, 欄位詞典: `${d.dict.fields.length} 格` };
  const bodyOf = (k) => (k === '身分' ? setIdentitiesHtml(d) : k === '記憶總覽' ? setMapHtml(d) : k === '欄位詞典' ? setDictHtml(d) : setKnowHtml(d));
  return SET_TABS.個人與記憶.map((k) => `${setSec(k, sums[k] ?? '')}${open && k === tab ? bodyOf(k) : ''}`).join('');
}
// 外部服務卡面上的現值（連線輪取代「連接器與金鑰」兩列「尚未提供」）
function extSum() {
  const c = state.connectors;
  if (c === undefined) return '讀取中';
  if (c === null) return '讀不到';
  if (!c.checked_at) return '還沒檢查';
  return `${(c.servers ?? []).filter((x) => x.status === 'connected').length} 家已連上`;
}
...
function setMapHtml(d) {
  ...
  const catBlock = (cat, wfs) => {
    ...
    const catCtx = profiles.filter((c) => c.layer === 'content' && c.scope?.level === 'category' && c.scope.category === cat).sort(byRecentDesc);
    // Workflow 一列：名字＋帶幾張的數字；底下只列「生在這條」的卡——全部／分類範圍的卡印在它們自己那一塊，不在每條底下重複
    const flowBlock = (w) => {
      ...
      const ownCtx = matched.filter((c) => c.scope?.level === 'workflow');
      const items = [...ownCtx.map((c) => memCardLi(c, '按場合帶・生在這條')), ...own.map((c) => memCardLi(c, '選項・生在這條'))].join('');
      const over = matched.length - ctx.length;
      return `<div class="mflow" data-mapflow="${esc(w.id)}"><div class="fh"><i class="ph ph-flow-arrow"></i>${esc(w.name)}<span class="cnts">每步帶 ${exp.length + ctx.length}${over > 0 ? `（另 ${over} 張場合對上、排在前 ${MEM_CONTENT_TOP} 張之外）` : ''}・選項 自己 ${own.length}＋繼承 ${inh.length}</span></div>
        ${items ? `<ul>${items}</ul>` : ''}</div>`;
    };
    const catItems = [...rules.map((r) => memCardLi({ ...r, bucket: 'group' }, `${cat}的規矩`, cat)), ...catCtx.map((c) => memCardLi(c, '按場合帶・這個分類')), ...catHabits.map((c) => memCardLi(c, '選項・這個分類'))].join('');
    return `<div class="mcat" data-mapcat="${esc(cat)}"><div class="mh"><i class="ph ph-folder"></i>${esc(catLabel(cat))}<span class="sub">規矩 ${rules.length} 條・共用 ${catHabits.length} 張習慣卡</span><span class="chip">${wfs.length} 條 Workflow</span></div>
      ${catItems ? `<ul>${catItems}</ul>` : ''}
      ${wfs.map(flowBlock).join('') || '<div class="none">這個分類還沒有 Workflow</div>'}</div>`;
  };
  const allHabits = habits.filter((c) => c.scope?.level === 'all');
  const allCtx = profiles.filter((c) => c.layer === 'content' && c.scope?.level === 'all').sort(byRecentDesc);
  const allN = exp.length + allCtx.length;
  const allBlock = `<div class="mcat" data-mapcat="*"><div class="mh"><i class="ph ph-globe-simple"></i>全部 Workflow<span class="sub">到哪都帶</span><span class="chip">${allN} 張認識卡・${allHabits.length} 張習慣卡</span></div>
    ${allN || allHabits.length ? `<ul>${exp.map((c) => memCardLi(c, '每步帶')).join('')}${allCtx.map((c) => memCardLi(c, '按場合帶')).join('')}${allHabits.map((c) => memCardLi(c, '全部 Workflow 的選項')).join('')}</ul>` : '<div class="none">還沒有</div>'}</div>`;
  // 例外有東西才放最上面；全空就不印四個「現在沒有」的框
  const ex = d.summary.exceptions;
  const excN = Object.values(ex).reduce((a, v) => a + v.length, 0) + d.cards.filter((c) => c.status === 'retired').length;
  const exc = excN ? `<h5 class="sech"><i class="ph ph-info"></i>需要你看一眼的${hint('也會出現在儀表板「要你處理」。點任何一條可以看出處原話')}</h5>${setExcHtml(d)}` : '';
  return `${exc}<h5 class="sech"><i class="ph ph-tree-structure"></i>完整記憶地圖${hint(`「每步帶 N」是不指定身分時的口徑（表達層全部＋場合對上的事實最近用過的前 ${MEM_CONTENT_TOP} 張）；指定身分的 Workflow 實際帶的會更少。每張卡只印在它自己的適用範圍`)}</h5>
...
function setExcHtml(d) {
  ...
  const row = (c, extra, acts) => `<li><span class="cardlnk" data-act="mem-card" data-id="${esc(c.id)}" data-bucket="${esc(c.bucket)}" data-text="${esc(c.text)}" title="點開看出處原話" tabindex="0" role="button">${memTagHtml(c)}<span class="t">${esc(c.text)}</span></span>${extra ? `<span class="m">${extra}</span>` : ''}<span class="acts">${acts}</span></li>`;
...
function setKnowHtml(d) {
  ...
  return `${introBlock}<div class="group">
    ...
    <div class="group" data-layer="expression"><h5>每步帶・${expLive.length} 條${hint('語氣、長度、格式、禁忌，每步附上')}</h5>${exp.map(crow).join('') || '<div class="none">還沒有</div>'}</div>
    <div class="group" data-layer="content"><h5>按場合帶・${ctxLive.length} 條${hint('讀者、公司、進行中的事；僅場合相符的 Workflow 附上')}</h5>${ctx.map(crow).join('') || '<div class="none">還沒有</div>'}</div>`;
}
...
function setDictHtml(d) {
  ...
  const mergeRow = (f) => (m?.drop === f.name
    ? `<tr class="mergerow"><td colspan="5"><div class="mergebox"><span>把「${esc(f.name)}」併進</span>
        ...
        <span class="sub" style="margin:0">同義詞併過去、掛在它上面的習慣卡與分類守則改掛過去；之後詞典裡就沒有「${esc(f.name)}」了。</span>
...
function setIdentitiesHtml(d) {
  ...
  return `<div class="group"><h5>身分${hint('認識卡的封套，開跑可切換；僅限縮「關於你」')}</h5>
...
async function openAssets() {
  ...
  closeDash();
  closeCategory();
  closeSettings(); closeAssets();
  stashWorkspace(); // 同其他離開站：離開工作區先打包
  state.run = null;
  state.showTrash = false;
  state.corrupt = null;
  state.importPreview = null;
...
// 浮窗焦點關在裡面（L058）：閱讀面板、成品預覽、步驟彈窗任一開著，後面的 .layout 設 inert——Tab／Shift+Tab、滑鼠、讀屏都碰不到後面；
// 成品預覽疊在閱讀面板上時面板也鎖，焦點只在最上面那張浮窗裡走。選 inert 不自己算 Tab 循環：可聚焦清單交給瀏覽器管，
// 不用維護「哪些元素可聚焦」的選擇器，三種浮窗一條規則就夠。每次 render 換完 DOM 呼叫（屬性跟著 DOM 一起重算，關掉即恢復）
function lockBehindOverlays(root) {
  const rd = root.querySelector('.rdback');
  const pv = root.querySelector('[data-act="preview-back"]');
  const step = root.querySelector('.stepmodal');
  root.querySelector('.layout')?.toggleAttribute('inert', !!(rd || pv || step));
  rd?.toggleAttribute('inert', !!pv);
}
function render() {
  state.pollDirty = false; // F5：整頁從 state 畫，畫完就沒有「輪詢到了還沒畫的」——不論是誰叫的 render
  ...
  else if (state.wfOpening) { inner = ''; landing = true; } // US-107 ⑥：正在打開上次看的那條——右邊骨架（wfStateWrap 換上）
  else if (state.wfEmpty && !state.wf) { inner = wfEmptyHtml(); landing = true; } // 合併輪（US-106）：Workflow 頁還沒選流程＝右邊列三件事，不留白
  else {
  ...
  const rdScroll = app.querySelector('.rdbody')?.scrollTop ?? 0; // 閱讀面板同理：跑完那幾秒還在輪詢，讀到一半不該被捲回頂端
  ...
  const keepFocus = fa?.dataset?.act && fa.closest('.side, .wftree, .rowmenu') && fa.matches(':focus-visible') ? { ...fa.dataset } : null;
  ...
  rendering = true;
  // 合併輪（US-105）：Workflow 頁＝左樹右內容。樹只在這條路上（儀表板／行事曆／設定／執行頁沒有樹）；
  // 收起時整個版位換成一條窄邊（.wfpage.treeoff），右邊寬度回到合併前——畫布不因為合併而變窄
  const shell = onWfRoute()
    ? `<div class="wfpage${state.treeOff ? ' treeoff' : ''}">${wfTreeHtml()}<section class="wfmain">${wfStateWrap(inner, landing)}</section></div>`
    : inner;
  app.classList.toggle('navoff', !!state.navOff); // US-107 ①：主導覽收合掛在 #app（不隨重繪換掉），收合時欄寬才有過場
  ...
  if (rdScroll) { const b = app.querySelector('.rdbody'); if (b) b.scrollTop = rdScroll; }
  ...
  if (state.permFlashUntil > Date.now()) document.getElementById(state.permFlashId === 'permissions.connectors' ? 'perm-connectors' : 'perm-files')?.scrollIntoView({ block: 'center', behavior: 'auto' });
...
function schedulePoll(recordDelay) {
  ...
  state.pollTimer = setTimeout(async () => {
    ...
    try {
      ...
      // F5：這一輪若轉成 paused／done，下面就不再排下一輪，帳由 flushPollDirty（離開輸入框時）來清——不為了 dirty 永遠輪詢。
      if (changed || state.pollDirty) {
        if (isTyping()) state.pollDirty = true;
        else { state.pollDirty = false; render(); }
      }
    } catch (e) { if (gen !== runGen) return; pollFailed('執行狀態', e); }
    schedulePoll(running ? undefined : 5000); // 跑完等紀錄：第一次 2 秒，之後退避到 5 秒
  }, delay);
}

async function refreshRun() {
  const gen = runGen; // F1：同 schedulePoll——發前記代次
  const w = state.run.workflow;
  const before = state.run.status;
  const fresh = await api('GET', `${wfPath(w)}/runs/${state.run.run_id}`);
  if (gen !== runGen) return; // F1：等回應期間離開／換頁了，這份是舊頁的
...
async function sendChat(round = null) {
  ...
  try {
    ...
    if (out.phase === 'shape' && !out.auto) {
      ...
      // 等待中它的分類被改名或搬走＝路徑換了（舊路徑 PUT 會 404、改動丟掉）；原鍵不在最新清單時，
      // 照 id 找回現在的分類——同 id 恰好一條才認（範例檔 id 可能在兩個分類重複，認不準就照舊路徑、寧可報錯不寫錯條）
      ...
      await api('PUT', wfPath(target), { def: out.draft, from: 'chat' }); // from: chat＝伺服器沿用舊權限，AI 的 YAML 動不了開關
      if (state.wf && flowKey(state.wf) === flowKey(target)) state.wf.def = { ...out.draft, permissions: state.wf.def?.permissions ?? out.draft.permissions }; // 切走了＝開回來會重抓；權限照伺服器沿用舊值
...
async function cvUndo() {
  ...
  try { await cvApplySnapshot(prev); } catch (e) { h.undo.push(prev); h.redo.pop(); toast(e.message, 'bad'); render(); }
}
async function cvRedo() {
  ...
  h.undo.push(cur);
  try { await cvApplySnapshot(nxt); } catch (e) { h.redo.push(nxt); h.undo.pop(); toast(e.message, 'bad'); render(); }
}
...
async function cvSave() {
  ...
  if (empty.length) {
    toast(`還有 ${empty.length} 條擇一的線沒寫條件——點線上的條件格寫好再存檔`, 'bad');
    render();
    return;
  }
  if (state.cvSaving) return; // 連按兩次會送兩次 PUT，版本履歷就多一筆一模一樣的
  state.cvSaving = true;
  render();
  try {
  ...
  // 存檔＝記一版：版本標籤要跟著換，不然得離開再回來才看得到新版號
  try { if (state.wf && !subjectIsDraft()) state.versions = await api('GET', `${wfPath(state.wf)}/versions`); }
  catch { /* 版號沒抓到不影響已經存好的檔 */ }
  if (issues.length) toast(`已存檔。注意：有 ${issues.length} 處還沒接好（已標紅）——接好並存檔之前，這個 Workflow 按「開始」不會跑。`, 'bad');
  else toast('已存檔');
  render();
}
...
// 未存檔離開攔截（US-105）：右邊直接可編輯的代價——改到一半切別的流程、切組織、離開這一頁都先問一句。
// 用既有 askBox（原生 confirm 會把整個分頁凍住、長相也不受控）。草稿不攔（瀏覽器暫存有），執行頁不攔。
async function leaveGuard() {
  if (!state.wf || state.run || subjectIsDraft() || !state.cvDirty) return false;
  return !(await askBox('還沒存檔，確定要走嗎？', { ok: '不存檔就走', cancel: '留下來', title: '這條 Workflow 改了還沒存' }));
}
// Workflow 頁（US-105）：側欄「Workflow」與所有舊的「Workflow 庫」連結都走這裡。
// 有正在看的流程＝回到它（樹上仍選著同一條，舊連結不會變死連結）；沒有＝右邊列空狀態三件事。
async function openWfPage() {
  ...
  state.corrupt = null;
  state.importPreview = null;
  state.wfEmpty = !state.wf && !draftLive();
  // US-107 ⑥：沒有正在看的東西＝直接打開上次看的那條（記在本機）；第一次進來、那條已刪除或不在目前組織才留在落地頁
  const last = state.wfEmpty ? lastFlowPick() : null;
  if (last) {
    state.wfEmpty = false;
    state.wfOpening = true;
    render(); // 先出骨架，不讓人對著舊畫面等
    let ok = false;
    try { ok = await openWorkflow(last.category, last.id); } catch { /* 讀不到＝退回落地頁 */ }
    state.wfOpening = false;
    if (!ok) state.wfEmpty = !state.wf && !state.corrupt; // 壞檔＝openWorkflow 已切到「讀不懂」卡，別蓋掉它
  }
  await refreshTreeQuiet(); // 樹上狀態點要新的（US-107：清單每筆帶 last_run）
  if (state.wfEmpty) await refreshRecent(); // 落地頁的「卡住的／最近跑完的」才要跨流程最近幾趟
  render();
}
// 重抓清單但不往外丟：讀不到就是 treeLoad=error（樹停用＋重新讀取）；開站那次沒讀到的草稿暫存，讀到了才接回
async function refreshTreeQuiet() {
  try { await refreshLibrary(); } catch { /* refreshLibrary 已把 treeLoad 記成 error */ }
  dsResume();
}
function dsResume() {
  if (state.treeLoad === 'ok' && dsKeyHeld) { dsKey = dsKeyHeld; dsKeyHeld = null; dsLoad(); }
}

// 清單讀不到→「重新讀取」（US-107 ③）：先出載入中（樹停用；右邊有內容就留著，沒有才出骨架），讀完回 ok 或 error。
// 開站那次就讀不到的話，瀏覽器暫存的草稿還沒讀回（dsLoad 要拿清單判哪份還在）——這次讀到了才接回
async function treeReload() {
  state.treeLoad = 'loading';
  render();
  await refreshTreeQuiet();
  if (state.wfEmpty) await refreshRecent();
  render();
}

// 打開一條流程（樹上點選、監工紀錄的「改這裡」共用）：讀定義＋執行清單＋提議＋參考檔，工作區狀態歸零。
...
async function openWorkflow(category, id) {
  ...
  state.run = null;
  state.wfEmpty = false; // 選到流程了，右邊不再是空狀態
...
async function cvTidy() {
  if (!cvDef()) return;
  if (!await askBox('整理排列會照步驟順序把圖重新排好——你自己拖過的位置會被蓋掉。\n排完可以按 Ctrl+Z 復原；沒按存檔就不會寫回去。')) return;
  try {
...
document.addEventListener('keydown', (e) => {
  ...
  if (e.key === 'Escape' && state.connPick) { // 步驟服務選單 Esc 關，焦點回那顆標籤
    const { node } = state.connPick;
    state.connPick = null;
    render();
    [...document.querySelectorAll('[data-act="conn-pick"]')].find((x) => x.dataset.node === node)?.focus();
    return;
  }
  ...
  if (e.key === 'Escape' && state.reader) { closeReader(); return; } // 閱讀面板：Esc 關，焦點回原來那一列（放在成品預覽之後——浮窗疊在面板上時先關浮窗）
...
// F5（Codex 09-25 審查）：打字時輪詢只記帳不重繪（schedulePoll）；流程在你打字時停下（paused／done）就沒有下一輪來補畫，
// 畫面會一直停在「還在跑」。離開輸入框＝手停了，這裡補畫一次。工單給的兩種做法選「失焦補畫」不選「只換狀態區塊」：整頁由字串模板產生，
// 狀態散在標題、步驟卡、按鈕列，沒有一塊獨立的狀態區可局部換。延到下一拍再看：focusout 當下 activeElement 還是 body，
// Tab 到另一個有字的框要等那一拍才看得到、isTyping 才擋得住（等那格也離開再補）。重繪中拔掉輸入框造成的 focusout 不算（rendering）。
// 只補畫、不打 API、不重啟輪詢：要不要再輪詢由 schedulePoll 自己看狀態決定。
function flushPollDirty() {
  if (!state.pollDirty || rendering) return;
  setTimeout(() => { if (state.pollDirty && !rendering && !isTyping()) { state.pollDirty = false; render(); } }, 0);
}
...
app.addEventListener('animationend', (e) => {
  if ((e.target.id === 'perm-files' || e.target.id === 'perm-connectors') && e.animationName === 'permflash') {
...
app.addEventListener('input', (e) => {
  ...
  if (t.id === 'tree-search') { // 樹的搜尋（US-105）：只換分類與流程那一塊，不整頁重繪（打字不被洗掉）
    state.treeQ = t.value;
    const g = document.getElementById('tree-nodes');
    if (g) g.innerHTML = treeNodesHtml();
    const miss = document.getElementById('tree-search-empty'); // US-107 ④：落空才出「清除搜尋」
    if (miss) miss.hidden = !treeSearchMiss();
  }
...
app.addEventListener('change', async (e) => {
  ...
  // 標題列「搬到別的分類」下拉退場（驗收第 5 條：移至分類在「⋯」選單，走 moveWorkflowTo）
  ...
  if (e.target.id === 'shape-category') { // 成品卡分類下拉：只進 state，W1 第二趟 body 帶它；W2：分類規範數跟著分類換，再讀一遍
  ...
  if (e.target.dataset?.dictKind !== undefined && state.settings) {
    try { await api('PUT', `/api/memory/dict/${encodeURIComponent(e.target.dataset.dictKind)}`, { kind: e.target.value }); } catch (err) { toast(err.message, 'bad'); }
    await loadSettings();
    render();
  }
...
document.getElementById('sample-file').addEventListener('change', async (e) => {
  ...
  try {
    ...
    if (file.size > 10 * 1024 * 1024) { toast('這份檔案超過 10MB，剝繭存不下——換一份小一點的，或把它放進 Workflow 的參考檔。', 'bad'); return; }
    const buf = new Uint8Array(await file.arrayBuffer());
    let bin = '';
    for (let i = 0; i < buf.length; i += 0x8000) bin += String.fromCharCode(...buf.subarray(i, i + 0x8000));
...
// 公司頁／分類頁共用檔上傳——層與區看 state.sharedUpload；規範區前端先擋副檔名（句子同伺服器），伺服器再擋一次；
...
app.addEventListener('click', async (e) => {
  ...
  // 步驟服務選單點外面就關（同上）
  if (state.connPick && !e.target.closest('.connpop') && !e.target.closest('[data-act="conn-pick"]')) {
    state.connPick = null;
    render();
  }
  ...
  try {
    ...
    // ===== 連線輪：外部服務清單、步驟要用哪家 =====
    else if (act === 'conn-refresh') await refreshConnectors();
    else if (act === 'conn-go-settings') { state.connPick = null; await openSettings('連線', '外部服務'); }
    else if (act === 'conn-pick') {
      const node = el.dataset.node;
      if (state.connPick?.node === node) { state.connPick = null; render(); return; } // 再點同一顆＝關
      state.connPick = { node, ...connPickPos(el.getBoundingClientRect(), window.innerWidth, window.innerHeight) };
      render();
      document.querySelector('.connpop [data-act="conn-toggle"]:not([disabled])')?.focus();
    } else if (act === 'conn-toggle') {
      // 清單／檢視器／選單：即改即存（清單）、進工作本（畫布）——都走 canvasOp；使用者一改就記成 user，之後重拆不改回去
      const id = el.dataset.node;
      await canvasOp((def) => {
        const n = def.nodes.find((x) => x.id === id);
        if (!n) return;
        n.connectors = connToggled(n.connectors, el.dataset.connName);
        n.connectors_set_by = 'user';
      });
      if (state.connPick) document.querySelector(`.connpop [data-conn-name="${CSS.escape(el.dataset.connName)}"]`)?.focus();
    } else if (act === 'conn-dlg-toggle') {
      // 步驟彈窗：只改膠囊與隱藏欄，按「套用」才寫回（整頁重繪會洗掉彈窗裡沒套用的勾）
      const box = document.getElementById('cv-connectors');
      if (!box) return;
      const next = connToggled(JSON.parse(box.value || '[]'), el.dataset.connName);
      box.value = JSON.stringify(next);
      const on = next.includes(el.dataset.connName);
      el.classList.toggle('on', on);
      el.setAttribute('aria-pressed', String(on));
      el.disabled = !on && /（(還沒授權|連不上|不在清單上)）$/.test(el.dataset.connText); // 取消掉沒授權的就勾不回去
      el.innerHTML = `${on ? '<i class="ph ph-check"></i>' : ''}${esc(el.dataset.connText)}`;
      el.closest('.connpick')?.querySelector('.connai')?.remove(); // 改過就不是 AI 的建議了
    }
    ...
    } else if (act === 'open-assets') { if (await leaveGuard()) return; await openAssets(); } // 共用素材整頁（樹底部入口）
    // ===== 儀表板=====
    else if (act === 'open-dash') {
      if (await leaveGuard()) return; // US-105：改到一半離開這一頁先問
      stopPoll();
      closeCalendar();
      closeCategory();
      closeSettings(); closeAssets();
    ...
    // ===== 閱讀面板（US-103）：清單點一列從右側滑出；上一份／下一份留在面板裡不關掉 =====
    else if (act === 'open-reader') openReader(el.dataset.node);
    else if (act === 'reader-prev' || act === 'reader-next') openReader(el.dataset.node, state.reader?.run ?? null, state.reader?.from ?? null);
    else if (act === 'reader-close' || (act === 'reader-back' && e.target === el)) closeReader();
    else if (act === 'ov-cite') openReader(el.dataset.node, state.reader?.run ?? null, state.reader?.from ?? null); // 總覽的「出處 › 第 N 步」：直接開那一步的閱讀面板
    else if (act === 'ov-record') document.getElementById('run-record-anchor')?.scrollIntoView({ block: 'start', behavior: 'smooth' });
      ...
      render();
    } else if (act === 'perm-conn-toggle') {
      // 讀服務權限開關（permissions.connectors）：同產檔那顆——切換即 PUT，沒存成撥回並在列上講一句
      const def = structuredClone(state.wf.def);
      def.permissions = { ...(def.permissions ?? {}), connectors: el.checked };
      state.permConnErr = null;
      try {
        await api('PUT', wfPath(state.wf), { def });
      } catch (err) {
        state.permConnErr = `沒存成：${err.message}`;
        render();
        return;
      }
      state.wf.def = def;
      if (state.cvWork) state.cvWork.permissions = def.permissions;
      render();
      recheckHealth(); // 開了就不再擋：健檢卡跟著換
    }
    // ===== 交貨查核=====
    ...
    // ===== 行事曆=====
      ...
      if (!await askBox(`取消這一次的「${el.dataset.title}」？（只影響這一次，排程照常）`)) return;
      ...
      if (act === 'cal-lead-del') leads.splice(Number(el.dataset.i), 1);
        ...
        const pick = await askInput('提前多久提醒？填 10m／30m／1h／2h／3h／6h／12h／1d／2d，或自訂時刻（例 2026-08-23T20:00）', { value: '1d', title: '加一次提醒' });
        if (!pick) return;
    ...
    else if (act === 'smgr-del') {
      // 後端一直都有 DELETE，只是前端從來沒人呼叫它——排程建了就刪不掉，只能一直「暫停」
      if (!await askBox(`刪掉排程「${el.dataset.name}」？以後不會再自動跑，已經跑過的紀錄留著。`, { ok: '刪掉排程', danger: true })) return;
      try {
        await api('DELETE', `/api/schedules/${encodeURIComponent(el.dataset.sid)}`, undefined);
        await loadCalendar();
        toast('排程已刪除');
      } catch (err) {
        toast(err.message, 'bad');
    ...
    else if (act === 'run-resume-stuck') {
      try {
        const r = await api('POST', `${wfPath(state.run.workflow)}/runs/${state.run.run_id}/resume-stuck`, {});
        state.run = r;
        toast('接上了，繼續跑');
      } catch (err) {
        toast(err.message, 'bad');
      ...
      const s = state.settings;
      try { s.data.autostart = await api('POST', '/api/autostart', { enabled: !(s.data.autostart?.enabled === true) }); } catch (err) { toast(err.message, 'bad'); }
      render();
    ...
    // ===== 群組圈分類頁=====
      ...
      e.preventDefault(); // summary 上的文字＝開分類頁，不讓 details 原生收合（樹不跳動）
      if (await leaveGuard()) return;
      stopPoll();
      closeCalendar();
      closeDash();
      ...
      state.corrupt = null;
      state.importPreview = null;
      state.sharedOpen = false; // U4b：從共用檔浮窗「去分類頁管理」進來的，回流程頁時浮窗不該還開著
      state.categoryPage = { category: el.dataset.cat, data: null, cards: [], err: null, saving: false, saveErr: null };
      render();
      await loadCategoryPage();
      render();
    } else if (act === 'open-company') { // 樹最上面那一列／麵包屑第一節→組織頁（分類頁特例）
      if (await leaveGuard()) return; // US-105：離開這一頁先問
      stopPoll();
      closeCalendar();
      closeDash();
      closeCategory();
      ...
      input.click();
    } else if (act === 'shared-view') { // 組織頁／分類頁共用檔「查看」——開既有預覽浮窗
      ...
      if (await leaveGuard()) return; // US-105：改到一半按「建立新 Workflow」也是離開這一頁，先問
      // 已有一份沒存的草稿（暫存的，或正在看的）→先問；丟掉＝不進暫存
      const onDraft = !state.wf && subjectIsDraft();
      if ((onDraft || wsByFlow.has('__draft__')) && !await askBox('已有一份沒存的草稿，要丟掉重來嗎？')) return;
      closeCalendar();
      closeDash();
      closeCategory();
      closeSettings(); closeAssets();
      ...
      state.mode = 'chat';
      render();
    } else if (act === 'open-draft') { // 樹上「草稿・還沒存」列→讀回暫存的新流程草稿
      if (await leaveGuard()) return;
      state.wfEmpty = false;
      stopPoll();
      closeCalendar();
      ...
      if (!await askBox('丟掉這份還沒存的草稿？裡面的對話與成品卡都會不見，救不回來。')) return;
      wsByFlow.delete('__draft__');
      ...
      if (!await askBox(`把「${name}」移到垃圾桶？30 天內都能復原。`)) return;
      ...
      await refreshLibrary();
      render();
    } else if (act === 'mode-chat') { if (await canvasLeaveBlocked()) return; state.mode = 'chat'; render(); }
    else if (act === 'mode-list') { if (await canvasLeaveBlocked()) return; state.mode = 'list'; render(); }
    else if (act === 'mode-canvas') { state.mode = 'canvas'; render(); }
    else if (act === 'mode-history') {
      if (await canvasLeaveBlocked()) return;
      ...
      if (sample) {
        try {
          ...
          toast(`Workflow 存好了，但那份舊作品沒上傳成功：${err.message}\n到 Workflow 頁的參考檔再上傳一次就好。`, 'bad');
        }
      }
      state.chat = emptyChat();
      state.savingDraft = false;
      ...
      const same = state.wf?.category === cat && state.wf?.id === id;
      if (!same && await leaveGuard()) return; // US-105：改到一半切別條流程先問
      await openWorkflow(cat, id);
      render();
    } else if (act === 'del-run') {
      if (!await askBox('這一趟不用跑完了？會連同它的紀錄、產出檔、指示卷宗一起刪掉，救不回來。')) return;
      ...
      if (act === 'start') {
        ...
        if (pf?.issues?.length) {
          state.startCheck = { key: defKey, level: pf.issues.some((i) => i.level === 'block') ? 'block' : 'warn', issues: pfIssuesView(pf.issues) };
          render();
          return;
        }
      }
      ...
      try {
        ...
        if (err.status === 409 && Array.isArray(err.body?.issues)) { // 後端兜底擋下：用同一張卡顯示
          state.startCheck = { key: defKey, level: 'block', issues: pfIssuesView(err.body.issues) };
          render();
          return;
        }
        throw err;
      ...
      input.click();
    } else if (act === 'run-attach-add') {
      const input = document.createElement('input');
      input.type = 'file';
      input.accept = RUN_UPLOAD_ACCEPT;
      input.multiple = true; // 一次挑好幾個也行
      input.addEventListener('change', () => { for (const f of input.files ?? []) uploadRunFile(RUN_ATTACH_KEY, f); });
      input.click();
    } else if (act === 'run-attach-del') {
      const list = runAttachList();
      list.splice(Number(el.dataset.i), 1);
      if (list.length) state.runUploads[RUN_ATTACH_KEY] = list;
      else delete state.runUploads[RUN_ATTACH_KEY];
      render();
      recheckHealth();
      ...
      if (el.dataset.kind === 'node') { // 開那一步的抽屜
        ...
        state.permFlashId = el.dataset.id;
      ...
      state.reader = null; // US-103：離開 run 也收掉閱讀面板
    ...
    else if (act === 'open-settings') { if (await leaveGuard()) return; await openSettings(state.settings?.group ?? '個人與記憶'); }
    // 合併輪（US-105）：Workflow 頁。open-library 是舊連結的相容別名（指向新頁、選著同一條流程），不留死連結
    else if (act === 'open-wf-page' || act === 'open-library') { await openWfPage(); }
    else if (act === 'tree-toggle') { // 整棵樹收起／展開；記在本機，重開還在
      state.treeOff = !state.treeOff;
      treeStateSave();
      // US-107：只換樹與版位的 class（.wfpage 留著，欄寬才有過場；減少動畫偏好下 CSS 關掉過場）；找不到版位才整頁重繪
      const page = app.querySelector('.wfpage');
      const tree = page?.querySelector(':scope > .wftree');
      if (page && tree) { page.classList.toggle('treeoff', state.treeOff); tree.outerHTML = wfTreeHtml(); } else render();
      document.querySelector('[data-act="tree-toggle"]')?.focus();
    }
    else if (act === 'nav-toggle') { // US-107 ①：主導覽收成只剩圖示／展開；記在本機。只換側欄，#app 的 class 一切換欄寬就有過場
      state.navOff = !state.navOff;
      navStateSave();
      app.classList.toggle('navoff', state.navOff);
      const side = app.querySelector('.side');
      if (side) side.outerHTML = sideHtml(); else render();
      document.querySelector('[data-act="nav-toggle"]')?.focus();
    }
    else if (act === 'tree-retry') { await treeReload(); } // US-107 ③：清單讀不到→重新讀取
    else if (act === 'recent-retry') { await treeReload(); } // US-107 ③：執行狀態只讀回一部分→再讀一次（狀態跟著清單回來）
    else if (act === 'tree-search-clear') { // US-107 ④：搜尋落空→清除搜尋；分類的收合回到搜尋前（搜尋從沒改過分類收合的記錄）
      state.treeQ = '';
      const inp = document.getElementById('tree-search');
      if (inp) inp.value = '';
      const g = document.getElementById('tree-nodes');
      if (g) g.innerHTML = treeNodesHtml();
      const miss = document.getElementById('tree-search-empty');
      if (miss) miss.hidden = true;
      inp?.focus();
    }
    else if (act === 'open-run-reader') { // 「最近三趟」「看上次成品」「空狀態看成品」：走閱讀輪那個右側滑出面板
      const d = el.dataset;
      try { await openRunReader(d.cat, d.id, d.rid, { act, cat: d.cat, id: d.id, rid: d.rid, src: d.src }); } catch (err) { toast(err.message, 'bad'); }
    }
    else if (act === 'try-run') { // 空狀態「試跑」：打開那條流程並切到「本次資料」（填完就能按開始）
      if (await leaveGuard()) return;
      state.wfEmpty = false;
      if (await openWorkflow(el.dataset.cat, el.dataset.id)) state.flowTab = 'data';
      render();
    }
    ...
    // ===== 設定頁後四組：新流程的預設／執行與排程的開關即 PUT；連線、資料的動作走既有 API =====
      ...
      if (!await askBox('刪掉這個身分？認識卡都還在，只是這個封套沒了。')) return;
      ...
      if (!await askBox(`刪掉「${el.dataset.name}」？已經帶進步驟的不受影響。`)) return;
    ...
    // ===== （M2）：首次三題介紹、通知「不要記」=====
      ...
      if (!text) { toast('欄位是空的——先填內容再存成常用', 'bad'); return; }
      const name = await askInput('給這個常用內容取個名字', { placeholder: '例：資深客服主管', title: '存成常用片段' });
      if (!name?.trim()) return;
      ...
      if (!name) { toast('先在「常用片段⋯」裡選一個要刪的', 'bad'); return; }
      if (!await askBox(`把常用「${name}」刪掉？（已填進各步驟的內容不受影響）`)) return;
      ...
      if (!await askBox(`刪掉參考檔「${el.dataset.name}」？有步驟正在用的話會讀不到。`)) return;
      ...
      await canvasOp((def) => {
        ...
        if (kindOf(n) === 'task') {
          ...
          // 這步要用哪家（連線輪）：有動才寫，一寫就記成使用者改的
          const connEl = document.getElementById('cv-connectors');
          if (connEl && n.executor === 'ai' && connEl.value !== JSON.stringify(n.connectors ?? [])) {
            n.connectors = JSON.parse(connEl.value || '[]');
            n.connectors_set_by = 'user';
          }
        }
        if (kindOf(n) === 'branch') {
      ...
      // 只開選檔；選到檔才關頁（在 import-file 的 change 裡）——取消選檔＝留在原地
      document.getElementById('import-file').click();
    } else if (act === 'cancel-import') { // 「不匯入」＝回 Workflow 頁（庫頁退場後沒有第二個去處）
      state.importPreview = null;
      render();
    } else if (act === 'confirm-import') {
      if (await canvasLeaveBlocked()) return; // 匯入完會切到新流程——畫布未存檔先問
      ...
      if (!await askBox(`把「${state.wf.def.name}」移到垃圾桶？30 天內都能復原。`)) return;
    ...
    // ===== 多組織：側欄切換器＋設定頁組織管理。一律追加在鏈尾，不動上面任何一支 =====
      ...
      const { id } = el.dataset;
      if (id !== state.orgId && await leaveGuard()) return; // US-105：改到一半切組織先問（切組織會整頁重載）
      state.orgMenu = false;
      ...
      try {
        ...
        if (await askBox(`「${made.name}」建好了。要現在切過去嗎？`)) {
    ...
    toast(err.message, 'bad'); // 精簡: 動作層錯誤先用原生 alert，之後換 Toast
  }
});
...
document.getElementById('import-file').addEventListener('change', async (e) => {
  ...
  e.target.value = '';
  if (!file) return;
  closeCalendar(); // 匯入預覽畫面在儀表板／行事曆／分類頁／設定頁之下，開著時會看不到
  closeDash();
  closeCategory();
  closeSettings(); closeAssets();
...
async function noticePollGlobal() {
  try {
    if (!state.calendar && !state.dash) {
      ...
      const dots = () => JSON.stringify([state.treeLoad, state.workflows.map((w) => `${w.category}/${w.id}/${w.last_run?.run_id ?? ''}/${w.last_run?.status ?? ''}`), (state.recent ?? []).map((r) => `${r.category}/${r.id}/${r.status}`)]);
      const beforeDots = dots();
      applyNotices(await api('GET', '/api/notices'));
      if (onWfRoute()) { // 樹上的狀態點跟著新跑完的趟走（US-107：讀清單每筆的 last_run）；落地頁的兩列才要最近幾趟
        await refreshTreeQuiet();
        if (state.wfEmpty) await refreshRecent();
      }
      const changed = before !== state.notices.unread.length || beforeDots !== dots();
      if (changed && !isTyping() && !state.run) render();
    }
...
// ---------- 啟動 ----------
(async function init() {
  navStateLoad(); // US-107 ①：主導覽收起來沒（記在本機）
  await refreshHealth();
  await refreshCompanyName(); // 樹最上面那一列的組織名
  try { await refreshLibrary(); } catch { dsKeyHeld = dsKey; dsKey = null; } // US-107 ③：清單讀不到不再整站停擺——Workflow 頁出「清單讀不到＋重新讀取」；草稿暫存先不讀不存
  treeStateLoad(); // 樹收起來沒、哪些分類收著（記在本機，重開還在）
  dsLoad(); // 上次沒存完的草稿讀回來（側欄「草稿・還沒存」那列）；要在 refreshLibrary 之後，已經不在庫裡的那份才判得掉（清單讀不到時 dsKey 是 null＝不動）
  try {
    state.presets = await api('GET', '/api/presets');
  } catch { /* 常用庫讀不到就先空著 */ }
  try {
