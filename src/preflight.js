// preflight — 開跑前健檢：每一步「開跑時會拿到什麼」＋固定規則（不花 AI）。
...
import { predecessors, ancestorIds, layers } from './graph.js';
import { attName, attKey } from './shared.js';
import { CONNECT_LINK, labelOf, normalizeCache } from './connectors.js';
...
// 沿路全部內容來源（與 runner.upstreamText 同一套）：所有祖先裡的 task（AI／人做），最近在前；
...
// 每次上傳的檔由誰讀——沒有 AI 祖先的 AI 步驟（通常就是第一個 AI 步驟）；後面的步驟靠沿路全帶拿它的產出
// 任一條到的卡只收先到的那條線——只要有一條線（連同它的祖先）沒有 AI，就可能沒 AI 讀過檔，算「可能沒有 AI 祖先」。
...
export function inputSources(def) {
  ...
  for (const n of def.nodes) {
    ...
    // 任一條到＝直接接進來的線不一定都趕得上，照列但標出來；更早的祖先一定到，不標
    ...
    // 參考檔（三層共用檔）：流程層字串與組織／分類層 {scope,name} 都算「這一步有輸入」；規範類不走 attachments，健檢不受影響
...
// /api/preflight 傳給前端的輸入來源（不重複的寫法）——原本每步列全部祖先的整句，200 步 119 萬字元、隨步數平方成長。
...
// 【已知缺陷，不動，記在 STATE 列管】前五段（貼上／請填／填入／輸入／範例）是就有的
// 佔位文字的判斷（2026-09-22 修）：全部加上句首錨點。
// 舊規則的前五個詞（貼上／請填／填入／輸入／範例）任何位置命中就擋，沒有邊界——
// 「已貼上出貨憑證」「本月輸入項目共 12 筆」「以範例 A 版為準」這類真答案會被誤擋（實測 24/27），
// 而誤擋的代價是那個欄位永遠開不了跑，使用者只能改成假動作騙過它。
// 代價說清楚：句中的佔位（「把資料貼上這裡」）從此放掉——那種放掉只是內容不對，使用者自己看得出來；
// 誤擋是連跑都跑不起來。兩邊不對等，寧可漏擋。
const PLACEHOLDER_RE = /^\s*(請填|請?(貼上|填入|填寫|輸入)|範例)|^\s*此處[填放寫貼]|^\s*例[：:]|^\s*待填\s*$|^\s*[xXｘＸ]{3,}\s*$|^\s*[（(\[]/;

// 勾了服務的 AI 步驟（human／分岔上的 connectors 不算——它們不會去讀）
const connectorNodes = (def) => def.nodes.filter((n) => nodeKind(n) === 'task' && n.executor === 'ai' && Array.isArray(n.connectors) && n.connectors.length);
export const usesConnectors = (def) => connectorNodes(def).length > 0;

// 第幾步（1 起）：task 依拓撲深度、同深度照定義順序——給「這一趟會讀你的 X（第 N 步）」那一行
function stepIndex(def) {
  const depth = layers(def);
  const at = new Map(def.nodes.map((n, i) => [n.id, i]));
  const tasks = def.nodes.filter((n) => nodeKind(n) === 'task')
    .sort((a, b) => depth.get(a.id) - depth.get(b.id) || at.get(a.id) - at.get(b.id));
  return new Map(tasks.map((n, i) => [n.id, i + 1]));
}

// 連線兩條擋（ADR-006／US-102）＋這趟會讀哪些。cache＝undefined 表示呼叫端沒給清單（不查可用性），null／物件＝要查
function connectorRules(def, cache) {
  const issues = [];
  const nodes = connectorNodes(def);
  if (!nodes.length) return { issues, connector_reads: [] };
  const idx = stepIndex(def);
  const connector_reads = nodes.map((n) => ({ node: n.id, index: idx.get(n.id) ?? null, labels: [...new Set(n.connectors)].map(labelOf) }));
  // (b) connector-permission：流程的連線開關關著（匯入的一律關）卻有步驟要讀服務
  if (def.permissions?.connectors === false) {
    const connectors = [...new Set(nodes.flatMap((n) => n.connectors))];
    issues.push({
      level: 'block', code: 'connector-permission', connectors, nodes: nodes.map((n) => n.id),
      title: `這條 Workflow 要讀你的 ${connectors.map(labelOf).join('、')}，但連線權限沒開`,
      detail: '匯入的 Workflow 連線權限一律先關著。確認它要讀的服務沒問題，再打開 Workflow 頁的「允許讀已連上的服務」後按開始。',
      fix: { kind: 'flow', id: 'permissions.connectors' },
    });
  }
  // (a) connector-unavailable：步驟勾的服務不在清單上、或不是已連上
  if (cache !== undefined) {
    const norm = normalizeCache(cache);
    const servers = norm.servers;
    for (const n of nodes) {
      for (const name of [...new Set(n.connectors)]) {
        const s = servers.find((x) => x.name === name);
        // unchecked＝從沒抓過清單（或這次抓失敗）；missing＝抓過但清單上沒有它
        const status = s ? s.status : norm.checked_at ? 'missing' : 'unchecked';
        if (status === 'connected') continue;
        const label = labelOf(name);
        issues.push({
          level: 'block', code: 'connector-unavailable', node: n.id, connector: name, label, status, link: CONNECT_LINK,
          title: `「${n.title}」要讀的 ${label} ${status === 'needs-auth' ? '還沒授權' : status === 'missing' ? '不在你 Claude 已連的服務裡' : status === 'unchecked' ? '還沒檢查過連上了沒' : '現在連不上'}`,
          detail: status === 'unchecked'
            ? `剝繭還不知道你的 Claude 連了哪些服務。到設定→連線按「重新檢查」，確認 ${label} 已連上再按開始。`
            : status === 'missing'
            ? `到 claude.ai 把 ${label} 連上後回設定→連線按「重新檢查」；或改這一步，不讀 ${label}。`
            : `到 claude.ai 的連接器設定${status === 'needs-auth' ? '完成授權' : '重新連線'}，回設定→連線按「重新檢查」再按開始；或改這一步，不讀 ${label}。`,
          fix: { kind: 'node', id: n.id },
        });
      }
    }
  }
  return { issues, connector_reads };
}

// 固定規則 → { issues, inputs, unused_params, connector_reads }；values＝這次的值（開跑前才給；抽屜預覽不給就不查欄位值）
// opts.connectors＝已連服務清單快取（伺服器給；沒給＝不查服務可用性，連線權限那條照查）
export function preflight(def, values = null, opts = {}) {
  const byId = new Map(def.nodes.map((n) => [n.id, n]));
  ...
  // R5 file-permission（擋，步驟要產 Word／Excel 真檔，但這條流程沒開產檔權限
  ...
  // R7 連線：服務不可用／權限沒開＝擋；順帶算「這趟會讀哪些」
  const conn = connectorRules(def, opts?.connectors);
  issues.push(...conn.issues);
  ...
  if (values) {
    ...
    for (const p of def.params ?? []) {
      // R6 upload-missing（擋，必填的上傳欄位這一趟沒有檔（values 裡是上傳檔名，沒傳＝空）
      if (p.input === 'file') {
  ...
  return { issues, inputs, unused_params, connector_reads: conn.connector_reads };
}
