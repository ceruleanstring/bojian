...
import { createStore, StoreError, deepMerge, DEFAULT_SETTINGS, safeFileName, dedupeFileName } from './store.js';
...
// 一次呼叫吃進去的 token：快取建立與快取讀取也算輸入（帳本三個欄位加總）。
// 這條公式原本在 byRun／usageByNode 各寫一次，改一邊忘一邊就會對不起來
const tokenInput = (u) => (u?.input_tokens ?? 0) + (u?.cache_creation_input_tokens ?? 0) + (u?.cache_read_input_tokens ?? 0);
...
export function createApp({ dataDir, adapter, uiDir = path.join(HERE, '..', 'ui'), examplesDir = path.join(HERE, '..', 'examples'), now = () => Date.now(), autostartDir, fetchConnectors = () => fetchHostConnectors({ now }) } = {}) {
  ...
  function makeOrg(id, dir) {
    ...
    function composeContext(body) {
      ...
      try {
        ...
        let conn = [];
        try { conn = connectedServers(store.readConnectors()).map(({ name, label, status }) => ({ name, label, status })); } catch { /* 當沒有 */ }
        ctx.capabilities = { web: settings.exec?.web !== false, files_default: settings.defaults?.permissions_files !== false, office: ['docx', 'xlsx', 'pptx', 'pdf'], downgrade: [], refs: true, connectors: conn };
    ...
    // 通知動作進行中的一次性認領（B04，Codex 2026-09-25）：同一則通知的動作有 await（讀 body、重抓快照）時，
    // 第二發在第一發落地前進來也只做一次；做完（成功或失敗）就放掉，失敗的還能重試
    const noticeActsInFlight = new Set();
    ...
    // 公司檔＝全部流程都看，分類檔＝只看該分類；有變的流程升一版（履歷寫原因），回被清掉的步驟數。壞檔跳過不擋刪除。
    ...
    return {
      id, store, memory, runner, optimizer, scheduler, kick, logPair, adapter: orgAdapter, noticeActsInFlight,
  ...
  const orgs = {
    ...
    remove(id) {
      ...
      const trash = path.join(dataDir, ORGS_TRASH, `${id}-${stamp}`);
  ...
  // 連線清單（ADR-006）：宿主連了哪些服務是整台電腦共用的，快取在資料根；同時按兩次「重新檢查」只起一個子行程
  let connInflight = null;
  function refreshConnectors(store) {
    if (!connInflight) {
      connInflight = (async () => {
        const cache = await fetchConnectors();
        store.writeConnectors(cache);
        return cache;
      })().finally(() => { connInflight = null; });
    }
    return connInflight;
  }
  // 健檢要的清單：從沒抓過、而且這條定義真的有步驟勾了服務，才自動抓一次（只這種情況）；抓失敗＝照沒抓過健檢（會擋）
  async function connectorCacheFor(store, def) {
    let cache = null;
    try { cache = store.readConnectors(); } catch { /* 當沒抓過 */ }
    if (usesConnectors(def) && !normalizeCache(cache).checked_at) {
      try { cache = await refreshConnectors(store); } catch (e) { console.error('[bojian] 連線清單抓不到，健檢照沒檢查過處理：', e.message); }
    }
    return cache;
  }
  ...
  async function handleApi(req, res, segs) {
    ...
    const { store, memory, runner, optimizer, scheduler, kick, logPair, adapter, noticeActsInFlight,
    ...
    try {
      ...
      // /api/connectors：使用者 Claude 已連的外部服務（快取；從沒抓過＝{checked_at:null, servers:[]}）
      if (segs[1] === 'connectors' && req.method === 'GET' && segs.length === 2) {
        let cache = null;
        try { cache = store.readConnectors(); } catch { /* 當沒抓過 */ }
        return json(200, normalizeCache(cache));
      }
      // POST /api/connectors/refresh：重新檢查（起子行程讀宿主啟動訊息，讀到就收手，幾秒到二十秒）
      if (segs[1] === 'connectors' && segs[2] === 'refresh' && req.method === 'POST' && segs.length === 3) {
        try {
          return json(200, normalizeCache(await refreshConnectors(store)));
        } catch (e) {
          return json(502, { error: e?.message || '這次沒抓到你 Claude 連了哪些服務——稍後再按「重新檢查」。' });
        }
      }
      // /api/compose 用講的建流程
      ...
      if (segs[1] === 'preflight' && req.method === 'POST' && segs.length === 2) {
        ...
        const connectors = await connectorCacheFor(store, body.def);
        return json(200, { ...preflight(body.def, body.values ?? null, { connectors }), inputs: packInputs(body.def) });
      }
      ...
      if (segs[1] === 'import' && req.method === 'POST' && segs.length === 3) {
        ...
        if (segs[2] === 'confirm') {
          ...
          // 連線權限同樣一律關（ADR-006 第 6 點），要用自己到 Workflow 頁打開
          body.def = { ...body.def, permissions: { ...(body.def?.permissions ?? {}), files: false, connectors: false } };
          validateWorkflow(body.def, { allowFloating: true });
      ...
      if (segs[1] === 'settings' && segs.length === 2) {
        ...
        if (req.method === 'GET') return json(200, { ...store.readSettings(), data_dir: store.dataDir, app_version: PKG_VERSION });
        if (req.method === 'PUT') {
      ...
      if (segs[1] === 'notices') {
        ...
        if (req.method === 'POST' && segs[3] === 'act' && segs.length === 4) {
          ...
          if (!store.readNotices().some((x) => x.id === nid)) return json(404, { error: '這則通知不存在' });
          const { action } = await readBody(req);
          // 已處理的不再處理（B04，Codex 2026-09-25）：重複點、重試 HTTP、兩個分頁都不能第二次開跑。
          // 讀完 body 才重讀狀態（另一發可能在這段 await 裡已經做完）；回 409 帶 already_done，第一次的留痕不動
          const alreadyDone = () => json(409, { error: '這則通知已經處理過了，不再重複執行', already_done: true });
          if (noticeActsInFlight.has(nid)) return alreadyDone();
          const n = store.readNotices().find((x) => x.id === nid);
          if (!n) return json(404, { error: '這則通知不存在' });
          if (n.status === 'done') return alreadyDone();
          noticeActsInFlight.add(nid);
          try {
            const schedOf = () => {
            ...
            return json(200, { ok: true });
          } finally { noticeActsInFlight.delete(nid); }
        }
      }
      ...
      if (segs.length === 2) {
        ...
        // US-107：每筆多回 last_run（這條自己的最近一趟），樹上狀態點讀它——既有欄位不動
        if (req.method === 'GET') return json(200, store.listWorkflows().map((w) => ({ ...w, last_run: store.lastRunOf(w.category, w.id) })));
      }
      ...
      if (segs.length === 4) {
        ...
        if (req.method === 'PUT') {
          ...
          // 權限（產檔／讀服務）不准被「漏寫」或「AI 自己寫」打開（連線輪覆核）：聊天改流程＝AI 回的整份 YAML，
          // 權限一律沿用存檔前的值；其他存法（開關、畫布、抽屜）沒帶的欄位沿用舊值，帶了才照存
          if (body.def && typeof body.def === 'object' && current.permissions && typeof current.permissions === 'object') {
            const given = body.from === 'chat' ? {} : (body.def.permissions && typeof body.def.permissions === 'object' ? body.def.permissions : {});
            body.def.permissions = { ...current.permissions, ...given };
          }
      ...
      if (segs[4] === 'files') {
        ...
        // 下載一份參考檔：上傳完看不到也拿不回來的話，等於丟進黑洞（只有 AI 讀得到）
        if (req.method === 'GET' && segs.length === 6) {
          const name = decodeURIComponent(segs[5]);
          const fp = store.refFilePath(category, id, name);
          if (!fp) return json(404, { error: '找不到這個檔案' });
        ...
        if (req.method === 'DELETE' && segs.length === 6) {
          const gone = decodeURIComponent(segs[5]);
          store.deleteRefFile(category, id, gone);
          // 勾選要跟著清：別的步驟還掛著這個檔名的話，開跑時那一步會讀不到。
          // 只清流程層（字串形式）的；{scope} 那種是共用檔，不歸這條路刪。
          let cleared = 0;
          try {
            const def = store.readWorkflow(category, id);
            for (const n of def.nodes ?? []) {
              const before = (n.attachments ?? []).length;
              if (!before) continue;
              n.attachments = n.attachments.filter((a) => !(typeof a === 'string' && a === gone));
              cleared += before - n.attachments.length;
              if (!n.attachments.length) delete n.attachments;
            }
            if (cleared) store.writeWorkflow(category, id, def);
          } catch { /* 定義讀不到就只刪檔——檔已經不在了，不能因為清不了勾選就回錯 */ }
          return json(200, { ok: true, files: store.listRefFiles(category, id), cleared });
        }
      }
      ...
      if (segs[4] === 'runs') {
        ...
        if (segs.length === 5 && req.method === 'POST') {
          ...
          for (const [k, one] of Object.entries(uploads)) {
            if (k !== RUN_ATTACH_KEY && !fileKeys.has(k)) return json(400, { error: `「${k}」不是上傳欄位` });
            // 本次附件可以好幾個（它不綁欄位）；欄位型的上傳一格仍舊只收一個檔
            const tokens = Array.isArray(one) ? one : [one];
            if (k !== RUN_ATTACH_KEY && tokens.length > 1) return json(400, { error: `「${k}」一次只能一個檔` });
            for (const token of tokens) {
              const up = store.readUpload(token, { category, id }); // 別條 Workflow 發的代碼拿不到（移分類／改名時記號已跟著改，L13b）
              if (!up) return json(400, { error: store.uploadProblem(token, { category, id }) });
              // 同一趟兩份同名（或只差大小寫）的檔不互蓋（B02，Codex 2026-09-25）：開跑前先算好各自的落盤名（-2、-3），
              // run.params／run.run_files 記的就是實際檔名，之後 claimUpload 照這個名字搬
              const name = dedupeFileName(up.name, claimedTokens.map((c) => c.name));
              if (k === RUN_ATTACH_KEY) attachNames.push(name);
              else overrides[k] = name;
              claimedTokens.push({ token, name });
            }
          }
          ...
          const pf = preflight(defForRun, values, { connectors: await connectorCacheFor(store, defForRun) });
        ...
        // 路由放在 /runs/:rid 之前；run id 一律 r- 開頭，不會撞名
        if (segs.length === 6 && segs[5] === 'stats' && req.method === 'GET') {
          const all = [];
          for (const rid of store.listRuns(category, id)) {
        ...
        // 總覽規格單（US-104）沒有專屬路由：跑完那一刻就算好存進 run.overview，GET /runs/:rid 本來就整份帶回去，
        // 前端直接讀那一欄（只讀不重算）。多開一支只給它用的 GET 沒有呼叫端，所以不留
        ...
        if (req.method === 'POST' && action && segs.length === 7) {
          ...
          // 卡住的那一趟手動接上：停點改完、擬規則跑到一半程式被關掉，resume 沒跑到，
          // 那一趟就永久停著而且畫面上沒有任何卡片可按。放行條件由 runner 判（真的沒人在等才放）
          else if (action === 'resume-stuck') {
            if (!runner.resumeStuck(category, id, runId)) return json(409, { error: '這一趟還有步驟在等你處理，先處理那一步' });
          }
