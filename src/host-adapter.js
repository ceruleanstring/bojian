...
// 逾時後等子行程樹結束的上限（A1，Codex 2026-09-25 審查）：taskkill /T /F 對整棵樹是強制終止，正常一秒內就結束；
// 5 秒還等不到 close＝行程卡在核心（磁碟 I/O、防毒掃描）或 taskkill 沒權限——再等下去只是拖住使用者，
// 所以照樣回報 TIMEOUT，但在錯誤物件上標 stopConfirmed=false／detail「可能還沒停乾淨」，讓上層自己決定要不要重試。
export const KILL_WAIT_MS = 5_000;
const TIMEOUT_MSG = '這一步等太久沒有回應，已先停下。可以按「重試這步」再來一次。';

// killTreeOpts／killWaitMs 只為回歸測試能注入假的 spawn／平台／短上限（不殺真行程）；預設＝真 taskkill＋本機平台＋5 秒。
export function createHostAdapter({ timeoutMs = 300_000, spawnFn = defaultSpawn, lean, killTreeOpts = {}, killWaitMs = KILL_WAIT_MS } = {}) {
  // 輕裝與否開工時定一次
  ...
  function collect(child, timeout, onDone, onFail) {
    ...
    let err = '';
    let settled = false;
    // 逾時（A1）：以前只 child.kill()——Windows 正式 spawn 是 cmd /c claude，殺掉的是 cmd，底下的 claude 變孤兒繼續跑、
    // 繼續花額度、還可能繼續寫這趟的檔；runner 隨即重試＝兩份同節點呼叫互踩。改成：走 killTree 殺整棵，
    // 並等子行程真的 exit／close 才回報，讓上層重試時舊的一定已經不在；等不到就到 killWaitMs 上限照報但標未確認。
    // settled 先立起來：逾時之後遲來的 stdout／close 0 只會走到下面的 settle 被擋掉，不會再觸發 onDone。
    const timer = timeout
      ? setTimeout(() => {
          if (settled) return;
          settled = true;
          ...
          const report = (confirmed) => {
            ...
            const e = new HostError(TIMEOUT_MSG, 'TIMEOUT');
            e.stopConfirmed = confirmed;
            if (!confirmed) e.detail = `子行程可能還沒停乾淨（已下令終止，但 ${killWaitMs} 毫秒內沒等到它結束）`;
            onFail(e);
          };
          // 先掛監聽再殺：假子行程可能在 kill() 裡同步發 close
          child.once('exit', () => report(true));
          child.once('close', () => report(true));
          waitTimer = setTimeout(() => report(false), killWaitMs);
          try { killTree(child, killTreeOpts); } catch { /* 盡力；等不到就由上限回報 */ }
        }, timeout)
      : null;
    const settle = (fn, v) => {
      if (settled) return;
