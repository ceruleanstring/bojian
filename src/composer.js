...
9. 產檔：使用者要「做成 Word／Excel／簡報交出去」時，該步 output_file 用 docx、xlsx 或 pptx（真檔，由工人在產出資料夾用內建套件做；需流程開產檔權限，你不用管權限）；pdf 目前會降級成 .md，除非使用者明說否則不用。成品是簡報時，output_length 用張數（例「12 張」）不要用頁數。產檔步驟的 instruction 要寫明檔案裡放什麼（章節／表頭／份量）與檔名用途；使用者提到有公司範本／設計規範（Word 範本、Excel 範本、色票字型）時，在該步 template_file 指定範本檔名並在 instruction 說明「套用範本、只填內容不改版型」。
...
// 能耐表五行固定字——伺服器只給布林／清單，字在這裡組；office／downgrade／refs／connectors 目前都是固定值，先不接字
...
function contextSections(ctx) {
  ...
  // 三層共用檔：公司／部門規範整份給拆解器當已知條件（跟第 12 條的分類守則同一待遇）；外圈在前
