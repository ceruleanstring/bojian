...
export function createScheduler({ store, runner, kick = () => {}, now = () => Date.now() }) {
  ...
  function fire(sched, { makeup = false } = {}) {
    ...
    // 服務授權也要查（連線輪 US-102）：只讀快取、不在排程裡起子行程重抓——沒抓過就照「還沒檢查」擋，通知叫你去檢查一次
    let connectors = null;
    try { connectors = store.readConnectors?.() ?? null; } catch { /* 讀不到當沒抓過 */ }
    const blocks = preflight(def, values, { connectors }).issues.filter((i) => i.level === 'block');
    if (blocks.length) {
