...
test('L018：同一句話只記一次——拆解器第二趟重跑不會再記一組意思一樣的卡', () => {
  ...
  // 實作層級：記過就不再記、路由成功之後才登記、檔案不會無限長大
  const src = fs.readFileSync(new URL('../src/memory.js', import.meta.url), 'utf8');
  assert.ok(src.includes('if (fp && seenChat(fp)) return null;'), '記過就不再記');
  assert.ok(src.includes('markChat(fp)'), '有登記這一步');
  const i1 = src.indexOf('const notices = await routeAndLand({');
  const i2 = src.indexOf('markChat(fp)');
  assert.ok(i1 > 0 && i2 > i1, '路由成功之後才登記——中途失敗下次還要有機會記');
