...
test('監工：三份固定交代逐字（改動＝改契約，測試要一起改）', () => {
  assert.equal(BRIEF_RULES, '你是「剝繭」流程的監工。這趟流程要開跑了，你先通盤看一次這條流程的全部設定與這次帶進來的資料，寫一份給每一步工人的開場備註：這趟的資料長什麼樣（件數、類別、期間、缺什麼）、哪幾步要特別注意什麼、成品一定要有什麼。只寫從設定與資料看得出來的事，不編、不加新要求、不改任何步驟的指示；最多八條，一條一句。你只交接、不設限：不要寫「不得」「不准」「只能」「禁止」這類限制句，工人該守的規則由設定與使用者的要求管，不由你管。全篇使用與流程相同的語言。只輸出一個 JSON 物件，前後不要任何其他文字：{"note":"…"}');
...
test('開場 prompt：固定交代＋步驟一行＋欄位值（超過 2,000 字截）＋參考檔只列名稱／掛在哪步／行數／欄位，不附內文（L068：監工讀截斷節錄就寫收窄規定）', () => {
  ...
  assert.equal(section(p, '參考檔'), '【參考檔：規格.md】（掛在：寫初稿）｜行數 1', '純文字檔：名稱、掛在哪步、行數，不印內文');
  assert.ok(!p.includes('yyyyy'), '參考檔內文一個字都不附');
});

test('開場 prompt：參考檔行數以換行計（最後一行沒換行也算）；csv／tsv 或第一行有 tab 才印「欄位：第一行」，第一行只是含逗號的散文不印；掛在多步用「、」分隔', () => {
  const def = structuredClone(DEF);
  def.nodes[0].attachments = ['名單.csv', '說明.md', '備忘.md'];
  def.nodes[2].attachments = ['名單.csv', '表.txt'];
  const p = buildBriefPrompt({
    def,
    params: {},
    paramLabels: {},
    refTexts: {
      '名單.csv': '姓名,金額,日期\n甲,100,2026-01-01\n乙,200,2026-01-02\n',
      '說明.md': '第一行\n第二行\n第三行',
      '備忘.md': '先看北區,再看南區,最後看總表\n第二行',
      '表.txt': 'a\tb\tc\n1\t2\t3',
      '規格.md': '',
    },
  });
  const refs = section(p, '參考檔').split('\n\n');
  assert.ok(refs.includes('【參考檔：名單.csv】（掛在：看資料、收尾）｜行數 3｜欄位：姓名,金額,日期'), `csv：掛兩步、末尾換行不多算一行、印欄位：${refs}`);
  assert.ok(refs.includes('【參考檔：說明.md】（掛在：看資料）｜行數 3'), `md：最後一行沒換行也算一行、不印欄位：${refs}`);
  assert.ok(refs.includes('【參考檔：備忘.md】（掛在：看資料）｜行數 2'), `md 第一行含逗號的散文不是表格：只有名稱與行數：${refs}`);
  assert.ok(!p.includes('先看北區'), '散文第一行不印成欄位');
  assert.ok(refs.includes('【參考檔：表.txt】（掛在：收尾）｜行數 2｜欄位：a\tb\tc'), `第一行有 tab 就印欄位：${refs}`);
  assert.ok(refs.includes('【參考檔：規格.md】'), '空字串＝讀不出文字，只印名稱');
  for (const w of ['甲,100', '第二行', '1\t2\t3']) assert.ok(!p.includes(w), `不附內文：${w}`);
});
...
test('交接 prompt：沒判路題不出現該段；三個勾全關時允許清單寫（無）；空的段落一律（無）；上一步成品超過 8,000 字整段換成一句、不截', () => {
  ...
  assert.equal(section(p, '上一步的成品'), '【步驟：看資料】（成品 8500 字，太長未附；下一步工人會拿到全文）', '超長成品不截、換成一句');
  assert.ok(!p.includes('zzzzz') && !p.includes('（截）'), '超長成品一個字都不附');
  // 剛好 8,000 字不算超過，逐字照舊
  const exact = buildHandoffPrompt({ node, upstream: [{ title: '看資料', text: 'z'.repeat(8000) }] });
  assert.ok(exact.includes(`【步驟：看資料】\n${'z'.repeat(8000)}`), '8,000 字整段逐字附');
});
...
test('開場呼叫：回合法 JSON→ok；卷宗兩份各寫一次；meta 帶 kind supervisor', async () => {
  ...
  assert.deepEqual(r, { ok: true, text: '先數件數', dropped: [] });
...
test('交接呼叫：沒勾的欄位程式強制 null；勾了才吃監工給的值；route 照收', async () => {
  ...
  assert.deepEqual(off, { ok: true, text: '按三類分節', tier: null, web: null, route: '2', dropped: [] });
  const on = await handoff({ ...args, flags: { note: true, tier: true, tools: true } });
  assert.deepEqual(on, { ok: true, text: '按三類分節', tier: 'deep', web: true, route: '2', dropped: [] });
});

// ---- 監工只交接、不設限：程式再剔一次限制句（L068：監工讀了截斷節錄寫「只能用上一步的數字」，工人照做少了客單價） ----

const LIMIT_ARGS = (reply) => ({
  adapter: fakeAdapter(reply),
  node: { id: 'b', title: '寫初稿', instruction: '寫初稿' },
  routeOptions: [], brief: '', priorHandoffs: [], rules: [], interjections: [], upstream: [],
  flags: { note: true, tier: false, tools: false },
});

test('剔限制句：單條——含「只能」的那條整條剔掉，其餘接回去；dropped 記原句', async () => {
  const r = await handoff(LIMIT_ARGS('{"note":"上一步共 14 件；只能用上一步的數字；按三類分節"}'));
  assert.deepEqual(r, { ok: true, text: '上一步共 14 件；按三類分節', tier: null, web: null, route: null, dropped: ['只能用上一步的數字'] });
});

test('剔限制句：多條——換行與「；」都當分隔；不得／不准／不可以／只能／禁止五個字各剔一條；開場也剔', async () => {
  const note = '1. 資料兩個月\n2. 不得新增建議\n3. 不准改欄位名；4. 不可以省略北區\n5. 成品要有客單價\n6. 禁止用去年資料；7. 只能寫三段';
  const r = await brief({ ...LIMIT_ARGS(JSON.stringify({ note })), def: DEF, params: {}, paramLabels: {}, refTexts: {} });
  assert.equal(r.ok, true);
  assert.equal(r.text, '1. 資料兩個月\n5. 成品要有客單價');
  assert.deepEqual(r.dropped, ['2. 不得新增建議', '3. 不准改欄位名', '4. 不可以省略北區', '6. 禁止用去年資料', '7. 只能寫三段']);
});

// L084①：規則寫的是「不可」，詞表原本只有「不可以」——「不可新增欄位」剔不掉
test('剔限制句：「不可」也剔（規則寫的是「不可」，不只「不可以」）——「不可新增欄位」整條進 dropped', async () => {
  const note = '1. 資料兩個月\n2. 不可新增欄位；3. 按三類分節\n4. 不可以省略北區';
  const r = await brief({ ...LIMIT_ARGS(JSON.stringify({ note })), def: DEF, params: {}, paramLabels: {}, refTexts: {} });
  assert.equal(r.ok, true);
  assert.equal(r.text, '1. 資料兩個月\n3. 按三類分節');
  assert.deepEqual(r.dropped, ['2. 不可新增欄位', '4. 不可以省略北區']);
});

test('剔限制句：全剔光＝note 空字串，照舊 ok:true；dropped 留全部', async () => {
  const r = await handoff(LIMIT_ARGS('{"note":"不得新增建議；只能用上一步的數字","route":null}'));
  assert.equal(r.ok, true);
  assert.equal(r.text, '');
  assert.deepEqual(r.dropped, ['不得新增建議', '只能用上一步的數字']);
});

test('剔限制句：沒有限制句＝逐字不變（含尾端分隔）；只比對中文關鍵字，英文 must not 不動；沒勾寫備註時 text 空、dropped 空', async () => {
  const note = '上一步共 14 件；must not skip 北區；\n按三類分節\n';
  const same = await handoff(LIMIT_ARGS(JSON.stringify({ note })));
  assert.equal(same.text, note.trim(), '沒剔就是解析後原樣（parseHandoff 本來就 trim）');
  assert.deepEqual(same.dropped, []);
  const off = await handoff({ ...LIMIT_ARGS('{"note":"只能寫三段；按三類分節"}'), flags: { note: false, tier: false, tools: false } });
...
test('U1b ⑥：開場 prompt attachments 混型——{scope,name} 顯示「名稱（公司）」「名稱（分類）」並以此當 refTexts 的鍵；不炸、無 [object Object]', () => {
  ...
  assert.ok(p.includes('【參考檔：規格.md】（掛在：寫初稿）｜行數 1'));
  assert.ok(p.includes('【參考檔：範本.md（組織）】（掛在：寫初稿）｜行數 1'), p);
  assert.ok(!p.includes('公司範本內容'), '共用層的內文也不附');
