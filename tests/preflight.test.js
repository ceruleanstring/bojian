...
test('必填欄位的預設值本來就是答案：不准因為「值等於預設」就永遠擋著不給開跑', () => {
  ...
  // 前五個詞本來沒有邊界，任何位置命中就擋。這些是實測被誤擋的真答案。
  for (const dft of [
    '已貼上出貨憑證', '本月輸入項目共 12 筆', '以範例 A 版為準',
    '資料已填入系統', '客戶填寫完畢', '照上季範例做', '輸出入明細表',
  ]) {
    assert.deepEqual(codes(mk(dft), { len: dft }), [], `真答案被誤擋（句中命中）：${dft}`);
  }
  // 句首的照舊要擋
  for (const dft of ['請填這一格', '貼上你的名單', '輸入月份', '填寫收件人', '範例：春季新品']) {
    assert.deepEqual(codes(mk(dft), { len: dft }), ['len'], `句首的佔位沒擋到：${dft}`);
  }
});

// ---- 連線輪（US-102）：服務不可用／權限沒開＝擋；connector_reads＝開跑前那一行 ----
const CONN_CACHE = {
  checked_at: '2026-09-22T10:00:00.000Z',
  servers: [
    { name: 'claude.ai Gmail', label: 'Gmail', status: 'connected', source: 'claudeai', summary: '', read_tools: [], blocked_tools: [] },
    { name: 'claude.ai Google Drive', label: 'Google Drive', status: 'connected', source: 'claudeai', summary: '', read_tools: [], blocked_tools: [] },
    { name: 'claude.ai Notion', label: 'Notion', status: 'needs-auth', source: 'claudeai', summary: '', read_tools: [], blocked_tools: [] },
  ],
};
const connDef = (perm, nodes) => ({ format: 1, name: '讀信', params: [], ...(perm === undefined ? {} : { permissions: perm }), nodes: nodes ?? [
  { id: 'mail', title: '讀來信', executor: 'ai', stop_point: 'never', instruction: '讀近 7 天來信', next: ['drive'], connectors: ['claude.ai Gmail'] },
  { id: 'drive', title: '找附件', executor: 'ai', stop_point: 'never', instruction: '依上一步找檔', next: ['who'], connectors: ['claude.ai Google Drive', 'claude.ai Gmail'] },
  { id: 'who', title: '寄出', executor: 'human', stop_point: 'never', instruction: '自己寄', next: [], connectors: ['claude.ai Notion'] },
] });

test('連線：服務都已連上＝不擋；connector_reads 列每個勾了服務的 AI 步驟（第幾步＋服務短名），人做步驟不算', () => {
  const r = preflight(connDef(), null, { connectors: CONN_CACHE });
  assert.equal(r.issues.filter((i) => i.level === 'block').length, 0, JSON.stringify(r.issues));
  assert.deepEqual(r.connector_reads, [
    { node: 'mail', index: 1, labels: ['Gmail'] },
    { node: 'drive', index: 2, labels: ['Google Drive', 'Gmail'] },
  ]);
  assert.deepEqual(preflight(connDef(undefined, [{ id: 'a', title: 'A', executor: 'ai', stop_point: 'never', instruction: '寫一首詩', next: [] }])).connector_reads, [], '沒勾＝空陣列');
});

test('連線 (a) connector-unavailable：勾的服務還沒授權／不在清單／從沒檢查過＝擋，帶 node、connector、status、link；沒給清單＝不查這條', () => {
  const def = connDef(undefined, [
    { id: 'n1', title: '讀筆記', executor: 'ai', stop_point: 'never', instruction: '讀 Notion 筆記', next: ['n2'], connectors: ['claude.ai Notion'] },
    { id: 'n2', title: '讀 CRM', executor: 'ai', stop_point: 'never', instruction: '依上一步查客戶', next: [], connectors: ['my-crm'] },
  ]);
  const r = preflight(def, null, { connectors: CONN_CACHE });
  const un = r.issues.filter((i) => i.code === 'connector-unavailable');
  assert.equal(un.length, 2);
  assert.deepEqual({ ...un[0], title: undefined, detail: undefined }, {
    level: 'block', code: 'connector-unavailable', node: 'n1', connector: 'claude.ai Notion', label: 'Notion', status: 'needs-auth',
    link: 'https://claude.ai/settings/connectors', fix: { kind: 'node', id: 'n1' }, title: undefined, detail: undefined,
  });
  assert.match(un[0].title, /讀筆記.*Notion.*還沒授權/);
  assert.equal(un[1].status, 'missing');
  assert.equal(preflight(def, null, { connectors: null }).issues.find((i) => i.code === 'connector-unavailable').status, 'unchecked', '從沒抓過');
  assert.equal(preflight(def).issues.filter((i) => i.code === 'connector-unavailable').length, 0, '呼叫端沒給清單＝不查可用性');
});

test('連線 (b) connector-permission：流程連線權限關著（匯入的）卻有步驟勾了服務＝擋一條，帶 connector 清單、指到流程開關；開著或沒寫＝不擋', () => {
  const r = preflight(connDef({ files: false, connectors: false }), null, { connectors: CONN_CACHE });
  const perm = r.issues.filter((i) => i.code === 'connector-permission');
  assert.equal(perm.length, 1);
  assert.equal(perm[0].level, 'block');
  assert.deepEqual(perm[0].connectors, ['claude.ai Gmail', 'claude.ai Google Drive'], '只算 AI 步驟、去重');
  assert.deepEqual(perm[0].fix, { kind: 'flow', id: 'permissions.connectors' });
  assert.equal(preflight(connDef({ connectors: true }), null, { connectors: CONN_CACHE }).issues.filter((i) => i.code === 'connector-permission').length, 0);
  assert.equal(preflight(connDef(), null, { connectors: CONN_CACHE }).issues.filter((i) => i.code === 'connector-permission').length, 0, '舊流程沒寫＝開');
  assert.equal(preflight(connDef({ connectors: false })).issues.filter((i) => i.code === 'connector-permission').length, 1, '沒給清單也照查權限');
});
