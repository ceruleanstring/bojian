...
// pass＝13 項關鍵數字全中、冤枉（兩種加總）≤ 1、且要人出手 0 次（使用者 09-24 裁定不停下叫人）。假話反向計分沿用 q1 的 forbidden。
...
export async function score({ finalText, steps = {}, truth, drive = null }) {
  ...
  return {
    pass: m.misses.length === 0 && wrongfulTotal <= 1 && humanTouches === 0, // 關鍵數字全中、冤枉（含 missing 錯判）≤1、沒停下叫人
    metrics: {
