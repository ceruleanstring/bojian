...
try {
  ...
  const s1 = await q10.score({ arm: 'bojian', finalText: good, steps: steps1, truth: T, artifacts: [], prompts: [], runStatus: 'done' }); // 2026-09-25 E2：pass 要 runStatus done＋三步都有查核紀錄
  ...
  const cleanSteps = { numbers: { status: 'done', output: 'x', check: passCheck }, points: { status: 'done', output: 'y', check: passCheck }, report: { status: 'done', output: good, check: passCheck } };
  const s6f = await q10.score({ finalText: good, steps: cleanSteps, truth: T, drive: { interventions: [] }, runStatus: 'done' });
  check(s6f.pass === true && s6f.metrics.interventions === 0 && s6f.metrics.stops === 0, '乾淨趟（done、三步都有查核）：0 停、0 冤枉、0 出手 → pass');
  // 2026-09-25 E2 完整性閘：同一趟少了 runStatus done、或少一步查核紀錄、或夾一句假話 → 不過，reasons 寫明哪一項
  const s6g = await q10.score({ finalText: good, steps: cleanSteps, truth: T, drive: { interventions: [] } });
  check(s6g.pass === false && s6g.reasons.some((r) => r.includes('沒跑完')), `沒給 runStatus → 不過（${s6g.reasons.join('、')}）`);
  const s6h = await q10.score({ finalText: good, steps: { numbers: cleanSteps.numbers }, truth: T, drive: { interventions: [] }, runStatus: 'done' });
  check(s6h.pass === false && s6h.reasons.some((r) => r.includes('查核沒跑') && r.includes('points') && r.includes('report')), `只有一步有查核紀錄 → 不過（${s6h.reasons.join('、')}）`);
  const s6i = await q10.score({ finalText: `${good}\n退貨集中在防潑水後背包。`, steps: cleanSteps, truth: T, drive: { interventions: [] }, runStatus: 'done' });
  check(s6i.pass === false && s6i.metrics.false_claims >= 1 && s6i.reasons.some((r) => r.includes('假話')), `夾一句假話 → 不過（${s6i.reasons.join('、')}）`);
  ...
  // 真趟 2144 三步都有查核紀錄（numbers／points redone、report pass）且 run status done——照那趟的形狀傳 runStatus
  const s7 = await q10.score({ arm: 'bojian', finalText: BOJIAN_REPORT_20260924_2144, steps: passSteps7, truth: T, drive: { interventions: [] }, runStatus: 'done' });
  ...
  check(s7.pass === true, `真趟 2144 成品（查核紀錄乾淨、done）→ pass＝${s7.pass}`);
  const s7b = await q10.score({ arm: 'bojian', finalText: BOJIAN_REPORT_20260924_2144, steps: {}, truth: T, drive: { interventions: [] }, runStatus: 'done' });
  check(s7b.pass === false && s7b.reasons.some((r) => r.includes('查核沒跑')), `同一份成品但查核紀錄全無 → pass＝${s7b.pass}（${s7b.reasons.join('、')}）`);
