...
test('預設補值 create 帶 defaults：查核、數字對原始資料、監工、產檔權限、每個 task 節點的三個勾都照設定', () => {
  ...
  assert.deepEqual(out.permissions, { files: false, connectors: true }, '連線權限：親手建的預設開（連線輪）');
...
test('預設補值：defaults 省略＝程式缺省（產檔權限開、不補節點三個勾）；save 模式不碰 facts、不補節點、不補權限、不吃 defaults', () => {
  ...
  assert.deepEqual(plain.permissions, { files: true, connectors: true });
...
test('帳本 Task 1：check 區塊的 null／空陣列輸入——沿 permissions 的慣例，不炸也不誤擋', () => {
  const base = { format: 1, name: 'w', params: [], nodes: [{ id: 'a', title: 'x', executor: 'ai', stop_point: 'never', instruction: 'x', next: [] }] };
  // 複查結果：check／permissions／supervisor 三個區塊對 null 與陣列的處理**一致**（都擋），
  // 帳本當時只是沒測。這裡把一致性釘住——以後有人放寬其中一個，這條會紅。
  assert.doesNotThrow(() => validateWorkflow({ ...base, check: {} }), '空物件＝用預設');
  for (const k of ['check', 'permissions', 'supervisor']) {
    for (const v of [null, [], 'on', 0]) {
      assert.throws(() => validateWorkflow({ ...base, [k]: v }), (e) => e instanceof SchemaError, `${k} 給 ${JSON.stringify(v)} 要擋——三個區塊的規則得一致`);
    }
  }
});

test('L004：參數 key 不能叫 __run__——那是開跑時「本次附件」的保留鍵，取同名會被靜默吃掉', () => {
  const base = { format: 1, name: 'w', nodes: [{ id: 'a', title: 'x', executor: 'ai', stop_point: 'never', instruction: '用 {{__run__}}', next: [] }] };
  assert.throws(
    () => validateWorkflow({ ...base, params: [{ key: '__run__', label: '附件', default: '' }] }),
    (e) => e instanceof SchemaError && e.message.includes('保留字'),
    '手改 YAML 或匯入別人的檔才碰得到，但碰到時要講清楚，不是默默吃掉',
  );
  validateWorkflow({ ...base, params: [{ key: 'run_x', label: '正常', default: '' }], nodes: [{ ...base.nodes[0], instruction: '用 {{run_x}}' }] });
});

// ---- 連線輪（ADR-006）：步驟 connectors／connectors_set_by、流程 permissions.connectors ----
test('連線：步驟 connectors 要是服務名稱清單、connectors_set_by 只能 ai／user；permissions.connectors 要是開或關；沒帶全部合法（舊流程照舊）', () => {
  validateWorkflow(soloDef({ permissions: { files: false, connectors: true } }, { connectors: ['claude.ai Gmail'], connectors_set_by: 'user' }));
  validateWorkflow(soloDef({}, { connectors: [] }));
  validateWorkflow(soloDef());
  const bad = (extra, node) => assert.throws(() => validateWorkflow(soloDef(extra, node)), SchemaError);
  bad({}, { connectors: 'claude.ai Gmail' });
  bad({}, { connectors: [''] });
  bad({}, { connectors: [3] });
  bad({}, { connectors_set_by: 'me' });
  bad({ permissions: { connectors: 'yes' } });
});

test('連線：新建（create）預設開連線權限，草稿帶了 permissions 沒寫 connectors 也補開、明寫關的照留；存檔（save）不補', () => {
  assert.equal(applyDefaults(soloDef(), { mode: 'create' }).permissions.connectors, true);
  assert.deepEqual(applyDefaults(soloDef({ permissions: { files: true } }), { mode: 'create' }).permissions, { files: true, connectors: true });
  assert.equal(applyDefaults(soloDef({ permissions: { files: true, connectors: false } }), { mode: 'create' }).permissions.connectors, false);
  assert.equal(applyDefaults(soloDef(), { mode: 'save' }).permissions, undefined);
});
